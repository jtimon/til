...
- Process and NEXT_ISSUE counter are in `issues/process.org`.
- New issues go at the top of their priority, numbered from NEXT_ISSUE.
- When work starts, change OPEN -> WIP in the title line.
- Fixed issues move to `issues/fixed/`, cancelled to `issues/cancelled/`.
- `make test` regenerates `issues/summary.org` and updates `NEXT_ISSUE` in `issues/process.org`.
