...
CTIL=bin/ctil
TESTS="src/test/types.til src/test/operators.til src/test/functions.til src/test/structs.til src/test/optional_args.til src/test/forward_refs.til src/test/flow.til src/test/strings.til"
FAIL=0
...
if [ "$errors" -ne 31 ]; then
    echo "FAIL: typer_errors expected 31 errors, got $errors"
    FAIL=1
fi
