...
TokenType lookup_single_char(I8 * c);
...
Bool init_is_lift_target(Expr * stmt);
Str * init_lookup_name(Map * renamings, Str * name);
void init_rename_ident_in_place(Expr * e, Map * renamings);
void init_rename_decl_in_place(Declaration * dd, Map * renamings);
void init_rewrite_refs(Expr * e, Map * renamings);
void init_lift_in_body(Expr * body, Str * parent_prefix, Vec * top_level);
void init_lift_in_ns_decls(Expr * def, Str * parent_prefix, Vec * top_level);
void init_recurse_into_subbodies(Expr * body, Str * parent_prefix, Vec * top_level);
void init_recurse_into_expr(Expr * e, Str * parent_prefix, Vec * top_level);
...
Str * err_slot_name(Str * t);
Expr * make_int_lit(I64 n, U32 line, U32 col);
Expr * make_typed_mut_decl(Str * name, Str * type_name, Expr * init, U32 line, U32 col);
Expr * make_assign_stmt(Str * name, Expr * rhs, U32 line, U32 col);
Expr * make_default_value_expr(Str * type_name, U32 line, U32 col);
I64 * tag_for_type(Str * type_name, Vec * _type_to_tag_names);
void register_throw_type(Str * name);
Expr * make_if_kind_eq_zero_wrap(Expr * stmt, U32 line, U32 col);
void rewrite_ident(Expr * e, Str * old_name, Str * new_name);
Expr * lower_catch_node(Expr * catch_node, Vec * type_to_tag_names);
...
Bool vec_str_contains(Vec * v, Str * name);
void vec_str_push_uniq(Vec * v, Str * name);
void validate_one_fcall_consolidated(Expr * e, Vec * pending, Vec * seen, Vec * types_to_declare, Str * path);
void inject_err_args_on_one_fcall_consolidated(Expr * e);
void validate_and_inject_expr_consolidated(Expr * e, Vec * pending, Vec * seen, Vec * types_to_declare, Str * path);
Expr * build_bang_lowered(Str * error_type, U32 line, U32 col);
void hoist_walk_consolidated(Expr * parent, U32 * child_idx, Vec * pre_stmts, Vec * types_to_declare, Vec * seen);
void hoist_nested_bangs_consolidated(Expr * stmt, Vec * pre_stmts, Vec * types_to_declare, Vec * seen);
void process_throw_catch_in_body(Expr * body, Vec * fdef_throws, Vec * pending, Vec * seen, Vec * types_to_declare, Str * path);
...
Bool body_pre_passes(Expr * body, TypeScope * scope, I32 owns_scope);
...
Bool dce_one_body(Expr * body, Bool at_top_level);
Bool match_named_delete(Expr * stmt, Str * name);
void fix_stale_litstr_in_body(Expr * body);
void body_dce_walk(Expr * e, Bool at_top_level);
...
Bool pod_ctor_args_are_safe(Expr * ctor);
...
void emit_func_forward_decl(File * f, Str * name, Expr * fdef, Bool top_level);
...
Str * dyn_type_str_display(Str * vn);
Bool find_loaded_enum_variants(Str * ename, Vec * variant_names, Vec * variant_types);
...
Bool bind_is_ws(I8 * c);
U32 * skip_ws(Str * s, U32 from);
U32 * skip_word(Str * s, U32 from);
U32 * trim_right(Str * s, U32 from, U32 to);
Bool range_eq_lit(Str * s, U32 start, U32 end, Str * lit);
I64 * parse_c_int_literal(Str * s, U32 from, U32 to, Bool * success);
Bool * push_til_base(Str * out, Str * c);
void register_alias(Str * name);
void register_dynamic_alias(Str * name);
Bool is_alias_to_primitive(Str * name);
...
U32 * emit_typedef_enum(Str * out, Vec * lines, U32 start, U32 open, U32 close);
Bool * emit_typedef_alias(Str * out, Str * line);
Bool * emit_typedef_fnptr(Str * out, Str * line);
Bool * emit_define(Str * out, Str * line);
Bool * emit_function(Str * out, Vec * lines, U32 start, U32 end);
void walk_header(Str * out, Vec * lines);
Str * header_dir_of(Str * path);
