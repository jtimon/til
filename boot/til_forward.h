...
Bool enum_variant_is_payload_less(Expr * enum_def, Str * variant_name);
...
Bool fdef_returns_unborrowable(FunctionDef * fd, TypeScope * scope);
...
Bool is_shallow_enum(Str * type_name, TypeScope * scope, Context * ctx);
Bool is_shallow_type_name(Str * type_name, TypeScope * scope, Context * ctx);
...
I64 priv___src_self_typer_til__registry_exact_idx(Context * ctx, Str * key);
I64 priv___src_self_typer_til__registry_suffix_idx(Context * ctx, Str * method);
I64 priv___src_self_typer_til__find_callee_throws_idx(Context * ctx, Expr * fcall, Expr * body);
...
Str * delete_call_free_lit(Str * type_name, Bool set_free, Context * ctx);
Bool callee_param_is_shallow(Str * callee_name, U32 arg_index, Context * ctx);
Str * enum_eq_param_ctype(Str * type_name, Expr * eq_fdef, Bool has_eq);
...
void emit_return_box(File * f, Expr * e, I32 depth, Context * ctx);
...
I32 compile_c(Str * c_path, Str * bin_path, Str * ext_c_path, Str * user_c_path, Str * link_flags, Str * include_flags, Target * target, Str * cc_override, Str * mode_auto_import, Bool asan, Bool prof);
...
I32 compile_ast(LoadedProgram * lp, priv___src_self_builder_til__BuildPaths * paths, Target * target, Str * cc_override, Bool asan, Bool prof);
...
Value val_enum_flat(Str * enum_name, I32 etag, void * payload_data, I32 payload_size, I32 total_enum_size);
Value val_enum_simple(Str * enum_name, I32 etag, I32 total_enum_size);
