...
USize * Map_len(Map * self);
void * Map_key_ptr(Map * self, USize * i);
void * Map_val_ptr(Map * self, USize * i);
...
void Expr_todo_error(Expr * self, Str * path, Str * msg);
void Expr_lang_error(Expr * self, Str * path, Str * msg);
...
Expr * Expr_child(Expr * parent, USize * i);
...
void swap(void * a, void * b, UPtr size);
void move(void * dest, void * src, UPtr size);
Set * Set_new(Type * T);
USize * Set_len(Set * self);
...
Bool * is_target_driven_alias(Str * name);
Str * resolve_alias_for_codegen(TypeScope * scope, Str * name);
...
void hoist_and_insert_field_deletes(Expr * body, TypeScope * scope, I32 do_hoist);
...
Bool func_uses_unknown_globals(Expr * e, Expr * func_def, Scope * precomp_scope, Expr * body_root, Bool lenient);
...
void children_filter(Vec * v, void * marks, USize n);
...
Str * param_ctype(FunctionDef * fd, USize i);
...
Vec * collect_user_argv(LoadedProgram * lp, Array * args, USize start_idx);
