...
*** Vec.get vs Vec.get_by_ref

~Vec.get~ (vec.til:117) does NOT return a value. It copies bytes into a
caller-provided ~mut T: Dynamic~ out-param via ~copy_to_dynamic~ (memcpy).
No ~Ptr.is_borrowed~ is involved -- the caller gets a raw byte copy of the
element, including any Ptr fields. Those Ptrs are ALIASED with the ones
still inside the Vec's buffer.

~Vec.get_by_ref~ (vec.til:131) returns a ~Ptr~ via ~self.ptr.offset(...)~.
~Ptr.offset~ sets ~is_borrowed = 1~. So get_by_ref correctly returns a
borrowed pointer -- no copy, no aliasing.

For the ownership model, ~get~ (memcpy out-param) doesn't play nicely
because it creates aliased Ptrs outside the container with no borrowed
flag. Either callers should use ~get_by_ref~ instead, or ~get~ needs
redesign.
...
#+BEGIN_SRC til
e := vec.get_by_ref(0)    // borrowed -- but garbager can't always tell
