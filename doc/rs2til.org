...
** if let Some with early return (Bug #96 workaround)

When Rust uses ~if let Some~ to check-and-return-early, TIL needs a workaround
because putting ~return~ before ~catch~ causes an "unreachable code" error:
...
TIL (WORKAROUND - wrap in if true):
#+begin_src til
ctx.hoisted_exprs.get(expr_addr, temp_var)
...
The ~if true { }~ wrapper makes the catch reachable because the compiler sees
the return as conditional. See Bug #96 in bugs.org for details.
...
process_type := proc(param_type: Ptr) {  // Ptr to ValueType, NULL = None
    if not(param_type.is_null()) {
        // Read the value via memcpy
        mut vt := ValueType.TCustom("")
        memcpy(to_ptr(vt), param_type.data, size_of(ValueType))
        // use vt
    }
}
#+end_src
...
Use ~memcpy~ to copy from the pointer into a local variable:

#+begin_src til
// 1. Create a default-initialized local variable
mut local_val := ValueType.TCustom("")

// 2. Copy from pointer to local variable
memcpy(to_ptr(local_val), ptr.data, size_of(ValueType))
...
21. ~Option<T>~ struct fields/params -> ~Ptr~ (NULL = None); read with ~memcpy(to_ptr(local), ptr.data, size_of(T))~
22. Variable shadowing: TIL doesn't allow it. Rename in BOTH Rust and TIL to use unique names (see dedicated section)
23. Tuples: TIL doesn't have tuples. Use structs instead (e.g., ~HashSet<(String, usize, usize)>~ -> struct with fields)
