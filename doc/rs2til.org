...
Rust uses ~if let~ for Option destructuring. TIL uses ~get_by_ref~ + ~create_alias~ for
...
TIL (zero-copy with get_by_ref + create_alias):
#+begin_src til
val_ref := map.get_by_ref(key)?
create_alias(val, ValType, val_ref.data)
...
TIL:
#+begin_src til
temp_ref := ctx.hoisted_exprs.get_by_ref(expr_addr)?
create_alias(temp_var, Str, temp_ref.data)
output.push_str(temp_var)
return
catch (err: KeyNotFoundError) {
...
for i in items.len().sub(1)..sub(0, 1) {
    item_ref := items.get_by_ref(i)?
    create_alias(item, T, item_ref.data)
    // use item
}
#+end_src
...
TIL:
#+begin_src til
// Preferred: for-in loop (zero-copy, uses get_by_ref + create_alias internally)
for item: T in items {
    // use item
}

// When you need the index: range loop with get_by_ref + create_alias
...
use range loop with ~get_by_ref~ + ~create_alias~.
...
IMPORTANT: Internal errors (like ~IndexOutOfBoundsError~ from ~Vec.get_by_ref()~) should be caught
...
is_struct_declaration := func(expr: Expr) returns Bool {
    ...
    case NodeType.Declaration(_):
        if not(expr.params.is_empty()) {
            first_ref := expr.params.get_by_ref(0)?
            catch (err: IndexOutOfBoundsError) { return false }
            create_alias(first, Expr, first_ref.data)
...
| ~expr.params[0]~ | ~ref := expr.params.get_by_ref(0)?; create_alias(first, T, ref.data)~ | get_by_ref+create_alias pattern |
...
** Vec/Map access patterns (get_by_ref + create_alias)

Rust returns references directly. TIL uses ~get_by_ref~ + ~create_alias~ for zero-copy access:
...
TIL:
#+begin_src til
item_ref := vec.get_by_ref(i)?  // returns Ptr, throws IndexOutOfBoundsError
create_alias(item, T, item_ref.data)  // zero-copy alias into vec storage

val_ref := map.get_by_ref(key)?  // returns Ptr, throws KeyNotFoundError
create_alias(val, V, val_ref.data)  // zero-copy alias into map storage
#+end_src
...
Rust iterates directly over ~(key, value)~ pairs. TIL iterates over keys and uses ~get_by_ref~ for values:
...
for member_name: Str in struct_def.default_values.keys {
    val_ref := struct_def.default_values.get_by_ref(member_name)?
    create_alias(default_expr, Expr, val_ref.data)
...
- TIL: ~for key: KeyType in map.keys~ + ~get_by_ref~ + ~create_alias~ - iterate keys, get values by ref
...
lookup_func := func(self: ScopeStack, name: Str) returns SFuncDef throws KeyNotFoundError {
    for frame: ScopeFrame in self.frames {
        if frame.funcs.contains_key(name) {
            val_ref := frame.funcs.get_by_ref(name)?
            create_alias(func_def, SFuncDef, val_ref.data)
            return func_def
...
// When index is needed: get_by_ref + create_alias
for i in vec.len().sub(1)..sub(0, 1) {
    item_ref := vec.get_by_ref(i)?
    create_alias(item, T, item_ref.data)
    // use item

    catch (err: IndexOutOfBoundsError) {
        panic(loc(), "get_by_ref can't fail: i is always within bounds: ", err.msg)
    }
}
#+end_src
...
get_value := func(map: Map, key: Str) returns Str throws KeyNotFoundError {
    val_ref := map.get_by_ref(key)?
    create_alias(result, Str, val_ref.data)
    return result
}
...
10. ~vec[i]~ / ~map.get(key)~ -> ~get_by_ref~ + ~create_alias~ (zero-copy)
...
20. HashMap iteration: ~for (k, v) in &map~ -> ~for k: T in map.keys { ref := map.get_by_ref(k)?; create_alias(v, V, ref.data) }~
