...
Rust uses ~if let~ for Option destructuring. TIL uses ~get~ + ~create_alias~ for
...
TIL (zero-copy with get + create_alias):
#+begin_src til
val_ref := map.get(key)?
...
TIL:
#+begin_src til
temp_ref := ctx.hoisted_exprs.get(expr_addr)?
...
TIL:
#+begin_src til
// Preferred: for-in loop (zero-copy, uses get + create_alias internally)
for item: T in items {
    // use item
}

// When you need the index: range loop with get + create_alias
...
use range loop with ~get~ + ~create_alias~.
...
| ~expr.params[0]~ | ~ref := expr.params.get(0)?; create_alias(first, T, ref.data)~ | get+create_alias pattern |
...
** Vec/Map access patterns (get + create_alias)

Rust returns references directly. TIL uses ~get~ + ~create_alias~ for zero-copy access:
...
TIL:
#+begin_src til
item_ref := vec.get(i)?  // returns Ptr, throws IndexOutOfBoundsError
create_alias(item, T, item_ref.data)  // zero-copy alias into vec storage

val_ref := map.get(key)?  // returns Ptr, throws KeyNotFoundError
...
for member_name: Str in struct_def.default_values.keys {
    val_ref := struct_def.default_values.get(member_name)?
...
- TIL: ~for key: KeyType in map.keys~ + ~get~ + ~create_alias~ - iterate keys, get values by ref
...
// When index is needed: get + create_alias
...
10. ~vec[i]~ / ~map.get(key)~ -> ~get~ + ~create_alias~ (zero-copy)
...
20. HashMap iteration: ~for (k, v) in &map~ -> ~for k: T in map.keys { ref := map.get(k)?; create_alias(v, V, ref.data) }~
