...
Note: Functions returning simple Vec types (Vec<I64>, Vec<U8>, Vec<Str>) can use ~func~ normally.
For nested types (Vec<Vec<T>>, List, Map), use ~proc~ to prevent precomputation (Bug #133 partial fix).
