...
| ~.unwrap()~ | ~!~ | Panic on throw (Issue #180) |
| ~match~ on Option/Result | ~switch~ | Explicit case handling |

~.unwrap()~ maps to the ~!~ operator (Issue #180). Use ~!~ instead of ~?~ when
you want a panic on error, just like Rust's ~.unwrap()~.
...
Rust uses ~if let~ for Option destructuring. TIL uses ~get~ + ~cast~ for
zero-copy access, or ~contains_key~ for existence-only checks:
...
TIL (zero-copy with get + cast):
#+begin_src til
val := cast(ValType, map.get(key)?)
// use val (zero-copy alias into map storage)

catch (err: KeyNotFoundError) {
    // REM: Rust's None case did nothing
}
#+end_src

TIL (existence check only, when you don't need the value):
#+begin_src til
if map.contains_key(key) {
    // key exists, don't need the value
}
#+end_src
...
TIL:
#+begin_src til
temp_var := cast(Str, ctx.hoisted_exprs.get(expr_addr)?)
output.push_str(temp_var)
return
catch (err: KeyNotFoundError) {
...
TIL:
#+begin_src til
// Preferred: for-in loop (zero-copy, uses get + cast internally)
for item: T in items {
    // use item
}

// When you need the index: range loop with get + cast
...
Note: TIL's ~for item: T in vec~ iterates by reference (zero-copy). For indexed access,
use range loop with ~get~ + ~cast~.
...
** The ? operator in TIL (Issue #132)

In Rust, ~?~ propagates errors -- it unwraps Ok or returns Err early.

In TIL, ~?~ is NOT propagation. It marks that a call may throw, and that
the error is not unwrapped with ~!~. Whether the error is caught or
propagated is determined by catch blocks, not by ~?~ itself. The ~?~ is
a required annotation -- use ~?~ when you want errors to be catchable or
to propagate, use ~!~ when you want to panic on error (see Issue #180).
...
#+begin_src til
x := foo()?  // TIL: marks that foo() may throw, does NOT propagate
// Error is caught by a catch block (if present) or propagated (if not)
#+end_src

Key differences from Rust:
- In Rust, ~?~ is optional (you can match on Result instead). In TIL, ~?~ is MANDATORY
- In Rust, ~?~ propagates the error. In TIL, ~?~ just marks the call as throwing
...
** The ! operator in TIL (Issue #180)

The ~!~ operator is the TIL equivalent of Rust's ~.unwrap()~. It marks a call
that may throw, but instead of propagating the error, it panics.

#+begin_src rust
// Rust
let result = might_fail(x).unwrap();  // panics if Err
#+end_src
...
The desugarer transforms ~func()!~ into ~func()?~ followed by a catch block
that calls ~panic()~ for each thrown error type:
...
// After desugaring:
result := might_fail(x)?
catch (_bang_err_0: SomeError) { panic(loc(), _bang_err_0.msg) }
#+end_src

Key points:
- ~!~ can only be used on calls to throwing functions (typer error otherwise)
- Works for direct function calls; method calls (UFCS) are not yet supported
...
is_struct_declaration := func(expr: Expr) returns Bool {
    ...
    case NodeType.Declaration(_):
        if not(expr.params.is_empty()) {
            first := cast(Expr, expr.params.get(0)?)
...
| ~expr.params[0]~ | ~first := cast(T, expr.params.get(0)?)~ | get+cast pattern |
...
** Vec/Map access patterns (get + cast)

Rust returns references directly. TIL uses ~get~ + ~cast~ for zero-copy access:
...
TIL:
#+begin_src til
item := cast(T, vec.get(i)?)  // zero-copy alias into vec storage

val := cast(V, map.get(key)?)  // zero-copy alias into map storage
#+end_src

For existence checks only (don't need the value):
#+begin_src til
if map.contains_key(key) {
    // key exists
}
...
** Mutable access with cast (in-place mutation)

~cast~ can be used in ~mut~ declarations to get a mutable alias into collection
storage, allowing in-place mutation without extracting and reinserting:

Rust (~deps[idx]~ returns a mutable reference via indexing):
#+begin_src rust
// ccodegen.rs
deps[idx].push(dep_idx);
#+end_src

TIL (~mut~ + ~cast~ gives a mutable alias into Vec storage):
#+begin_src til
// ccodegen.til
mut dep_vec := cast(Vec, deps.get(idx)?)
dep_vec.push(dep_idx)  // modifies the Vec in-place inside deps
#+end_src

Rust (~.last_mut()~ returns a mutable reference to last element):
#+begin_src rust
// init.rs
if let Some(frame) = self.frames.last_mut() {
    frame.heap_index.set(name.clone(), offset);
...
#+begin_src til
// init.til
mut current_frame := cast(ScopeFrame, self.frames.get(last_idx)?)
current_frame.heap_index.set(name.clone(), offset)
current_frame.symbols.set(name, symbol)
#+end_src

This works because ~cast~ creates a zero-copy alias (pointer into collection
storage). When declared ~mut~, you can call mutating methods on it, and the
changes apply directly to the collection element. No need to ~set~ back.

NOTE: The alias is only valid while the collection is not resized. Pushing to
a Vec may reallocate its storage, invalidating existing aliases.
...
Rust iterates directly over ~(key, value)~ pairs. TIL iterates over keys and uses ~get~ for values:
...
for member_name: Str in struct_def.default_values.keys {
    ...
    case:
        ...
        // REM: key came from .keys, should never happen
    }
}
#+end_src
...
- TIL: ~for key: KeyType in map.keys~ + ~get~ + ~cast~ - iterate keys, get values by ref
- Catch is technically unnecessary (key came from .keys), but compiler requires it
...
lookup_func := func(self: ScopeStack, name: Str) returns SFuncDef throws KeyNotFoundError {
    for frame: ScopeFrame in self.frames {
        if frame.funcs.contains_key(name) {
            func_def := cast(SFuncDef, frame.funcs.get(name)?)
            return func_def

            catch (err: KeyNotFoundError) {
                // REM: contains_key was true, should never happen
            }
        }
    }
...
#+begin_src til
// Preferred: for-in loop (no index errors possible)
for item: T in vec {
    // use item
}

// When index is needed: get + cast
for i in vec.len().sub(1)..sub(0, 1) {
    item := cast(T, vec.get(i)?)
    // use item

    catch (err: IndexOutOfBoundsError) {
        panic(loc(), "get can't fail: i is always within bounds: ", err.msg)
    }
}
#+end_src
...
TIL (preferred: for-in loop avoids index errors entirely):
#+begin_src til
...
TIL (when index is needed):
#+begin_src til
process := func(items: Vec) returns Result throws Str {
    for i in 0..items.len() {
        item := cast(Item, items.get(i)?)
        // ...
    }
    catch (err: IndexOutOfBoundsError) { throw err.msg }
}
...
10. ~vec[i]~ / ~map.get(key)~ -> ~get~ + ~cast~ (zero-copy); ~mut~ + ~cast~ for in-place mutation
...
20. HashMap iteration: ~for (k, v) in &map~ -> ~for k: T in map.keys { v := cast(V, map.get(k)?) }~
...
27. ~.unwrap()~ -> ~!~ operator: ~foo().unwrap()~ -> ~foo()!~ (panics on throw, Issue #180)
