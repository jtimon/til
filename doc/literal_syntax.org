...
|----------+-----------------------------------+----------------------------------------------|
| CIL      | start_fib: [3]I64 = [1, 1, 2]      | Vec2 := struct { x: I64; y: I64 } \n v: Vec2 = Vec2{ x: 1, y: 2 } |
...
* Notes:
- CIL intends to use square brackets for array literals (`[1, 1, 2]`) and braces for struct literals (`Vec2{ x: 1, y: 2 }`).
...
* Observations:
- CIL's plan resembles Rust but drops the confusing dot notation of Jai.
