...
** DRY #18: Conditional Parameter Validation                        :LOW:
:PROPERTIES:
:DISCOVERED: 2025-01-10
:IMPACT: 4 locations in typer.rs and interpreter.rs
:EFFORT: 1 hour
:LOCATIONS: typer.rs lines 163-166, 202-206; interpreter.rs lines 80-82, 98-100
:BENEFIT: Consistency improvement, minimal line savings
:END:

*** Description
Similar parameter count validation for if/while statements repeated in both type checker and interpreter.

*** Current Pattern
#+BEGIN_SRC rust
// In typer.rs and interpreter.rs:
if e.params.len() != 2 {
    return Err(e.exit_error("type", "If nodes must have exactly 2 parameters."))
}

if e.params.len() != 2 {
    return Err(e.exit_error("type", "While nodes must have exactly 2 parameters."))
}
#+END_SRC
...
fn validate_conditional_params(
    ...
    if e.params.len() != expected {
        return Err(e.exit_error("type", &format!(
            "{} nodes must have exactly {} parameters.", stmt_type, expected
        )));
    }
    Ok(())
}
...
// Usage:
validate_conditional_params(e, "If", 2)?;
validate_conditional_params(e, "While", 2)?;
#+END_SRC

*** Affected Files
- typer.rs: 2 locations
- interpreter.rs: 2 locations

*** Estimated Savings
~10 lines (minimal, but improves consistency)
...
(Empty - refactorings will be moved here as they are completed)
...
| LOW      |     3 | 4 hours                | ~55 lines         |
|----------+-------+------------------------+-------------------|
| TOTAL    |    10 | 25-33 hours            | 660-760 lines     |
