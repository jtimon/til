...
** TODO Fix Variadic Argument Handling                              :MEDIUM:
:PROPERTIES:
:FILE: src/rs/typer.rs:1175, src/core/typer.til:188
:END:

*** Current State
- =src/rs/typer.rs:1175= - "TODO: Check the args too" for variadic functions
- =src/core/typer.til:188= - Variadic args not properly handled
- Tests in =src/test/variadic.til= and =src/test/variadic_type_check.til= pass but may not cover all cases

*** What's Needed
- [ ] Complete variadic argument type checking in typer.rs
- [ ] Add comprehensive variadic tests
- [ ] Document variadic function semantics
- [ ] Handle edge cases (zero variadic args, type mismatches)

*** Why This Matters for Self-Hosting
Self-hosted code may need variadic functions for flexible APIs. Type checking must be complete.
...
*** What's Needed
- [ ] Add missing type checks identified in comments
- [ ] Test struct member type checking
- [ ] Validate default values don't call procs
- [ ] Ensure function arguments don't call procs inappropriately
- [ ] Handle enum types in TCustom context
...
1. [ ] Fix variadic argument handling
2. [ ] Improve type checking edge cases
3. [ ] Better error messages and backtraces
