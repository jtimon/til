...
- [X] Implement pass-by-reference semantics with three argument modes
  ...
  - [X] Bug #26: Vec/Map corruption with nested structs (FIXED)
    ...
    - All tests pass (exit code 0)
  - [X] Phase 4: Update core library to remove Bug #24 workarounds (COMPLETE)
    - Status: COMPLETE - All Bug #24 workarounds removed from codebase
    - Removed copy-modify-reassign patterns (e.g., tmp = x; tmp.field = val; x = tmp)
    - Workarounds removed from:
      - List.push(), List.set(), List.pop() - now use direct field mutations
      - Map.insert(), Map.set() - now use direct field mutations
      - lexer.til, parser.til - no workarounds found (already clean)
      - test files - no workarounds found (already clean)
    - All tests pass (exit code 0)
    - Performance: ~42s (unchanged - copying small structs was already fast)
...
- [ ] Bug #28: Fix Variable Shadowing Bug (HIGH PRIORITY)
  - Status: BUG CONFIRMED - must fix before self-hosting
  - Design decision: TIL will ALLOW shadowing (like Rust) - decided 2025-11-20
  - Problem: Nested-scope variable declarations MUTATE outer variables instead of shadowing
    #+BEGIN_SRC til
    test_shadowing := proc() {
        z := 5
  ...
  - Root Cause:
    ...
    - When inner := is used, it mutates the HashMap entry → looks like mutation not shadowing
  - Residual Code Preventing Shadowing (needs cleanup):
    - src/rs/init.rs:898-900 - init_context prevents global shadowing (ACTIVE - needs removal)
    - src/rs/init.rs:120-137 - declare_var has shadowing check (UNUSED - never called)
  - Why Critical for Self-Hosting:
  ...
  - Implementation approach:
    - Option A: Create new scope frames for block expressions
    - Option B: Handle shadowing within same frame (stack-based bindings)
    - Need to ensure outer binding is restored after inner scope exits
