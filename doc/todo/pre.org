...
:IMPACT: Simplifies compiler -- removes 321 is_copy references across 23 files
:STATUS: Open
:END:

*** Overview

The ~copy~ parameter mode (~func(copy x: T)~) is syntactic sugar: the garbager
auto-inserts ~.clone()~ at call sites for struct-typed identifier args. The user
can achieve identical semantics with ~own~ params and calling ~.clone()~ explicitly
at the call site. Removing ~copy~ simplifies the compiler significantly.

Can be re-added later as a desugarer pass if desired.

*** What copy currently does

1. *Parser*: Recognizes ~copy~ keyword, sets ~Declaration.is_copy = true~
2. *Type checker*: Validates struct has ~.clone()~, excludes copy params from ~borrowed_args~
3. *Garbager*: ~transform_fcall_copy_params()~ wraps identifier args in ~Type.clone()~ at call sites
4. *Ccodegen*: Copy params passed by value (no ~*~), NOT added to ~current_ref_params~
5. *All other files*: Propagate ~is_copy~ flag through SymbolInfo, Declaration, etc.

*** Migration

~copy x: T~ becomes ~own x: T~ with caller writing ~.clone()~ explicitly.
For I64 primitives, ~copy~ is meaningless (already value-type in C), so those
become plain immutable params.

#+BEGIN_SRC til
// Before: copy param (garbager auto-clones)
test_copy_struct : proc(copy v: Vec2) returns Vec2 = { ... }
result := test_copy_struct(original)

// After: own param (caller clones explicitly)
test_copy_struct : proc(own v: Vec2) returns Vec2 = { ... }
result := test_copy_struct(own original.clone())
#+END_SRC

*** Impact analysis

| Area                | Rust files | TIL files | Total |
|---------------------+------------+-----------+-------|
| is_copy references  |        168 |       153 |   321 |
| Files touched       |         12 |        11 |    23 |
| Functions deleted   |          1 |         1 |     2 |
| Test lines removed  |       ~150 |         - |  ~150 |

No compiler source code uses ~copy~ params -- only ~src/test/args.til~ tests.

Most ~is_copy~ occurrences are boilerplate field propagation (~is_copy: false~,
~is_copy: arg.is_copy~). Real logic removed: garbager auto-clone (~114 lines),
typer clone-check (~40 lines), ccodegen by-value passing (~20 lines), parser
keyword handling (~20 lines). Total ~200 lines of actual logic, ~300 lines of
boilerplate.

*** Removal plan

**** Step 1: Remove is_copy from AST and data structures
- ~src/rs/ast.rs~ -- remove ~is_copy~ from Declaration struct
- ~src/rs/init.rs~ / ~src/self/init.til~ -- remove ~is_copy~ from SymbolInfo and all initializations

**** Step 2: Remove from parser/lexer
- ~src/rs/lexer.rs~ / ~src/self/lexer.til~ -- remove TokenType::Copy, keyword mapping
- ~src/rs/parser.rs~ / ~src/self/parser.til~ -- remove copy parsing, validation, Declaration field

**** Step 3: Remove from type checker
- ~src/rs/typer.rs~ / ~src/self/typer.til~ -- remove clone-check validation, borrowed_args exclusion

**** Step 4: Remove from garbager
- ~src/rs/garbager.rs~ / ~src/self/garbager.til~ -- delete ~transform_fcall_copy_params()~ entirely, remove call site

**** Step 5: Simplify ccodegen
- ~src/rs/ccodegen.rs~ / ~src/self/ccodegen.til~ -- simplify ~param_needs_by_ref()~, remove copy branch in signature generation, remove copy exclusion from ~current_ref_params~

**** Step 6: Remove from remaining files
- precomp, interpreter, desugarer, preinit, eval_heap, ufcs (Rust + TIL pairs)
- All just boilerplate ~is_copy~ field propagation

**** Step 7: Update tests
- ~src/test/args.til~ -- delete copy test section (lines 296-446) or convert to ~own~ + ~.clone()~ equivalents

**** Step 8: Update documentation
- ~doc/modifiers.org~ -- remove copy from param mode table
...
:STATUS: Step 1 done (existing), Step 2 done (namespace mutable statics), Step 3 open
...
:PROPERTIES:
:DISCOVERED: 2026-02-23
:IMPACT: Memory leak reduction -- first step in memory management pipeline
:STATUS: In Progress
:RELATED: Issue #188 (defer, step 2), Bug #170 (scope-exit, step 3), Issue #117 (ASAP, step 4)
:END:

*** Memory management strategy (pipeline order)

1. *#191 (done)* -- garbager inserts ~Type.delete(var)~ before reassignment of owned locals.
2. *#183 (this)* -- manual .delete() calls in TIL source. Current work.
3. *#188 (defer)* -- ~defer~ statement for scope-exit cleanup. AST desugaring.
4. *#170 (scope-exit deletion)* -- garbager inserts deletes at scope boundaries. Depends on #188.
5. *#117 (ASAP destruction)* -- fully automated last-use deletion. Final step.

Note: #191 already handles the reassignment leak pattern automatically. Manual
deletes (#183) are for end-of-scope / end-of-use patterns only.

Each step subsumes the previous: when #188 lands, some #183 manual deletes become
~defer~. When #170 lands, scope-exit handles most cases automatically. When #117
finishes, all deletion is automated and manual deletes are removed.

*** Description

Add manual ~.delete()~ calls in TIL source code where values are allocated
and never freed -- the same way you would in C. Work through files
incrementally: a few functions at a time, one commit per batch. No need to
be exhaustive in any single pass -- just add obviously safe deletes, skip
anything uncertain, and come back for more later. Multiple passes per file
...
| 5b0d7bbd  | Remove #117 auto-delete from garb   | 7701 MB         | 21318 MB  |
| 5e96534c  | lexer.til: lookahead/tmp_substr     | 7700 MB (-1)    | 21318 MB  |

*** Done so far

- ~scope_stack.pop()?.delete()~ on 28 sites. No improvement.
- ~.delete()~ on 2 cloned StructDef locals (-93 MB).
- lexer.til: ~lookahead~ and ~tmp_substr~ for two-char checks, semicolons,
  comments, fractional dot scanning (-1 MB). First pass, not exhaustive.
...
Files:
- [X] lexer.til -- lookahead, tmp_substr for two-char checks, comments, semicolons
- [X] parser.til -- to_str/concat temporaries in next_loop_var, get_combined_name, etc.
- [X] init.til -- concat chains in get_field_offset, to_str in collect_anon_funcs, import paths
- [X] typer.til -- lookup_func results, concat chain in check_fcall, import path
- [X] preinit.til -- format() temporaries for contains() checks
- [ ] desugarer.til
- [ ] ufcs.til
- [ ] garbager.til
- [ ] precomp.til
...
*** Phase 1: Reject invalid usage in typer -- DONE (2026-02-24)

The typer now detects and rejects ~cast()~ in unsupported positions:
1. As a function argument: ~func(cast(I64, x))~
2. In return/throw: ~return cast(I64, x)~
3. In assignment: ~x = cast(I64, y)~

Added ~is_cast_call()~ helper (Rust + TIL) and checks in ~check_fcall~,
~check_assignment~, and Return/Throw handlers.
...
:STATUS: Open - partial implementation (Steps 1a/1b/1c done, Steps 2-11 not started)
:RELATED: Bug #168 (ret_var_alias optimization), Bug #173 (missing *_ret assignment), Bug #159 (shallow copies), doc/first_class.org (function pointer typedefs)
:END:
...
*** Work done (incremental approach, bottom-up)

Rather than implementing all 11 steps at once (which failed on branch oom175
due to OOM), taking an incremental approach: make non-throwing struct-returning
functions use ~_ret~ first, then unify calling conventions later.

**** Step 1a: Non-throwing struct-returning functions use ~_ret~ (140e868b)

Changed ~fcall_uses_out_ret~ to return true for non-throwing functions that
return structs with mutable data fields (or enums). These now use destination-
passing style (~void func(RetType* _ret, ...)~) matching throwing functions.
Ext functions and primitive-returning functions still use direct return.
Guarded ~ret_var_alias~ at all three emit sites to only activate when the
function has a ~_ret~ parameter.

Impact: generated C grew from 103.5K to 134.5K lines (+30%) because each
~_ret~ call is 2 lines (~Type tmp; func(&tmp, ...);~) instead of 1
(~Type tmp = func(...);~). Plus ~6359 remaining ~= _tmp_~ copies from
declarations where the call wrote to a temp but the declaration variable
still copied from it.

**** Step 1b: Eliminate redundant temp-to-temp copies (c50ee7c4)

When a ~_ret~ FCall result was passed as a by-ref or Dynamic argument,
~emit_arg_string~ created a second temp and copied from the first. Since the
~_ret~ temp is already a declared lvalue, we now take ~&~ of it directly.
Added ~fcall_expr_is_hoisted~ helper in TIL for safe KeyNotFoundError handling.

**** Step 1c: Unify declaration FCall handling with ~emit_ret_call~ (this commit)

Added ~emit_ret_call~ -- a new function that intercepts non-throwing ~_ret~
FCall declarations in ~emit_stmts~, writing directly to the destination
variable via ~build_dest_ptr_expr~ instead of going through
~emit_fcall_arg_string~ -> temp -> ~emit_declaration~ -> copy.

Key finding: *aliasing prevents assignment optimization*. For assignments like
~cloned.ptr = Vec._alloc_ptr(cloned, cap)~, the dest_ptr (~&cloned.ptr~)
overlaps with function arguments (~&cloned~). Inside ~_alloc_ptr~,
~Ptr_new_by_size~ zeroes ~*_ret~ (which IS ~cloned.ptr~), destroying
~self->ptr.elem_type/elem_size~ before they're read. The temp copy served as
a buffer preventing this aliasing. So the interception is restricted to
*declarations only*, where the fresh variable can't alias anything.

Results: 130.9K lines (down from 134.5K), 4477 ~= _tmp_~ copies (down from
6359). The ~has_ret~ hoisting path in ~emit_fcall_arg_string~ is left in place
-- still needed for nested FCalls (when an ~_ret~ call appears as an argument
to another call).
...
:STATUS: Paused -- auto-delete code removed, will re-implement after #188/#170
:DEPENDS: Bug #170 (scope-exit deletion), Issue #188 (defer), Bug #101 (unused warnings)
:RELATED: Bug #114 (memory leak documentation), Bug #144 (for-in by-ref), Bug #168 (ccodegen return copies), Issue #183 (manual deletes), Issue #191 (reassignment deletes, now fixed)
:END:
...
:STATUS: Blocked by #188 (defer) -- step 4 of memory management pipeline
:DEPENDS: Issue #188 (defer statement)
:RELATED: Bug #165 (memory leak), Bug #159 (shallow copies), Issue #117 (ASAP destruction), Issue #183 (manual deletes), Issue #191 (reassignment deletes, now fixed)
:END:
...
:STATUS: Open -- Phase 1 done, Phase 2 unblocked (Issue #91 has func ptrs), Phase 5 partially done (#191), Phases 3/4/6 not started. Issue #175 Steps 1a-1c reduce ccodegen shallow copies for declarations.
:RELATED: Bug #165 (memory leak blocked by this), Bug #168 (ccodegen return copies), Bug #114 (ScopeFrame leak), Bug #170 (scope-exit deletion blocked by this), Issue #91 (function pointers needed for deep clone/delete), Issue #191 (delete-before-reassignment blocked by this)
:END:
...
**** 1. ccodegen -- Function return values (partially addressed by Bug #168, Issue #175)
- =*_ret = value;= -- 696 remaining (was 976, commits 1/1b/2 eliminated 280)
- Non-throwing by-value returns: eliminated by Issue #175 Step 1a (all non-ext
  struct-returning functions now use =_ret= out-parameter)
- Issue #175 Step 1c added =emit_ret_call= to write directly to destination for
  declarations, reducing temp-to-var copies from 6359 to 4477. Assignment
  optimization blocked by aliasing (see Issue #175 Step 1c key finding).
...
*** Issue #191 finding: scope lookup shallow copies (2026-02-24)

Issue #191 implemented delete-before-reassignment but had to disable it because
locals holding values from =lookup_func=, =lookup_symbol=, =lookup_struct= are
shallow copies (memcpy) that share heap pointers with the scope stack. Deleting
them corrupts the scope stack, causing the typer to report incorrect "unused
variable" errors.

In Rust, these functions return =&references= (zero-copy, no ownership). In TIL,
there are no references, so the returned struct is a value copy via memcpy. The
=cast_vars= exclusion in the garbager only catches =cast()= at declaration time,
not the common pattern of declaring empty then assigning from a lookup:

#+begin_src til
mut func_def := FuncDef()
// garbager inserts: FuncDef.delete(func_def)  -- CORRUPTS scope stack!
func_def = context.scope_stack.lookup_func(name)?
#+end_src

This is a specific instance of the general shallow copy problem. Possible fixes:
1. Make lookup functions return deep clones (expensive but safe)
2. Track which locals hold aliases at a finer granularity (complex)
3. Wait for Phase 6 (full ownership tracking) to solve it holistically
...
5. *Collection .get()*: All copying .get() removed (Phase 1 done). Remaining:
   Vec.pop/List.pop use copy_to_dynamic (mitigated by clone-after-get, Step 9).
6. *Scope lookup functions*: =lookup_symbol=, =lookup_func=, =lookup_struct= return
   value types in TIL (Rust returns =&references=). The returned struct is a shallow
   memcpy that shares heap pointers with the scope stack. Deleting such a local
   corrupts the scope stack. This blocks Issue #191 delete-before-reassignment.
...
**** Phase 2: Function pointer builtins (Issue #91, minimal scope) -- UNBLOCKED
Deep Vec.clone/delete requires calling Type.clone()/Type.delete() on type-erased
Dynamic elements. Issue #91 now has function pointer support (FunctionSig type +
function pointers done as of 2026-02-21). The original plan used raw I64 hacks;
with real function pointers available, the implementation can be cleaner.

Minimal fix: 4 new builtins (or use function pointer values directly):
...
**** Phase 5: Garbager clone gaps -- PARTIALLY DONE
- Clone ALL Identifier-init declarations (not just mut) -- NOT DONE (is_mut check remains)
- Clone field-access assignments (remove params.is_empty check) -- DONE (Issue #191,
  assignment handler now uses =get_value_type= instead of =lookup_symbol=)
