...
Callees must construct directly into =*_ret= instead of creating a local
and copying. This eliminates the last shallow copy and matches the interpreter.

#+begin_src c
// Current (after Commit 2) -- still 1 shallow copy:
void til_make_thing(til_MyStruct* _ret) {
    til_MyStruct til_MyStruct_result = {0};  // local
    til_MyStruct_result.field = 42;
    *_ret = til_MyStruct_result;             // shallow copy, local leaked
}

// Goal -- zero copies, like the interpreter:
void til_make_thing(til_MyStruct* _ret) {
    *_ret = (til_MyStruct){0};               // construct into dest
    _ret->field = 42;                        // write into dest
    return;                                  // no local, no copy, no leak
}
#+end_src

Approach: detect the "return variable" in the callee and alias it to =_ret=.
When the codegen sees =result := MyStruct()= followed by =return result=, it
should emit construction into =*_ret= and field writes via =_ret->=.
