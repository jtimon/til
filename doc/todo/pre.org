...
- init.til: ~name_copy := name.clone()~ (inline form fails, see below)
...
Zeroing will be handled by the garbager in Step 9 instead, which inserts
~Type.delete()~ calls at the right scope boundaries rather than zeroing
inside container methods.

***** Bug #162 not fully fixed (UFCS + own params)
Two workarounds needed in init.til:
1. ~name.eq("_")~ fails even though ~name: Str~ hasn't been consumed yet.
   Workaround: ~Str.eq(name, "_")~ (non-UFCS form).
2. ~name.clone()~ inline as argument to ~set()~ fails.
   Workaround: ~name_copy := name.clone()~ then ~set(name_copy, ...)~.
The typer's own-param control-flow awareness (Bug #162) doesn't handle
UFCS resolution correctly when the variable is consumed later.
