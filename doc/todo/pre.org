...
- [ ] Implement pass-by-reference semantics with three argument modes
  ...
  - [ ] Phase 3b: Enable pass-by-reference for Dynamic parameters (fixes Bug #26)
    - Status: READY - Type/Dynamic separation complete, can now enable Dynamic pass-by-ref
    - Change: Remove Dynamic from pass-by-ref exclusion list (keep Type excluded)
    - Location: interpreter.rs:1679 (change custom_type_name != "Type")
    - Expected fix: Vec/Map retrieving complex types will work correctly
    - Test: Re-enable test_parser.til struct test (line 423)
    - Bug #26 context:
      - Issue: Vec/Map storing complex types does shallow copy (copies pointers, not data)
      - Effect: Retrieved values have corrupted nested fields (e.g., Expr.params._len = 0)
      - Root cause: Dynamic value parameters needed pass-by-reference, but Type params need copy
      - Solution: Separate Type (copy) from Dynamic (pass-by-ref)
