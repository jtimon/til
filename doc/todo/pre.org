...
This is pervasive in ccodegen.til and other files. Manual ~defer x.delete()~
cannot fix this because:
1. If the variable is reassigned to a borrowed value (from ~cast(Str, map.get())~),
   the defer would delete the map's data, not the original owned value
2. The defer only deletes whatever the variable holds at scope exit, not the
   intermediate owned values that were overwritten

*** Approach

In ~garbager.rs~ / ~garbager.til~, when processing an ~Assignment~ node:
1. Check if the target variable has a custom type (Str, Vec, Map, Set, struct)
2. If so, insert a ~Type.delete(var)~ call before the assignment
3. Skip if the old value is borrowed (from cast/get -- needs alias tracking)
4. Skip if the variable might not be initialized yet (conditional paths)

The safe subset: variables that were initialized with an owning expression
(constructor, clone, concat, split, etc.) and are being reassigned with
another owning expression. This avoids the alias problem.
