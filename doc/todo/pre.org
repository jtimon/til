...
- [ ] Improve test error handling with ERROR: prefix (IN PROGRESS - COMPLETED: dynamic_arrays.til, mut_test.til, pointers.til, enums.til, arrays.til, args.til, ufcs.til, editor_mode_test.til, maps.til (also removed 4 redundant tests); PARTIAL: test_lexer.til (6/28 functions); SKIP: errors.til (all catches test expected errors); REMAINING: test_parser.til, lists.til)
