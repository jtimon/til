...
2. [X] Eliminate Str double-copy via arena_offset in EvalResult - 31a10477
   ...
   Consumer sites (declaration, assignment, param binding, struct fields) use
   the offset directly instead of round-tripping through string_from_context
   + insert_string.
...
Primitives excluded: I64, U8, Type, Dynamic (NOT Str - Str is now cloned/deleted)

**Safety: own-transfer tracking.** If a variable is passed as ~own~ to another
function anywhere in the body, remove it from the delete set (callee deletes it).
Conservative: if own-transfer is conditional (inside if), still exclude.

**Where delete calls are inserted:**
1. Before Return: delete all in-scope candidates (minus the returned variable)
...
5. Call insert_deletes_in_body(new_body, param_candidates, own_transferred) -> final_body

**New helper functions:**
- ~build_delete_call_expr(type_name, var_name, line, col)~ - mirror of build_clone_call_expr
- ~is_deletable_type(value_type, context)~ - TCustom + not primitive + has_struct
- ~collect_own_transfers(stmts, context)~ - scan for own params in FCalls
- ~insert_deletes_in_body(stmts, inherited, own_transferred, context)~ - core recursive
- ~get_returned_var_name(return_expr)~ - extract simple identifier from return
