...
**** Step 1: Change ~emit_func_signature~

In ~emit_func_signature~ (ccodegen.rs:3455), unify the two branches:

- If function has return types AND does not throw: emit ~void~ as return
  type, add ~RetType* _ret~ as first parameter.
- If function has return types AND throws: unchanged (~int~ return,
  ~_ret~ + error params).
- If function has no return types (proc): unchanged (~void~, no ~_ret~).

The ~is_throwing~ branch for return type becomes:
- has returns + throws -> ~int~
- has returns + no throws -> ~void~
- no returns -> ~void~

The ~_ret~ parameter is emitted whenever ~return_types~ is non-empty,
regardless of ~is_throwing~.

Files: src/rs/ccodegen.rs, src/self/ccodegen.til

**** Step 2: Change ~emit_return~

Currently non-throwing functions emit ~return value;~. Change to:
- ~*_ret = value; return;~ (for non-throwing with return type)
- ~*_ret = value; return 0;~ (for throwing -- unchanged)
- ~return;~ (for procs -- unchanged)

The ~ret_var_alias~ optimization now applies to ALL functions with return
types, not just throwing ones.

Files: src/rs/ccodegen.rs, src/self/ccodegen.til

**** Step 3: Change ~emit_fcall~ call sites

Currently non-throwing call sites emit:
~result = func(&a, &b);~

Change to:
~func(&result, &a, &b);~

This means the call-site hoisting logic (declaring result variable, passing
its address) is the same for all functions. The only extra step for throwing
calls is: declare status variable, check status, read error if non-zero.

Files: src/rs/ccodegen.rs, src/self/ccodegen.til

**** Step 4: Extend ~find_ret_var_for_placement~ to non-throwing functions

Currently this optimization only runs for throwing functions. With the
unified convention, it applies to all functions with return types.

Remove the ~is_throwing~ guards around ~find_ret_var_for_placement~ calls
and ~ret_var_alias~ usage.

Files: src/rs/ccodegen.rs, src/self/ccodegen.til

**** Step 5: Update ~emit_declaration~ hoisting

The declaration hoisting for function call RHS currently has separate paths
for throwing (hoist into temp + status check) vs non-throwing (direct assign).
Unify into one path: always hoist result variable, always pass as ~_ret~.

Files: src/rs/ccodegen.rs, src/self/ccodegen.til

**** Step 6: Update ext_func/ext_proc declarations

External function declarations need to match the new convention. Any
~ext_func~ that returns a value now expects a ~_ret~ pointer parameter.

Review all ext_func declarations in src/core/*.til and update their C
implementations if they exist (most are builtins handled by ccodegen).

Files: src/core/*.til, src/rs/ccodegen.rs, src/self/ccodegen.til

**** Step 7: Tests

Run full benchmark. The generated C changes significantly but the behavior
must be identical. All 93+ tests must pass.

*** Notes

- ~main~ function is special -- it returns ~int~ directly to the OS. This
  is already a special case and stays that way.
- Builtins (hardcoded in ~emit_fcall~) need updating too -- they currently
  assume non-throwing functions return directly.
...
For enums, variants come first (with trailing comma on the last one),
then ~namespace:~ introduces associated items. The parser knows:
before ~namespace:~ = variants, after = associated items.

The ~namespace:~ keyword is indented at the same level as the opening brace
(one level less than fields), for visual separation.

*** What goes where

| Location           | What it means                           | Syntax                |
|--------------------+-----------------------------------------+-----------------------|
| Before ~namespace:~ | Per-instance mutable field              | ~mut x : I64 = 0~    |
| After ~namespace:~  | Constant (shared, immutable)            | ~PI := 3~            |
| After ~namespace:~  | Function/method                         | ~add := func(...)...~ |
| After ~namespace:~  | Static mutable (shared, future #161)    | ~static mut c := 0~  |

*** Standalone namespaces (utility modules)

A struct with no fields and only a ~namespace:~ section is a standalone
namespace (zero-size, no instances, just a bag of associated items):
...
**** Step 1: Parser -- recognize ~namespace:~ inside struct bodies

Modify ~parse_struct_definition~ in parser.rs/parser.til:

After parsing the normal body (~parse_body~), check if a declaration
has the identifier ~namespace~ followed by ~:~. Everything after that
point is treated as namespace members (same parsing as current namespace
block body -- declarations and assignments only, all must have values).

Store namespace members in a new field on ~SStructDef~:
~namespace_members: Vec<Declaration>~ and
~namespace_default_values: HashMap<String, Expr>~.

Alternatively, reuse ~SNamespaceDef~ as an embedded field in ~SStructDef~.

Files: src/rs/parser.rs, src/self/parser.til

**** Step 2: Parser -- recognize ~namespace:~ inside enum bodies

Modify the enum parser similarly. When parsing enum variants, if
~namespace~ + ~:~ is encountered after a comma, stop parsing variants
and switch to parsing declarations (same as namespace block body).

Store in a new field on ~SEnumDef~ or embed ~SNamespaceDef~.

Files: src/rs/parser.rs, src/self/parser.til

**** Step 3: Init -- merge inline namespace members

Currently init handles ~NamespaceDef~ as a top-level expression and merges
its members into the existing struct. With inline ~namespace:~, the members
are already part of the struct/enum definition.

Two approaches:
- *Option A*: Init detects the inline namespace fields on SStructDef/SEnumDef
  and merges them into the type (similar to current logic).
- *Option B*: Parser emits the same AST as today -- a StructDef followed by
  a synthetic NamespaceDef. Init code stays the same. This is simpler but
  means the parser needs to split one source declaration into two AST nodes.

Recommendation: Option A is cleaner long-term (one type = one AST node).

Files: src/rs/init.rs, src/self/init.til

**** Step 4: Typer -- handle inline namespace members

The typer already has ~check_namespace_def~ which type-checks namespace
function bodies. With inline members, either:
- Extend ~check_struct_def~ to also type-check namespace members.
- Or keep the existing ~check_namespace_def~ and call it from within
  ~check_struct_def~ when namespace members are present.

Also remove the error "Function 'X' is defined inside struct. Functions must
be in namespace blocks" (typer.rs:2134-2138) -- functions after ~namespace:~
are now valid.

Files: src/rs/typer.rs, src/self/typer.til

**** Step 5: Ccodegen -- emit inline namespace functions

Currently ~emit_namespace_func_prototypes~ and ~emit_namespace_func_bodies~
iterate top-level expressions looking for ~NamespaceDef~ nodes. With inline
members, these functions need to find namespace members inside StructDef
and EnumDef nodes instead (or in addition, during transition).

Files: src/rs/ccodegen.rs, src/self/ccodegen.til

**** Step 6: Migrate all existing namespace blocks

Update all ~.til~ files to use the new syntax:
- src/core/*.til (~10 files, 1 namespace each)
- src/self/*.til (~6 files, ~17 namespaces total)
- src/test/*.til (~9 files, various)
- src/test/cross_file_forward/*.til (2 files)

Each migration: move the namespace block body inside the corresponding
struct/enum definition, after a ~namespace:~ separator.

**** Step 7: Remove separate namespace block support

Once all files are migrated:
- Remove ~parse_namespace_definition~ from parser.rs/parser.til
- Remove ~Namespace~ token from lexer (or repurpose as the ~namespace:~ keyword)
- Remove ~SNamespaceDef~ as a top-level node type (keep if embedded in struct)
- Remove ~is_namespace_definition~ and related helpers from ccodegen
- Remove the NamespaceDef handler from init, typer, interpreter, ccodegen
- Update error messages referencing "namespace blocks"

**** Step 8: Tests

- Update src/test/108.til and src/test/namespaces.til to use new syntax
- Add tests for: struct with namespace, enum with namespace, standalone
  namespace (struct with only namespace section), empty namespace section
- Negative tests: namespace in wrong position, duplicate members, etc.
- Run full benchmark to verify no regressions
...
- #108 was not wasted work -- it enabled enum methods, and the init merging
  logic carries over directly. This issue just changes the syntax surface.
