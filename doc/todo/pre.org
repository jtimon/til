...
1. *Step 1* (DONE): Add heap allocation for malloc/free alongside the arena
   ...
   - heap_alloc adds 64-byte padding to tolerate arena-era off-by-one writes
...
2. *Step 2*: Migrate more types to heap allocation
   - Gradually move struct allocations from arena to heap
   - Arena remains for things that don't need individual deallocation
