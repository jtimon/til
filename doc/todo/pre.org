...
** Issue #179: Typer should reject ~cast()~ in non-declaration positions
:PROPERTIES:
:DISCOVERED: 2026-02-23
:IMPACT: Better error messages -- currently fails silently in interpreter or at gcc level
:STATUS: Open
:RELATED:
:END:
...
~cast(Type, expr)~ only works as a declaration directive:
...
The typer should detect and reject ~cast()~ when used as:
1. A function argument
2. The receiver of a UFCS method call
...
// Instead of: x := cast(I64, v.get(0)?).add(1)
e0 := cast(I64, v.get(0)?)
x := e0.add(1)
#+END_SRC
