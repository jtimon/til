...
- [ ] Improve test error handling with ERROR: prefix (IN PROGRESS - dynamic_arrays.til, mut_test.til, pointers.til, enums.til, arrays.til, args.til, ufcs.til, editor_mode_test.til, and test_lexer.til completed; errors.til requires no changes (all catches test expected errors); larger test files remain: test_parser.til, maps.til, lists.til)
