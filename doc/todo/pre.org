...
3. PARTIAL — Fix Bug #144 — desugarer rewrites ~for item: T in vec~ to use
   ~get_by_ref~ + ~create_alias~ + ~dont_delete~ for non-primitive types.
   Works but still constructs wasteful default. Next: redesign create_alias
   as a directive (declares + aliases + skips auto-delete in one step):
#+begin_src til
for _i in 0..vec.len() {
