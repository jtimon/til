...
1. *Step 1*: Add a global heap allocator alongside the arena
   - New module (eval_heap.rs / eval_heap.til) that wraps malloc/free
   - Same interface as arena (alloc returns an offset, read/write by offset)
   - Arena stays, heap is an alternative allocation target
   - Move one simple struct type onto the heap to prove it works
   - Run tests, verify delete() actually frees memory
