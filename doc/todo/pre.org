...
- [X] Standardize len/cap API across collections (COMPLETE)
  ...
  - Fixes Bug #25: Method resolution now works correctly (e.g., `s.items.len().eq(0)`)
  - [ ] TODO: Add len() and size() methods to Map struct
  - [ ] TODO: Add len() and size() methods to Str type
