...
:STATUS: In progress -- Commits 1-2 done, Commit 3 (placement construction) remaining
:RELATED: Bug #159 (shallow copies), Bug #165 (memory leak), Bug #80 (ccodegen divergence), Issue #117 (ASAP destruction)
:PLAN: .claude/plans/foamy-swimming-kernighan.md
:END:

*** Description
In the interpreter, function returns are shared offsets -- no copy happens.
The callee writes data to the heap, returns the offset, and the caller binds
that same offset to a new variable name. Two names, same data, zero copies.

In ccodegen, every struct return creates shallow memcpy copies. The callee's
local is abandoned with aliased heap pointers, never freed. This is the
fundamental semantic gap and the reason compiled TIL leaks far more than
interpreted.
...
*** IMPORTANT: This issue can't be closed without a noticeable improvement in performance
Commits 1 and 2 are infrastructure only. They eliminate unnecessary copies at
call sites but the callee still does =*_ret = local;= which is a shallow copy
that aliases heap pointers. The memory leak gap (Bug #165) won't improve until
Commit 3 (placement construction) eliminates that last copy. If performance
doesn't measurably improve after Commit 3, the issue stays open.

*** Done: Commit 1 -- Throwing calls eliminate intermediate =_ret_N= temp

Throwing functions already used out-pointer. Eliminated the unnecessary temp
at call sites:
...
- =build_dest_ptr_expr= helper computes &dest for all LHS patterns
- =emit_throwing_call_propagate= and =emit_throwing_call_with_goto= skip
  temp declaration, pass dest ptr directly, skip final copy
- Ported to ccodegen.til
- 98/98 tests pass

*** Done: Commit 2 -- Non-throwing struct returns switch to out-pointer

Changed non-throwing struct-returning functions from return-by-value to
out-pointer, matching throwing functions:
...
void til_Vec_new(til_Vec* _ret, til_Type T) {
    til_Vec vec = {...};
    *_ret = vec;           // one copy (STILL A SHALLOW COPY)
    return;
}
til_Vec dest;
...
Changes: =needs_out_pointer= helper, =emit_func_signature= (void + =_ret=),
=emit_return= (=*_ret = value; return;=), implicit return fallbacks,
=emit_fcall_arg_string= (hoist nested calls), =emit_variadic_call=,
=emit_variadic_arg_string=, =emit_fcall= (statement-level), =emit_declaration=
(UFCS chains), ext.c (=til_i64_to_str=, =til_fs_parent_dir=). All ported to
ccodegen.til. 98/98 tests pass.

This commit is infrastructure only -- call sites are clean but callees still
create a local and copy to =*_ret=.

*** TODO: Commit 3 -- Placement construction (the actual fix)

**** The root cause of the leak

The garbager inserts =Type.delete(result)= at function end. But =return result=
comes before it, making the delete unreachable. The local's heap pointers are
aliased with =*_ret= and never freed:

#+begin_src c
// After Commit 2 -- the local is the leak:
void til_concat(til_Str* _ret, ...) {
    til_Str til_Str_result = {...};   // local with heap pointers
    til_Str_result._len = ...;
    *_ret = til_Str_result;           // shallow copy aliases heap ptrs
    return;
    til_Str_delete(&til_Str_result);  // UNREACHABLE = LEAKED
}
#+end_src

**** The fix: eliminate the local entirely

#+begin_src c
// Goal -- zero copies, like the interpreter:
void til_concat(til_Str* _ret, ...) {
    *_ret = (til_Str){...};           // construct into caller's memory
    _ret->_len = ...;                 // write into caller's memory
    return;                           // no local, no leak, no delete needed
}
#+end_src

**** Approach: return variable aliasing

Pre-scan each function body to find the "return variable" -- a local that ALL
return paths return. Alias it to =_ret= throughout:

| Context        | Before (local)            | After (alias to _ret)       |
|----------------+---------------------------+-----------------------------|
| Declaration    | =Type var = init;=        | =*_ret = init;= (no local) |
| Field access   | =var.field=               | =_ret->field=               |
| Value access   | =var=                     | =(*_ret)=                   |
| By-ref arg     | =&var=                    | =_ret= (already ptr)        |
| Return         | =*_ret = var; return;=    | =return;= (data there)      |
| Delete         | =Type_delete(&var);=      | skip (no local to delete)   |

Detection: =detect_return_variable(body, func_def)= scans all Return nodes.
If ALL returns return the same Identifier(X), and X is a local declaration
(not a parameter) with =needs_out_pointer= type, and the body ends with a
Return (no implicit return path), then X is the return variable.

When not detected (multiple return variables, returns expressions, implicit
return path), keep current behavior -- no aliasing, =*_ret = local= copy.

**** Implementation steps

1. Add =detect_return_variable= function (pre-scan all Return nodes)
2. Add =current_return_alias= field to CodegenContext
3. Wire up in =emit_func_declaration= / =emit_funcdef= / nested functions
4. Modify =emit_expr= Identifier handler (=_ret->field=, =(*_ret)=)
5. Modify =emit_declaration= (=*_ret = init= instead of =Type var = init=)
6. Modify =emit_assignment= (=_ret->field = expr=, =*_ret = expr=)
7. Modify =emit_return= (just =return;= for the alias variable)
8. Modify =emit_arg_string= (=_ret= for by-ref, =(*_ret)= for by-value)
9. Modify =build_dest_ptr_expr= (return =_ret= for the alias)
10. Skip =Type.delete(var)= when var is the alias
11. Port all to ccodegen.til

Full plan: =.claude/plans/foamy-swimming-kernighan.md=
