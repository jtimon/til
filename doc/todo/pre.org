...
:STATUS: Steps 1-3 done, EvalArena cleanup remaining
...
1. *Step 1* (DONE): Added heap_alloc/heap_free alongside arena. Dual dispatch.
2. *Step 2* (DONE): Moved struct allocations from arena to heap.
3. *Step 3* (DONE): Removed arena from EvalArena. All allocations via heap.
   Deleted arena.rs. Kept arena.til as standalone std library.

*** Remaining: Remove or rename EvalArena

eval_arena.rs and eval_arena.til still exist as the memory management layer.
The "arena" name is now misleading — it's purely a heap allocator wrapper.

Options:
- Rename EvalArena to EvalHeap or EvalMem (and g_arena to g_heap/g_mem)
- Inline heap_alloc/heap_free into ext functions and remove EvalArena entirely
- Move remaining eval logic (insert_i64, insert_string, insert_struct, etc.)
  closer to the interpreter, since EvalArena is really an interpreter helper

Also: arena_index (HashMap in ScopeFrame) still uses the old name. All the
"arena_offset" fields in EvalResult, and comments throughout the codebase
still reference "arena". A rename pass would clean this up.
