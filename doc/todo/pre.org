...
** Issue #204: First-class values not fully usable after retrieval
:PROPERTIES:
:DISCOVERED: 2026-02-28
:IMPACT: Language feature gaps -- stored first-class values can't be called/instantiated
:RELATED: Issue #91 (First-Class Functions), Issue #105 (First-Class Structs), Issue #106 (First-Class Enums), doc/first_class.org
:END:

Issues #91, #105, and #106 allow storing FuncDef, StructDef, and EnumDef
values in Vecs and struct fields. But the stored values cannot be used
(called/instantiated) after retrieval. The typer rejects them.

*** Missing from #91 (First-Class Functions)
- [ ] FuncDef variables are not callable: ~Cannot call 'fn0', it is not a function, it is 'FuncDef'~
- [ ] FuncDef not assignable from named functions: ~Return value expected to be 'FuncDef', but found 'func'~
- [ ] FuncDef struct fields reject function defaults: ~field 'callback' declared as 'FuncDef' but default value has type 'func'~

*** Missing from #105 (First-Class Structs)
- [ ] StructDef variables are not callable as constructors: ~Cannot call 'def0', it is not a function, it is 'StructDef'~
- [ ] Struct lookup fails for StructDef variable names: ~Struct 'def0' not found~

*** Missing from #106 (First-Class Enums)
- [ ] EnumDef variables don't have variant access: ~Struct 'EnumDef' has no member 'North'~

*** Additional gaps
- [ ] ~struct_def_of~ and ~enum_def_of~ are procs, so they can't be called from funcs (limits return-from-function tests)

*** Test coverage
Commented-out tests added to ~src/test/first_class.til~ documenting each gap.
...
- [-] Issue #91: First-Class Functions (func ptrs, Vec of func ptrs, TFuncDef done; Vec of FuncSig/FuncDef + closures remaining)
...
*** Rejected Approach: branch ~claude/fix-issue-135-SMlEh~

Attempted separate ext.o compilation with hardcoded extern declarations:

1. Removed ~static inline~ from all ext.c functions, added standalone type
   definitions (til_Str, til_Ptr, til_Array, etc.) so ext.c compiles as its
   own translation unit.

2. Changed ~til_size_of~ from ~static inline~ to ~extern~ (generated per-program,
   resolved at link time from the generated C).

3. Replaced ~#include <ext.c>~ in ccodegen with 50+ hardcoded ~extern~
   declarations for every ext.c function.

4. Wrapped all stdlib calls in generated C: ~strlen~ -> ~til_raw_strlen~,
   ~strcmp~ -> ~til_raw_strcmp~, ~fprintf(stderr,...)+exit~ ->
   ~til_size_of_error~/~til_introspect_error~/~til_introspect_index_error~.

5. Moved ~#define true/false~ from ext.c into generated C (macros can't cross
   translation units).

6. Updated builder.rs/til to compile ext.c to ext.o separately, check for
   precompiled ~lib/ext-{target}.o~, then link with generated C.

*Why rejected:* 50+ manually-synced extern declarations in both ccodegen.rs
AND ccodegen.til is extremely brittle -- adding any function to ext.c
requires updating two codegen files. ext.c also duplicates type definitions
that must stay perfectly in sync with what ccodegen generates. The branch
also mixed in unrelated GNU time detection changes (~g_has_gnu_time~).
