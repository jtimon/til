...
Switch desugaring is now implemented in ~src/rs/desugarer.rs~. Simple enum switches
are desugared to if/else chains using ~enum_to_str()~ for comparison and
~enum_get_payload()~ for payload extraction.

Nested enum patterns (e.g., ~case ValueType.TType(TTypeDef.TEnumDef)~) are detected
and NOT desugared - these are left for ccodegen to handle, since checking the inner
pattern after payload extraction is complex and would require additional scope handling.
