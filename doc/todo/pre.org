...
**** Step 4: Auto-insert Type.delete() calls in garbager

WIP on branch ~latest117~. Double-free on container get() due to shallow copy
aliasing. Needs prerequisite: change container get() to return borrowed Ptr.

**Prerequisites** (incremental commits, each passing ~make benchmark~):

**4a. [X] Add Str to garbager clone set** - e4788b62
Remove "Str" from primitives exclusion in garbager clone logic.

**4b. [X] Eliminate Str double-copy via arena_offset** - 31a10477
EvalResult carries arena_offset (0 = None) through eval/return/call chains.

**4c. Container ~own~ params + zeroing**
~push~, ~set~, ~insert~, ~insert_at~ take ~own value: Dynamic~.
After copy, ~memset(to_ptr(value), zero, size)~ zeroes the caller's variable
so the caller's auto-delete becomes a no-op (all pointers zeroed).

Files: vec.til, array.til, set.til, list.til, map.til, hashmap.til
Details (from latest117):
- Vec: push, set, insert_at, extend_with (inlined - can't call push with own)
- Array: set
- Set: insert
- List: push, set (both cases: same-size and different-size)
- Map: insert, set (zero value on overwrite too)
- HashMap: insert, set (compute bucket BEFORE push moves key)
- list.til fix: ~memset(to_ptr(value), 0, size)~ -> ~zero : U8 = 0; memset(...)~

**4d. Clone fixes across TIL files**
~own~ params consume variables. Call sites that use a variable after passing it
to an ~own~ param need explicit ~.clone()~.

Files and fixes (from latest117):
- builder.til: ~insert(core_path.clone())~ x4
- eval_arena.til: ~push(frame.clone())~ x2, ~set(temp_id.clone(), ...)~ x2,
  ~insert_string_into_frame(ctx, frame, temp_id.clone(), ...)~ x2
- ccodegen.til: ~set(decl.name.clone(), value_type.clone())~
- interpreter.til: ~set(var_name.clone(), offset)~
- parser.til: ~push(e.clone())~, ~push(result.clone())~ x2
- typer.til: ~insert(path.clone())~, ~push(return_val_e.clone())~,
  ~push(switch_case_expr.clone())~

**4e. Rust ccodegen: remove ~const~ for struct/enum locals**
...
File: ccodegen.rs (7 locations, from latest117):
...
**4f. Port ccodegen const removal to TIL**
Mirror the 7 Rust changes in src/self/ccodegen.til.

**4g. Rust typer: own-consumption analysis**
Control-flow-aware tracking of variables consumed by ~own~ params.

File: typer.rs (from latest117):
- Add ~snapshot_symbols~ / ~restore_removed_symbols~ helpers
- ~check_if_statement~: snapshot before branches, check each independently,
  restore between branches. After all branches, remove symbols consumed in
  ALL branches (intersection). If-without-else: no definite consumption.
- ~check_switch_statement~: snapshot before cases, restore before each case body
...
- Own consumption: skip primitives (I64, U8, Type, Dynamic, Ptr)
- Mut check: only ~mut~ params require mut variables, NOT ~own~ params

**4h. Port typer own-consumption to TIL**
Mirror the Rust typer changes in src/self/typer.til.

**4i. Garbager: refactor clone logic to use ~is_deletable_type~**
Extract inline ~is_primitive~ checks into shared ~is_deletable_type~ helper.

File: garbager.rs (from latest117):
- Add ~is_deletable_type(value_type, context)~ helper
...
**4j. Change container ~get~ to return borrowed Ptr**
...
Solution: ~get~ returns a borrowed Ptr to the element in place. No copy, no
aliasing, no ownership confusion. Caller reads through Ptr. To get an owned
copy, caller explicitly clones.

Files: vec.til, array.til, map.til, hashmap.til, list.til + ALL call sites.
This is a large refactor touching most of the codebase.

**4k. Garbager: delete insertion**
Core implementation, after all prerequisites are done.
...
- ~is_deletable_type(value_type, context)~ (already done in 4i)
...
**4l. Garbager: synthetic catch-and-rethrow for error propagation cleanup**
~?~ propagation bypasses explicit ~return~/~throw~ where deletes are inserted.
After ~insert_deletes_in_stmts~, for each throw type, append a synthetic catch
that deletes all candidates and rethrows.

**4m. Port garbager to TIL**
Mirror all Rust garbager changes in src/self/garbager.til.

**4n. Port ~get~ changes to TIL garbager/ccodegen/typer if needed**
...
- ~push~/~set~/~insert~ use ~own~ params: ownership transfers IN, source zeroed
