...
- [ ] Implement pass-by-reference semantics with three argument modes
  ...
  - [ ] Bug #26: Vec/Map shallow copy issue (UNRESOLVED)
    - Status: NOT fixed by pass-by-reference (different root cause)
    - Issue: Vec.push/get use memcpy for shallow copy of struct bytes
    - Effect: Storing Expr in Vec copies ptr field, but not heap data it points to
      - Later retrieving Expr gives corrupted nested Vec (dangling pointers)
    - Root cause: memcpy copies struct bytes including pointers, not pointed-to data
    - Pass-by-ref helps: Reduces parameter copying, but doesn't fix Vec storage
    - Workaround: test_parser.til struct test disabled (line 423)
    - Real solution: Deep copy support, reference counting, or arena-based allocation
