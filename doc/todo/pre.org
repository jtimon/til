...
**** Step 8: ~dont_delete~ directive (replaces zeroing)
...
push := proc(mut self: Vec, own value: Dynamic) {
    // ... copy bytes into vec ...
    dont_delete(value)  // garbager skips delete for value
}
#+end_src
Files: garbager.rs, garbager.til + container files (vec, array, map, etc.)
...
Agreed approach:
1. Add ~get_by_ref~ to containers — returns borrowed Ptr (~offset~ already
   sets ~is_borrowed=1~). Same signature pattern as ~get~ but returns Ptr
   instead of copying bytes.
2. Add ~Ptr.create_alias~ ext_func — zero-copy alias, follows U8 ext_func
   wrapper pattern. Sets up local variable to point at same memory as the
   Ptr without copying.
...
for _i in 0..vec.len() {
    ...
    mut item := MyStruct()
    _ref.create_alias(MyStruct, item)  // zero-copy alias
