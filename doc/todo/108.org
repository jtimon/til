...
**** Step 1.2: Init - merge namespace into existing type
...
1. Look up the existing StructDef/EnumDef for =Foo=
2. Merge all members from the namespace into that type's =default_values=
3. Remove the NamespaceDef from the AST (it's been absorbed)
