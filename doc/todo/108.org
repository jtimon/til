...
**** Step 1.3: Port to TIL [DONE]

Ported all changes to TIL:
- typer.til, ufcs.til, precomp.til, interpreter.til: Skip NamespaceDef (already processed by init)
- ccodegen.til: Added is_namespace_definition, emit_namespace_func_prototypes, emit_namespace_func_bodies
- init.til: Check registered functions before struct members in get_fcall_value_type
...
Current status: Both UFCS calls (=f.get_x()=) and qualified calls (=Foo.get_x(f)=) now work.
The fix was to check registered functions before struct members in get_fcall_value_type.
...
***** Typer fix for collection-like structs (DONE)

The typer checks that collection-like structs (those with get/set/push/insert
methods) also have len() and size() methods. This check was using the original
struct_def from the AST, which doesn't include namespace methods.

Fix: Modified check_struct_def in typer.rs and typer.til to use
context.scope_stack.lookup_struct() to get the merged struct (which includes
namespace members merged by init). Falls back to original struct_def if not found.

This enables collection-like structs (Array, Vec, etc.) to have their methods
in namespace blocks without failing the collection validation check.

***** Precomp fix for namespace methods (DONE)

precomp now calls update_func() after transforming namespace functions so the
interpreter uses the precomp-transformed version (with loc() resolved, etc.)
rather than the original untransformed version from init.

***** Scavenger fix for variadic support (DONE)

When namespace methods are discovered after the main reachability loop, they're
processed in a second loop. This second loop wasn't checking for variadic
parameters, so when format() was discovered via namespace methods,
needs_variadic_support stayed false. This caused Array.set and Array.delete
prototypes to not be emitted.

Fix: Added variadic parameter check to the second while loop so that functions
discovered via namespace blocks properly trigger variadic support.

***** Status: Ready for Step 2.2

All blocking issues have been fixed. Namespace blocks now work correctly for
struct methods that:
- Call external functions (malloc, loc, memset, etc.)
- Use variadic functions (format, panic)
- Are called by the runtime (panic calling Array methods)

**** Step 2.2: Migrate existing struct AFs to namespace blocks [DONE]
...
All struct AFs migrated to namespace blocks across the codebase.

**** Step 2.3: Disallow inline AFs in structs [DONE]
...
Added check in typer (both Rust and TIL) to reject FuncDef inside struct
definitions. Error message guides users to move functions to namespace blocks.

**** Step 2.4: Port to TIL [DONE]

Port all changes from Rust to the self-hosted TIL implementation.

Ported alongside Rust changes - both typer.rs and typer.til updated together.
