...
*** STEP 2: Migrate Context::symbols → ScopeStack::local_symbols

**Status**: TODO - Blocked by Step 1

**Goal**: Move all symbol lookups/insertions from Context.symbols to scope_stack.local_symbols

**Changes**:
1. During init_context(), populate scope_stack.frames[0].local_symbols with all globals/imports
2. In function calls/catch/pattern-match, use scope_stack.declare_symbol() for new symbols
3. Update all context.symbols.get() → context.scope_stack.lookup_symbol()
4. Update all context.symbols.insert() → context.scope_stack.declare_symbol()
5. Remove symbols: HashMap<String, SymbolInfo> from Context struct
...
*** STEP 3: Migrate Context::funcs → ScopeStack::local_funcs

**Status**: TODO - Blocked by Step 2

**Goal**: Move all function lookups from Context.funcs to scope_stack.local_funcs

**Changes**:
1. During init_context(), populate scope_stack.frames[0].local_funcs with all functions
2. Update all context.funcs.get() → context.scope_stack.lookup_func()
3. Update all context.funcs.contains_key() → context.scope_stack.lookup_func().is_some()
4. Remove funcs: HashMap<String, SFuncDef> from Context struct

**Bonus**: Enables local function definitions in future!

**Files affected**: init.rs, interpreter.rs, typer.rs (~100 lookup sites)
...
*** STEP 4: Migrate Context::enum_defs → ScopeStack::local_enums

**Status**: TODO - Blocked by Step 3

**Goal**: Move all enum lookups from Context.enum_defs to scope_stack.local_enums

**Changes**:
1. During init_context(), populate scope_stack.frames[0].local_enums with all enums
2. Update all context.enum_defs.get() → context.scope_stack.lookup_enum()
3. Update all context.enum_defs.contains_key() → context.scope_stack.lookup_enum().is_some()
4. Remove enum_defs: HashMap<String, SEnumDef> from Context struct

**Bonus**: Enables local enum definitions in future!

**Files affected**: init.rs, interpreter.rs, typer.rs (~80 lookup sites)
