...
Semantics: function-level, LIFO order. Defers inside conditional blocks only run
if that block was entered (implemented via bool flags). Deferred code runs before
the return/throw expression is evaluated, so ~defer delete(x); return x~ deletes
x before returning.

Implementation: each ~defer <expr>~ becomes a bool flag (~_defer_N~) set to true
at the defer site. Before every return/throw and at end of body, if-guarded
cleanup blocks run in reverse order. Lexer, parser, AST, desugarer changes in
both Rust and TIL. Typer handles Defer as valid (runs before desugarer in pipeline).
...
:STATUS: Implemented (Step 4b deferred to Issue #178)
...
- [X] ~struct_def_of(type_name)~ runtime introspection via ext functions
- [X] Anonymous struct expressions: ~struct { ... }(args)~
...
Allow macros to return enum definitions via ~returns enum~. Enables
template-like patterns where macros generate typed enums at compile time.
...
- [X] ~enum_def_of(type_name)~ runtime introspection via ext functions
- [X] Anonymous enum expressions: ~some_macro(enum { A, B })~
- [X] Explicit metatype annotations (~Color : EnumDef = enum { ... }~)
- [X] All changes ported to both Rust and TIL, all 94 tests pass
