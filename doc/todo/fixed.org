...
*** Implementation
- ~src/rs/desugarer.rs~ - added ~desugar_switch()~ and helper functions (~1224 lines total)
- ~src/self/desugarer.til~ - TIL port of the same logic (~1988 lines total)
...
Nested enum patterns handled with match flag pattern (flat if blocks).
...
|------|--------|-------|----------|
| rust interpreted | 169.7s | 221.6s | +31% |
| rust compiled | 4.4s | 4.6s | +5% |
| til interpreted | 380.7s | 561.7s | +48% |
| til compiled | 15.4s | 27.0s | +75% |
| Total | 570.3s | 814.9s | +43% |
