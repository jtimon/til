...
*** Implementation
- ~src/rs/desugarer.rs~ - added ~desugar_switch()~ and helper functions (~1300 lines)
- ~src/self/desugarer.til~ - TIL port of the same logic (~1600 lines)
...
| rust compiled | 4.4s | 4.5s | +1% |
| til interpreted | 380.7s | 559.5s | +47% |
| til compiled | 15.4s | 26.3s | +71% |
| Total | 570.3s | 811.9s | +42% |
