...
*** Fix
Added struct field type checking in the type checker (check_fcall function).
For struct instantiations, when a named argument is provided, the type checker now:
1. Looks up the struct field's expected type from the struct definition
2. Gets the actual type of the provided value using get_value_type
3. Compares the types and reports an error if they don't match

Error format: "Struct 'X' field 'Y' expects 'ExpectedType', but 'ActualType' was provided."

Also supports U8/I64 coercion for literals (Bug #124) and Dynamic/Type fields.

*** Files Changed
- src/rs/typer.rs: Added field type checking in struct instantiation block
- src/self/typer.til: Same change for self-hosted type checker
