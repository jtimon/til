...
- Declarations (=mut x := y=): calls =Type.clone(y)= instead of =copy_fields=
- Reassignments (=x = y=): calls clone via =try_call_clone=
- Field assignments (=obj.field = y=): calls clone
- =copy= param passing: calls clone (regular/mut params are borrowed, don't clone)
...
- Skip primitives (I64, U8, Str, Bool, Ptr, Type) - they're C value types
