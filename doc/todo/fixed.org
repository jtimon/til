...
where ~params[0]~ is a throwing FCall. In such cases, hoist the FCall using
~emit_arg_string~ and append the field chain to the returned temp var.
...
| Mode | Before | After (initial) | After (optimized) |
|------|--------|-----------------|-------------------|
| rust interpreted | 169.7s | 221.6s (+31%) | 217.5s (+28%) |
| rust compiled | 4.4s | 4.6s (+5%) | 4.3s (-2%) |
| til interpreted | 380.7s | 561.7s (+48%) | 550.1s (+44%) |
| til compiled | 15.4s | 27.0s (+75%) | 25.6s (+66%) |
| Total | 570.3s | 814.9s (+43%) | 797.6s (+40%) |

After removing the match flag pattern and using combined conditions, performance improved
slightly across all modes. Trade-off: simpler code in interpreter/ccodegen/precomp at cost
of runtime performance. Future optimization: could add fast-path for enum switches avoiding
string comparison.
...
|----------|-------|-------|---------|-----|
| Rust     | 6     | +1055 | -1280   | -225 |
| TIL      | 6     | +1702 | -1380   | +322 |
| C        | 1     | +42988 | -36929 | +6059 |

Note: C changes are in bootstrap/til.c (generated from TIL self-hosted compiler).
After helper function refactoring, Rust now has net negative LOC (more code removed than added).
