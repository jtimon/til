...
** MOVED Variable Shadowing Bug → See pre.org

This item was moved to doc/todo/pre.org since it must be fixed BEFORE self-hosting.

The variable shadowing bug causes nested-scope declarations to mutate outer variables instead of creating shadows. This is a correctness bug that could cause silent data corruption in the self-hosted compiler.

See doc/todo/pre.org for full details and implementation plan.
