...
*** Problem
After implementing pass-by-reference semantics (see pre.org), TIL will have three argument modes:
- Default (immutable): Pass by reference, read-only
- mut: Pass by reference, mutable
- copy: Explicit copy, can modify without affecting caller
