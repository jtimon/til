...
3/68 tests failed:
- bug41_test.til (interpret output) - double "DEBUG:hello from bug41 test" output
- bug41_test.til (run output) - same double output issue
- introspections.til (run exit code) - linker error, pre-existing unrelated issue

** Next Steps

1. Investigate bug41_test.til double output (precomp evaluating something twice?)
2. Port precomp.rs changes to precomp.til
3. Restore Makefile (~tests: rstil til~)
4. Update bug docs, commit
