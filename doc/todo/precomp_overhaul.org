...
*** Difference H: Template creation [BUG - CURRENT BLOCKER]
...
- *PRECOMP*: Does NOT create templates, so ~insert_struct_instance~ fails

This causes failures when:
1. ForIn desugaring creates ~Str()~ constructor call
2. precomp_fcall sees struct constructor, calls ~insert_struct_instance~
3. Fails because Str template doesn't exist yet

*Fix needed:* Precomp should also create templates when processing struct definitions.
Look at where ~create_default_instance~ is called in interpreter.rs and replicate in precomp.
