...
Most tests pass. Remaining failures:

*** bug41_test.til - Double output
- "DEBUG:hello from bug41 test" appears twice
- Needs investigation - may be duplicate precomp processing

*** introspections.til - Linker errors (pre-existing)
- ~undefined reference to 'til_Introspection_has_field'~
- This is a compilation/linking issue, not related to precomp changes

*** test_parser.til and til.til - "Lexer.advance does not return anything"
- Error at parser.til:482
- Precomp is trying to fold something that includes a proc call
- ~is_comptime_evaluable~ needs to check that functions actually return values
- A proc (no return) should not be considered comptime-evaluable

** Next Steps

1. Fix ~is_comptime_evaluable~ to reject proc calls (functions without return type)
2. Investigate bug41_test.til double output
3. Run full test suite
4. Restore Makefile (~tests: rstil til~)
5. Port changes to TIL (~src/self/precomp.til~)
6. Update bug docs, commit
