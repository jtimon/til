...
1. *~src/rs/precomp.rs~*:
   - Modified ~is_comptime_evaluable~: Instead of blocking ALL throwing functions, now only blocks those throwing ~AllocError~, ~OutOfBounds~, or ~KeyNotFoundError~ (runtime-dependent errors)
