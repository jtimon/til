...
:STATUS: Nearly complete -- Vec of FuncDef remaining.
...
**** is_function_signature shared helper (DONE - bb45c784, 2026-02-27)
Extracted duplicated FuncSig detection pattern from precomp, typer, init,
interpreter (6 Rust + 5 TIL occurrences) into shared utils.rs/utils.til.

**** func_sig_of introspection (DONE - 8b63f79a, 2026-02-28)
6 ext functions for FuncSig introspection (__func_sig_param_count/type,
return_count/type, throw_count/type), high-level ~func_sig_of~ wrapper in
meta.til, C codegen emission, scavenger recognition. Parallel to
struct_def_of/enum_def_of.

**** Vec of FuncSig (DONE - 8b63f79a, 2026-02-28)
Store different function signature types in the same Vec via func_sig_of.

*** What is missing
- Vec of FuncDef: works in interpreted mode, fails in compiled mode.
  FuncDef clone triggers NodeType clone which has a Map(MapDef) variant,
  requiring ~Map_new~ to be declared before ~node_type_clone~ in generated C.
  Blocked by codegen function ordering (forward declaration issue).
