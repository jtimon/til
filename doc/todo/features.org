...
:STATUS: Partial (anonymous inline done)
...
- Anonymous enum registration with =AnonEnumN= temp names
...
**** Completed (2026-02-23) -- Anonymous enum as inline arguments

=expand_anon_enum_fcalls_recursive= in precomp walks the AST, finds bare
EnumDef nodes, registers them as =AnonEnumN= with auto-generated
delete/clone methods, and replaces with an Identifier. Pending declarations
are injected before the statement that uses them in =precomp_body=.

Tests: =count_variants(enum { Red, Green, Blue })= works in all 4 modes.
