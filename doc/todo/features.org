...
:STATUS: Nearly complete - introspection, collections, compiled mode all working (2026-02-26). Anonymous struct as Dynamic arg remaining.
...
- [ ] Anonymous struct in non-macro function args (~vec.push(own struct { ... })~)
      Works in interpreted mode. Compiled mode needs codegen support for emitting
      type definitions (StructDef) as runtime values.

** Issue #106: First-Class Enums
:PROPERTIES:
:DISCOVERED: 2026-01-12
:IMPACT: Language feature, anonymous data structures, templates
:STATUS: Nearly complete - introspection, collections, compiled mode all working (2026-02-26). Anonymous enum as Dynamic arg remaining.
...
- [ ] Anonymous enum in non-macro function args (~vec.push(own enum { ... })~)
      Works in interpreted mode. Compiled mode needs codegen support for emitting
      type definitions (EnumDef) as runtime values.
