...
:STATUS: Partially implemented - macro support done, collections and compiled introspection remaining
...
*** What is missing
- [ ] ~struct_def_of~ ext_funcs not linked in compiled mode (only works interpreted)
- [ ] Anonymous struct in non-macro function args (~vec.push(own struct { ... })~)
- [ ] Vec of StructDef (~Vec.new(StructDef)~) -- depends on compiled struct_def_of or direct StructDef construction
...
*** What is missing
- [ ] ~enum_def_of~ ext_funcs not linked in compiled mode (only works interpreted)
- [ ] Anonymous enum in non-macro function args (~vec.push(own enum { ... })~)
- [ ] Vec of EnumDef (~Vec.new(EnumDef)~) -- depends on compiled enum_def_of or direct EnumDef construction
...
:STATUS: In progress - FunctionSig + func ptrs + lambdas + return + struct fields + passthrough + throws + collection infrastructure done (2026-02-25), next: Vec/Map collection tests
...
*** What is missing
- Retrieve and call function pointers from Vec (~cast(BinaryOp, ops.get(0)?)~ then call)
