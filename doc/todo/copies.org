...
*** Category A: params.get(i, expr) -- 290 calls
...
These are Expr.params (a Vec of Expr). Could be converted to
get_by_ref if Vec supports it, but the alias semantics need care.

*** Category B: e.get(N)? shorthand -- ~200 calls
...
Same as params.get() but via a convenience method. Same fix applies.

*** Category C: self.frames.get(i, frame) -- ScopeStack lookups -- ~35 calls

All in src/self/init.til. These copy entire ScopeFrame structs
(which contain Maps of symbols, funcs, enums, structs, heap_index).

14 reverse loops (innermost-first for shadowing):
- lookup_var (line 163)
- lookup_symbol (line 184)
- lookup_type (line 212)
- lookup_func (line 352)
- lookup_enum (line 410)
- lookup_struct (line 444)
- has_var (line 466)
- has_symbol (line 480)
- has_func (line 494)
- has_enum (line 508)
- has_struct (line 522)
- lookup_method (line 580)
- lookup_or_infer (line 633)
- lookup_visible_symbols (line 658)

1 forward loop:
- dump_scope (line 391) -- debug only

~20 direct frame access (get last frame, etc.):
- current_frame (line 155)
- declare_* functions (lines 254, 275, 288, 315, 340, 374, 432)
- lookup_struct_for_member_access (lines 597, 758)

These are Bug #114. Cannot convert to for-in because they iterate
in reverse. Would need either:
1. get_by_ref for Vec (return pointer, no copy)
2. Reversed for-in iteration
3. Restructure to avoid frame iteration entirely
...
1. **params.get() / e.get()** -- 490 calls copying Expr
   Highest impact. Every compiler pass copies Expr nodes constantly.
   Fix: Add get_by_ref to Expr.params (Vec of Expr)
