* Copy Analysis - All .get() sites in the TIL codebase
  Date: 2026-02-14
  Note: ~get_by_ref~ was renamed to ~get~ (2026-02-21). References below updated.
...
across all 11 TIL self-hosted files. Uses get + create_alias
...
get + create_alias internally. All call sites have
...
to get + create_alias. Commits 40d2a579, 0cd9e119.

14 reverse loops: get + create_alias (zero-copy pointer into Vec storage).
10 mutating last-frame operations: get + create_alias, removed .set() copy-back.
1 forward loop (update_func): get + create_alias, removed .set() copy-back.
...
Converting to get + create_alias should eliminate all of that, but is
...
2. Change callers to use get + create_alias directly on
...
internal Map.get to get + create_alias and returning the alias still
...
2. Inline: callers use frame.symbols.get directly instead of calling
...
- Init phase (init.rs) can't resolve create_alias variables in declaration RHS.
  val_ref := frame.symbols.get(name)? fails with "Undefined symbol 'frame'"
  ...
  assign val_ref = frame.symbols.get(name)? as a statement (not declaration).
  Statements don't go through get_value_type so the alias is found normally.

**** Struct/enum default value lookups -- MOSTLY DONE (2026-02-17)

- struct_def.default_values.get(name, expr) -- copies Expr
- ns_def.default_values.get(name, expr) -- copies Expr

These are used IN-PLACE (not returned), so get + create_alias genuinely
eliminates the copy. No ret_var_alias problem.

Converted 25/35 calls:
- garbager.til: 2 calls (StructDef, NamespaceDef in garbager_recursive)
- typer.til: 5 calls (check_namespace_def, is_expr_calling_procs, resolve_inferred_types x3)
- ccodegen.til: 5 calls (emit_struct_default_constants, emit_namespace_func_prototypes,
  emit_namespace_func_bodies, emit_expr x2)
- interpreter.til: 2 calls (eval_static_init, eval_struct_init)
- desugarer.til: 2 calls (StructDef, NamespaceDef)
- precomp.til: 2 calls (NamespaceDef, StructDef)
- ufcs.til: 2 calls (StructDef, NamespaceDef)
- preinit.til: 1 call (StructDef)
- init.til: 4 calls (3x member dot-access resolution, 1x register struct members)

Not converted (10 calls) -- call methods (.clone/.error/.is_ext) on aliased variable:
- scavenger.til: 5 calls -- all call default_expr.clone() or func_def.is_ext().
  The typer can't resolve methods on pointer-typed create_alias variables.
- typer.til: 1 call (check_struct_def) -- calls inner_e.error()
- ccodegen.til: 3 calls (emit_struct_def, emit_struct_init_expr x2) -- emit_arg_string
  or emit_expr with create_alias inside a mut-declared variable causes missing temp
  variable declarations in generated C (undeclared _tmp vars in gcc).
- init.til: 1 call (register namespace members) -- calls ns_func_def.clone()

**** Other map lookups -- MOSTLY DONE (2026-02-17)

Converted 1/4:
- context.imported_asts.get() -- typer.til: converted (read-only use)

Not converted (3 calls):
- context.imported_asts.get() -- interpreter.til: reassigns ast = desugar_expr(ast),
  can't reassign a create_alias pointer variable with a new value
- context.imported_asts.get() -- builder.til: catch block reassigns dep_resolved
  from a different source, type conflict (Expr value vs pointer)
- symbols.get() -- interpreter.til x2 (lines ~2075, ~2218): call .clone() on result

Not worth converting (cheap types):
...
and cannot be converted to get.
...
2 Vec.get() loops converted to for-in (genuine copy savings):
- interpreter.til: eval_body statements iteration (Vec.get -> for-in)
- parser.til: SEnumDef.clone variants iteration (Vec.get -> for-in)

*** NOT converted -- segfault in compiled TIL binary (2026-02-17)

3 parser.til Vec.get() loops caused SIGSEGV in all 88 til_interpreted tests:
- SEnumDef.get() variants iteration
- SEnumDef.contains_key() variants iteration
- SStructDef.get_member() members iteration

These also triggered cascading throws/catch signature changes
(removing IndexOutOfBoundsError/Str from throws, removing ? from callers,
removing dead catch blocks in eval_heap.til, init.til, ccodegen.til,
desugarer.til). The segfault occurred even with just these 3 conversions
+ the cascading changes -- no Category 2 changes needed to reproduce.

Root cause unknown. The for-in generates correct-looking C code (pointer
alias via get into Vec storage). Needs investigation -- possibly
related to how the compiled TIL binary uses these functions during
initialization/parsing of core types.

*** NOT attempted -- blocked by above segfault

12 Expr.get() loops (Category 2, cleanup only, no copy savings since
Expr.get() already uses get + create_alias internally):
- parser.til: 7 loops (clone, params_clone, parse_struct body,
  parse_namespace body, arg_list args, get_combined_name, transform_continue_with_step)
- typer.til: 1 loop (check body params)
- desugarer.til: 4 loops (3x rename_identifier_in_expr, 1x transformed_body)
...
Options:
1. Use get + create_alias (already available, returns Ptr, no copy)
...
1. *DONE* **params.get() / e.get()** -- 490 calls copying Expr
   Converted to zero-copy via get + create_alias (2026-02-14).

2. *MOSTLY DONE* **ScopeStack frame iteration** -- 35 calls copying ScopeFrame
   24/25 .get() + 10 .set() converted to get + create_alias (2026-02-16).
   1 remaining: last_frame blocked by Bug #168 ret_var_alias.

3. *MOSTLY DONE* **Map.get() for default_values** -- 25/35 calls converted (2026-02-17)
   Uses get + create_alias. 10 remaining call methods on aliased variable
   (typer can't resolve methods on pointer-typed create_alias variables).
