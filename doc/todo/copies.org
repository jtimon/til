...
alias via get_by_ref into Vec storage). Needs investigation -- possibly
...
Expr.get() already uses get_by_ref + create_alias internally):
...
3. *MOSTLY DONE* **Map.get() for default_values** -- 25/35 calls converted (2026-02-17)
   Uses get_by_ref + create_alias. 10 remaining call methods on aliased variable
