...
:STATUS: Blocked (requires Issue #116 for escape tracking)
:DEPENDS: Issue #116 (ownership/escape tracking), Bug #101 (unused warnings - FIXED)
...
*** BLOCKING: Ownership/Escape Tracking Required
Auto-delete requires knowing when a value "escapes" and should NOT be deleted:
- Returned from function: ~return v~ - caller now owns it
- Assigned to struct field: ~container.field = v~ - struct now owns it
- Stored in collection: ~vec.push(v)~ - collection now owns it
- Passed to function that stores it

Without escape detection, auto-delete causes use-after-free and double-free errors.
Issue #116 must be implemented first to provide ownership/borrow tracking.

*** Correct Architecture: Precomp Phase
Auto-delete should be implemented in precomp, NOT ccodegen:
- Insert explicit ~var.delete()~ calls into the AST
- Works for both interpreter AND compiler (they both use precomp output)
- Works during constant folding too
- Single implementation, not duplicated between interpreter and ccodegen
- AST explicitly shows the delete calls, making debugging easier

*** Current State (2026-01-16)
Prototype infrastructure exists in ccodegen.rs (disabled) but has fundamental issues:
- Only works for compiler, not interpreter
- Cannot detect when values escape (return, field assignment, etc.)
- Causes 63/80 test failures when enabled

**** Prototype Infrastructure (ccodegen.rs, disabled):
- VarLifetime struct for tracking deletable variables
- type_has_delete() helper to check if type has delete method
- track_deletable_var() to register variables for cleanup
- emit_auto_delete_cleanup() to emit delete calls at function end
- is_constructor_call() to detect ~Type.new()~ patterns
- Scavenger marks delete() methods reachable for all used types

**** Fixed Delete Functions (2026-01-16):
All delete() functions are now idempotent (safe to call multiple times):
- Array.delete: Added ptr check before free
- Map.delete: Use self.keys/values.delete() directly
- Ptr.delete: Added NULL check, sets data=0, takes mut self
- List.delete: Added data ptr check before free

*** Types with delete()
Already implemented: Vec, Array, Set, Ptr, Map, List
After Issue #116: Str

*** Design Decisions
- Method name: Use existing ~delete()~
- Double-free prevention: delete() must set ptr to 0 after free (idempotent)
- Destruction order: Reverse order of declaration (like C++/Rust)
- Implementation: In precomp (insert AST nodes), not ccodegen

*** Future Implementation Steps (after Issue #116)
1. Add ownership tracking to precomp (detect escapes)
2. In precomp, insert ~var.delete()~ calls at appropriate points
3. Handle control flow: returns, throws, branches, loops
4. Remove prototype code from ccodegen.rs
5. Port to precomp.til
6. Run ~make benchmark~ to verify
...
:STATUS: Open
:RELATED: Bug #114 (memory leaks), ownership.org
:END:
