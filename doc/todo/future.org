...
:STATUS: In Progress (blocked by Bug #143 or #144)
:BRANCH: wip110final
:RELATED: Bug #143, Bug #144, Issue #111 (enum_payload nicer with returns Dynamic)
:END:
...
**** TIL desugarer port [BLOCKED by Bug #143 or #144]
Cannot port to ~src/self/desugarer.til~ because:
- Desugarer creates multiple expressions with same line/col (from switch statement)
- TIL ccodegen uses ~line*1000000+col~ as hoisted_exprs key
- These expressions collide, causing hoisting failures and segfaults

Two paths to unblock:
1. **Bug #143**: Redesign hoisted_exprs identity mechanism (unique IDs, explicit passing,
   or single-pass approach)
2. **Bug #144**: Fix for-in to iterate by reference instead of copy. This would make
   ~to_ptr(expr)~ return consistent addresses, allowing the Rust memory-address approach
   to work in TIL too (simpler fix, also improves performance)

**** Tests
- 92/94 tests pass with Rust compiler
- 2/94 fail with TIL compiler (til_compiled mode) due to Bug #143:
  - ~src/test/test_lexer.til~ - segfault
  - ~src/test/enums.til~ - segfault (via hoisting failures)
...
- Desugarer phase has switch desugaring (desugar_switch in desugarer.rs)
- Switch handling removed from: ccodegen (emit_switch deleted)
- Switch handling still in: ufcs, precomp, interpreter (to be removed)
- enum_to_str and enum_get_payload ext_funcs ready to use
- Scavenger has NO switch handling (only a comment mentioning switch)

*** Progress Tracking

| File           | Rust | TIL | Notes                                    |
|----------------+------+-----+------------------------------------------|
| desugarer      | DONE |     | desugar_switch added                     |
| ccodegen       | DONE |     | emit_switch + helpers removed            |
| ufcs           | DONE |     | ufcs_switch removed                      |
| precomp        | DONE |     | precomp_switch removed                   |
| interpreter    | DONE |     | Switch eval removed (~360 lines)         |
| typer          | KEEP | KEEP| Validation before desugaring             |
...
**** Step 1: Add switch desugaring to desugarer (2 commits) [Rust DONE]

Reference: ccodegen's ~emit_switch()~ (lines ~6309-6400) already does this work.

***** Commit 1a: Rust desugarer [DONE 2026-01-26]
- ~src/rs/desugarer.rs~ - added ~desugar_switch()~, ~desugar_enum_switch()~, ~desugar_non_enum_switch()~
- Handles: enum switches, struct switches (eq method), range patterns, payload extraction
- Uses unique binding names ~_sw_<binding>_<case_idx>~ for TIL's function-level scoping
- Added ~find_enum_with_variant()~, ~find_enum_with_variant_payload()~ to init.rs
- All 94 tests pass
...
**** Step 2: Remove switch handling from ufcs (2 commits) [Rust DONE]

***** Commit 2a: Rust ufcs [DONE 2026-01-26]
- ~src/rs/ufcs.rs~ - removed ~ufcs_switch()~ (~80 lines), Switch returns error
- Removed unused PatternInfo import
- All 94 tests pass
...
**** Step 3: Remove switch handling from precomp (2 commits) [Rust DONE]

***** Commit 3a: Rust precomp [DONE 2026-01-26]
- ~src/rs/precomp.rs~ - removed ~precomp_switch()~ (~80 lines), Switch returns error
- Removed unused PatternInfo import
- All 94 tests pass
...
**** Step 4: Remove switch handling from interpreter (2 commits) [Rust DONE]

***** Commit 4a: Rust interpreter [DONE 2026-01-26]
- ~src/rs/interpreter.rs~ - removed switch evaluation (~360 lines), Switch returns error
- Removed unused PatternInfo import
- All 94 tests pass
...
**** Step 5: Remove switch handling from ccodegen (2 commits) [Rust DONE]

***** Commit 5a: Rust ccodegen [DONE 2026-01-26]
- ~src/rs/ccodegen.rs~ - removed ~emit_switch()~, ~VariantInfo~, ~get_case_variant_info()~, ~parse_pattern_variant_name()~
- NodeType::Switch/DefaultCase/Range/Pattern now return error "should have been desugared"
- All 94 tests pass
