...
Pipeline position: parse -> desugar -> ufcs -> *garbager* -> precomp -> typer -> ...
...
**** Step 3: Garbager Phase - Auto-generate delete() for structs
New phase after desugarer that auto-generates ~delete()~ method for any struct that lacks one.
...
**** Step 4: Garbager Phase - Liveness Analysis Infrastructure
Add liveness analysis to track when variables are last used.
