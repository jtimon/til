...
Added ~all_common~ category in tests.til that runs 3 modes:
- rs_interpreted (rstil interpret)
- rs_compiled (rstil run)
- til_interpreted (til interpret)

til_compiled excluded for now (no tests pass with it yet).

*** Test Categorization Strategy
- ~all_common~: Tests producing identical output across all 3 modes
- ~rs_common~: Tests with rstil-specific behavior or output
- ~til_interpreted~: Tests with til-specific behavior or temporary divergences
