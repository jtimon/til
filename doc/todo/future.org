...
:STATUS: Partial (infrastructure ready, blocked on idempotent delete)
...
*** Partial Implementation (2026-01-16)
Infrastructure added and ready. Auto-delete disabled pending fixes to delete()
functions to make them idempotent (safe to call multiple times).
...
*** BLOCKING: Delete Functions Must Be Idempotent
Before enabling auto-delete, fix these delete() functions:

**** Array.delete (src/core/array.til:69)
Problem: calls ~free(self.ptr)~ unconditionally - crashes on double-free
Fix: add ~if gt(self.ptr, 0)~ check before free (like Vec.delete)

**** Map.delete (src/std/map.til:247)
Problem: creates local copies that don't propagate ptr=0 back to original
#+BEGIN_SRC til
mut keys_arr := self.keys  // shallow copy shares ptr
keys_arr.delete()          // sets keys_arr.ptr=0, NOT self.keys.ptr
#+END_SRC
Fix: use ~self.keys.delete()~ and ~self.values.delete()~ directly

*** To Enable Auto-Delete
1. Fix Array.delete to be idempotent (check ptr before free)
2. Fix Map.delete to not use local copies
3. Audit other delete() functions for similar issues
4. Uncomment track_deletable_var call in emit_declaration (ccodegen.rs)
5. Port changes to ccodegen.til
6. Run ~make benchmark~ to verify

*** Types with delete()
Already implemented: Vec, Array, Set, Ptr
In std/: Map
After Issue #116: Str
