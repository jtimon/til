...
:STATUS: In Progress (Step 1 COMPLETE - all patterns desugared)
...
*** Benefits
After desugaring switch to if/else in desugarer, post-desugarer phases can be simplified:
- ~typer.rs~ (45 switch refs) - KEEP: validates switches before desugaring (pre-desugarer)
- ~ufcs.rs~ (2 switch refs) - remove switch handling
...
**** Step 1 COMPLETE: Switch desugaring in desugarer
Branch: ~claude/fix-issue-110-IOhyZ~

All switch patterns are now fully desugared to if/else chains. ~make benchmark~ passes (91/91 tests).

***** Commits:
- ~ae4e6b82~: Rust desugarer - initial ~desugar_switch()~
- ~0d27a043~: TIL port - initial port to ~src/self/desugarer.til~
- ~7b5a985e~: Nested enum patterns - grouping logic for same-outer-variant cases

***** What IS desugared (ALL patterns now):
- Enum switches with simple patterns: ~enum_to_str()~ + ~Str.eq()~
- Non-enum switches with simple cases: ~val.eq(case_val)~
- Payload extraction: ~enum_get_payload()~ with default-initialized bindings
- Range patterns (e.g., ~case 10..20:~): ~val.gteq(lo).and(val.lteq(hi))~
- Nested enum patterns (e.g., ~Type.Variant(Inner.Variant)~): grouped by outer variant
- Binding variable patterns (e.g., ~Type.Variant(binding_var)~): payload extraction + rename

***** Nested enum pattern approach (grouping)
The key insight: consecutive cases with the same outer variant must share one outer if-block.

Problem with naive approach:
#+BEGIN_SRC til
// case TType(TEnumDef): and case TType(TStructDef): generated:
if outer == "TType" { ... TEnumDef ... }
else if outer == "TType" { ... TStructDef ... }  // NEVER MATCHES!
#+END_SRC

Solution - group cases by outer variant:
#+BEGIN_SRC til
if outer == "ValueType.TType" {
    mut _payload := TTypeDef.default()
    enum_get_payload(val, TTypeDef, _payload)
    if enum_to_str(_payload).eq("TTypeDef.TEnumDef") { ... }
    else if enum_to_str(_payload).eq("TTypeDef.TStructDef") { ... }
}
#+END_SRC

***** New helper functions (Rust + TIL):
- ~get_nested_outer_key()~: identifies nested enum patterns, returns (type, variant)
- ~group_nested_enum_cases()~: groups consecutive cases with same outer variant
- ~build_grouped_inner_if_chain()~: builds inner if-else for grouped cases

**** ccodegen hoisted_exprs fix (2026-01-25)
Range patterns were initially skipped because ccodegen had a bug: ~hoisted_exprs~
(HashMap for variadic call argument temps) wasn't saved/restored for nested functions.
Fix: save and clear ~hoisted_exprs~ on nested function entry, restore on exit.

**** BLOCKING: CI failures after Step 2 (commit 910be03)
Two tests fail in ~rs_compiled~ mode after removing switch handling from UFCS:

1. ~src/test/flow.til~ - assert_eq failed at line 111: expected '2', found '0'
   - Desugared switch code produces wrong result
   - Likely issue with if/else chain generation for some case pattern

2. ~src/test/test_lexer.til~ - exit code 1 (no output)
   - May be related to same desugaring issue

These must be fixed before continuing with Steps 3-6.

**** NEXT STEPS: Cleanup (Steps 2-6)
With Step 1 complete, post-desugarer phases can be simplified (in pipeline order):

- ~typer.rs~ (45 refs): KEEP - validates switches BEFORE desugaring
- Step 2: Remove switch handling from ~ufcs.rs~ (2 refs)
- Step 3: Remove switch handling from ~precomp.rs~ (12 refs)
- Step 4: Remove switch handling from ~scavenger.rs~ (1 ref)
- Step 5: Remove switch handling from ~interpreter.rs~ (24 refs)
- Step 6: Remove switch handling from ~ccodegen.rs~ (57 refs) - including payload hoisting

Pipeline order: Typer -> Desugarer -> UFCS -> Precomp -> Scavenger -> Interpret/Build
