...
- [-] Issue #91: First-Class Functions (FunctionSig done, function pointers next)
...
:STATUS: In progress - FunctionSig type done (2026-02-21), next: function pointers + call-through-variable
...
Next: function pointers (pass functions as arguments, call-through-variable).

*** Existing Infrastructure (as of 2026-02-21)

What already exists (~40-50% complete):
...
- Test file: ~src/test/function_pointers.til~ (draft, all commented out)
...
What is missing:
- Parser: bodyless function declarations (type decls) -- needs lookahead fix
- ValueType: ~TFunction~ only stores kind, not full signature (args/returns/throws)
- Interpreter: function calls resolved by name only, no "call through parameter"
- Ccodegen: no C function pointer typedefs or pointer-based calls
- Type checking: no signature compatibility verification
...
*** Not yet working
1. *Immutable-only fields in compiled mode*: codegen doesn't emit constants
   for macro-expanded structs (only mutable fields work)
2. *Constants in struct defaults inside macros*: e.g. ~mut data: I64 = NULL~
   fails during macro eval because precomp hasn't run yet. Workaround: use
   literal ~0~ instead of ~NULL~ in struct field defaults.
