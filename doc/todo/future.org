...
:STATUS: Partial (infrastructure only, disabled until Issue #116)
...
Infrastructure added but auto-delete is disabled until Issue #116 (ownership tracking)
is implemented. Without ownership tracking, we can't determine which variables own
their memory vs which are copies/borrows, leading to double-free errors.
...
- emit_auto_delete_cleanup() to emit delete calls at function end (ccodegen.rs, disabled)
- Scavenger updated to include delete() methods for all used types (scavenger.rs, scavenger.til)
- var_lifetimes saved/restored for nested functions (ccodegen.rs)

To enable auto-delete (after Issue #116):
1. Implement ownership tracking (is_own flag) in Issue #116
2. Uncomment the track_deletable_var call in emit_declaration (ccodegen.rs:5094-5097)
3. Port changes to ccodegen.til

*** Prerequisites
- Issue #116: Str needs len/cap refactor for ownership tracking
- Bug #101: Unused variable warnings use similar liveness analysis (FIXED)
