...
:STATUS: Step 1 done (2026-02-23), Step 2 open
:RELATED: Issue #91 (first-class functions), Issue #105 (first-class types), Issue #106 (first-class enums), Issue #182 (closures), doc/first_class.org
:END:

*** Step 1: Standalone body blocks [DONE 2026-02-23]

Bare ~{ ... }~ blocks already worked -- the parser (~parse_statement~) already
had a ~LeftBrace~ case that calls ~parse_body~, and both interpreter and ccodegen
already handle Body nodes. The only ~if true { ... }~ in the codebase (typer.til)
has been replaced with a standalone block.
...
Next: lambdas, storing function pointers in collections, closures (Issue #182).
...
- *Lambdas (anonymous inline functions)*: ~numbers.map(func(x: I64) returns I64 { return x.mul(2) })~
...
- *Closures*: See Issue #182 (explicit capture lists, block captures)
