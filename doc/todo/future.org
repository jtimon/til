...
** Issue #122: Add regex support to standard library [DONE]
:PROPERTIES:
:DISCOVERED: 2026-01-15
...
*** Implementation
Pure TIL regex engine in src/std/regex.til (no external dependencies).

*** API
- ~regex_match(pattern, text)~ - returns Bool
- ~regex_find(pattern, text)~ - returns position (I64), -1 if not found
- ~regex_sub(pattern, replacement, text)~ - replace first match
- ~regex_sub_all(pattern, replacement, text)~ - replace all matches

*** Supported Features
- Literals, dot (~.~), quantifiers (~*~, ~+~, ~?~), anchors (~^~, ~$~)
- Character classes: ~[abc]~, ~[^abc]~, ~[a-z]~
- Escape sequences: ~\d~, ~\D~, ~\w~, ~\W~, ~\s~, ~\S~, ~\n~, ~\t~, ~\r~

*** Not Yet Implemented (see TODOs in regex.til)
- Alternation (~|~), grouping ~()~, capture groups, backreferences
- Non-greedy quantifiers, ~{n,m}~ repetition, word boundaries

*** Files
- src/std/regex.til - implementation
- src/test/reg_expr.til - tests (runs in all 4 modes)
- src/core/str.til - added ~Str.from_byte~ helper
...
:STATUS: Implemented
:RELATED: Bug #126 (error message divergence), bug50 (rstil vs til prefix)
:END:
...
- ~all_common~: Tests producing identical output across all 4 modes (65 tests)
