...
- ~NodeType::Pattern(PatternInfo { variant_name, binding_var })~ - e.g., Color.Green(is_olive)
- ~NodeType::Identifier(...)~ - e.g., Color.Unknown
...
- ~build_variant_eq_condition~ - build Str.eq(var, "Variant") call
- ~build_payload_extraction_body~ - build {mut var := default; enum_get_payload(...); ...body...}
