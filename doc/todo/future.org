...
:STATUS: In progress - new syntax done (2026-02-21), leverages #105/#106 patterns
...
**** New unified syntax (DONE - b1264693, 2026-02-21)
Parser now accepts the unified ~name : Type = value~ syntax for functions,
as designed in doc/first_class.org:
#+BEGIN_SRC til
// New syntax (signature is the type, body is the value):
my_double : func(a: I64) returns I64 = {
    return a.mul(2)
}
...
#+END_SRC
Both syntaxes produce the same AST (Declaration + FuncDef). Parser-only change,
no typer/interpreter/ccodegen modifications. Handles all combinations: func,
proc, ext_func, ext_proc, macro, with returns and/or throws.

Next: bodyless declarations (~my_func_ptr : func(a: I64) returns I64~ with no
~= { }~) for function pointer types. Parameter names become optional in types:
~func(I64) returns I64~ and ~func(a: I64) returns I64~ are both valid.
See also Bug #90.
...
What is done:
- Parser: new unified syntax ~name : func(...) = { body }~ (b1264693)
