...
** Issue #127: Pipeline refactoring - Add Sugar and UFCS phases
:PROPERTIES:
...
- Sugar: desugar forin, switch, range for (needs type info)
- UFCS: desugar x.method(y) -> Type.method(x, y)
- Precomp: const-folding, macros only

*** Migration TODOs
See doc/pipeline.org "Migration TODOs" section for detailed steps.

*** Related Issues
- Issue #110: Switch desugaring (will move to Sugar phase)
...
:STATUS: Open - WIP in branch 110_attempt2
...
**** Commit 1: Switch desugaring in precomp

***** Files to modify
- ~src/rs/precomp.rs~ - add desugar_switch, modify Switch case
- ~src/self/precomp.til~ - port same changes (follow ~doc/rs2til.org~ translation rules)
