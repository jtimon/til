...
:STATUS: Open (Steps 1-3 done, Step 4 pending)
:DEPENDS: Bug #101 (unused warnings - shared infrastructure)
:RELATED: Bug #114 (memory leak documentation), Bug #141 (enum payloads)
:END:
...
Pipeline position: typer -> desugar -> *garbager* -> ufcs -> precomp -> ...
...
New phase after desugarer, before ufcs that auto-generates ~delete()~ method for
any struct that lacks one.

Algorithm:
1. When processing a struct definition, check if it has a ~delete~ method
2. If not, generate one that calls ~Type.delete(self.field)~ for every mutable field
   - Fields processed in reverse declaration order (like C++/Rust destructors)
   - Skip enum fields (enums don't have delete methods)
   - Skip primitive fields (I64, U8, Bool, Type, Dynamic)
   - Skip structs with no deletable fields
3. Insert the generated method into the struct's members and default_values
4. Register the modified struct in Context so UFCS can find the new method

Note: Uses post-UFCS form (~Type.delete(self.field)~) because garbager runs after
typer, so generated nodes don't have type annotations that UFCS needs.
...
delete := proc(mut self: MyStruct) {
    // count skipped - I64 is a primitive
    Vec.delete(self.data)   // Vec.delete() frees memory
    Str.delete(self.name)   // Str.delete() frees memory
}
#+END_SRC

Files: garbager.rs (DONE), garbager.til (DONE)

**** Step 4: Delete insertion infrastructure (DONE)
Built the infrastructure for inserting delete() calls, but disabled until Step 5.

Data structure:
#+BEGIN_SRC rust
struct VarLifetime {
    name: String,
    type_name: String,  // For post-UFCS delete calls
...
- ~collect_var_declarations~: Collect mutable variables that need deletion
- ~generate_delete_call~: Generate ~Type.delete(var)~ in post-UFCS form
- ~generate_deletes_for_vars~: Generate deletes for all live vars in reverse order
- ~needs_delete~: Check if type needs deletion (skips primitives, enums)

Why disabled: Naive insertion at function end caused double-free errors for variables
that were returned, assigned to other variables, or escaped scope. Proper liveness
analysis (Step 5) is required to know when a variable is truly dead.

Files: garbager.rs (DONE), garbager.til (DONE)

**** Step 5: Liveness analysis for safe delete insertion
Add liveness analysis to track when variables are truly last used, avoiding double-frees.
...
- Branches (if/else/switch): Conservative - only delete if ALL paths agree variable is dead
- Loops: Most conservative - variables used in loop body deleted after loop ends
- Returns: Do NOT delete variables that are being returned
- Assignments: Track when ownership transfers to another variable
...
- Desugarer phase EXISTS (from Issue #127) - handles ForIn, but NO switch desugaring yet
- Switch still handled by: ufcs, precomp, interpreter, ccodegen
- ccodegen's ~emit_switch()~ already generates if/else chains - reference implementation
