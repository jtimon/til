...
**** Step 1: Optional ? support (parser + typer accept it)
Goal: Add syntax support without breaking existing code.

***** Parser changes (parser.rs, parser.til)
- After parsing an FCall, check if next token is ~?~
- If yes, consume it and set flag in FCall node
- AST change: ~FCall~ becomes ~FCall(bool)~ where true = has ~?~
- All existing FCall patterns need updating: ~NodeType::FCall~ -> ~NodeType::FCall(_)~ or ~NodeType::FCall(false)~

***** Typer changes (typer.rs, typer.til)
- When checking FCall, look up the called function's throws section
- If function throws and call has ~?~: valid (propagates throw)
- If function throws and call has no ~?~: valid FOR NOW (still allow sneaky throws)
- If function doesn't throw and call has ~?~: type error ("? used on non-throwing function")
...
**** Step 2: Warning for missing ? (optional, -Wsneaky-throws)
- Add warning when calling throwing function without ~?~
- Off by default, enable with flag
- Helps find places that need updating before Step 3

**** Step 3: Mandatory ? (breaking change)
- Change typer: missing ~?~ on throwing call = type error
- Requires updating all call sites (~830+ in src/self alone)
- Big refactor, needs coordination

**** Step 4: Strip ? from AST after typer
- After typer validates, ~?~ info no longer needed
- Resolve phase or end of typer can convert ~FCall(true)~ back to ~FCall(false)~
