...
:STATUS: In Progress (Step 1 partial - some patterns not desugared)
...
**** Step 1 PARTIAL: Switch desugaring in desugarer
Branch: ~claude/fix-issue-110-IOhyZ~

- Commit ~ae4e6b82~: Rust desugarer - added ~desugar_switch()~ (~740 lines)
- Commit ~0d27a043~: TIL port - ported to ~src/self/desugarer.til~ (~750 lines)

***** What IS desugared (to if/else chains):
...
***** What is NOT desugared (left for interpreter/ccodegen):
- Nested enum patterns (e.g., ~Type.Variant(Inner.Variant)~) - too complex

All tests pass (~make benchmark~ exit 0).
...
**** REMAINING: Nested enum patterns

Before Steps 2-6 can proceed, Step 1 needs completion:
- Desugar nested enum patterns

***** Example pattern
~case ValueType.TType(TTypeDef.TStructDef):~ - used throughout TIL self-hosted compiler

***** ccodegen approach (lines 6762-6774)
Generates direct C struct access:
#+BEGIN_SRC c
switch_var.tag == til_ValueType_TType &&
switch_var.payload.TType == til_TTypeDef_TStructDef
#+END_SRC

***** desugarer approach
Generate nested ifs with enum_to_str and enum_get_payload:
#+BEGIN_SRC til
if enum_to_str(val).eq("ValueType.TType") {
    mut _tmp_payload := TTypeDef.default()  // first variant as default
    enum_get_payload(val, "TTypeDef", _tmp_payload)
    if enum_to_str(_tmp_payload).eq("TTypeDef.TStructDef") {
        // body
    } else {
...
***** Implementation notes
1. Detect nested pattern: already done in ~has_nested_enum_pattern~
2. Instead of skipping, recursively desugar the payload pattern
3. For nested enum check without binding (like ~TTypeDef.TStructDef~):
   - Need temp var with default value for enum_get_payload inout param
   - Generate inner if with enum_to_str check on extracted payload
4. For nested enum with binding (like ~TTypeDef.TStructDef(x)~):
   - Same as above, but also bind x to extracted inner payload

Once ALL patterns are desugared, cleanup Steps 2-6 can remove switch handling from downstream phases.
