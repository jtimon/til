...
Implemented in branch claude/fix-issue-108-37Gkm.

What works:
- ~namespace TypeName { ... }~ syntax for both enums and structs
- Namespace members registered as ~TypeName.member_name~
- UFCS works: ~val.method()~ calls ~TypeName.method(val)~
- Parser enforces only declarations inside namespace blocks

Known limitations:
- Chained calls like ~EnumType.Variant.method()~ require intermediate variable
- Namespace blocks in imported files don't work in compiled mode yet (works in interpreted)

Tests: src/test/namespaces.til
