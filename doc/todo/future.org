...
:STATUS: Partial (infrastructure ready, blocked on manual delete() removal)
:DEPENDS: Bug #101 (unused warnings - FIXED)
:RELATED: Bug #114 (memory leak documentation), Issue #116 (future borrow tracking)
:END:
...
All mut local variables are considered owned. For borrowed/aliased data, users must:
- Use Ptr or c_mem functions manually
- Implement their own delete() that handles aliasing correctly
See Issue #116 for future borrow tracking support.

*** Implementation Status (2026-01-16)
Infrastructure complete and tested. Auto-delete is DISABLED because the TIL
codebase itself has manual delete() calls that conflict with auto-delete.

**** Completed Infrastructure:
- VarLifetime struct for tracking deletable variables (ccodegen.rs)
- type_has_delete() helper to check if type has delete method (ccodegen.rs)
- track_deletable_var() to register variables for cleanup (ccodegen.rs, disabled)
- emit_auto_delete_cleanup() to emit delete calls at function end (ccodegen.rs)
- Scavenger marks delete() methods reachable for all used types (scavenger.rs, scavenger.til)
- var_lifetimes saved/restored for nested functions (ccodegen.rs)

**** Fixed Delete Functions (2026-01-16):
All delete() functions are now idempotent (safe to call multiple times):

- Array.delete (src/core/array.til): Added ~if gt(self.ptr, 0)~ check before free
- Map.delete (src/std/map.til): Fixed to use ~self.keys.delete()~ directly instead of local copies
- Ptr.delete (src/core/ptr.til): Added NULL check, sets data=0, takes mut self
- List.delete (src/std/list.til): Added ~if gt(self.data, 0)~ check before free

*** BLOCKING: Manual delete() Calls in TIL Codebase
Auto-delete is disabled because when rstil compiles til.til with auto-delete enabled,
the TIL interpreter/compiler code has manual delete() calls that conflict:
- Manual delete() + auto-delete = double-free

To enable auto-delete:
1. Remove ALL manual delete() calls from TIL codebase (src/self/*.til, src/core/*.til, src/std/*.til)
2. Uncomment track_deletable_var call in emit_declaration (ccodegen.rs:~2847)
3. Port changes to ccodegen.til
...
2. Double-free tests - all delete() must be idempotent
