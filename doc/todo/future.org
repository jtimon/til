...
**** Step 4: Liveness Analysis and Delete Insertion (DISABLED - partial impl)
Add liveness analysis to track when variables are last used, then insert ~var.delete()~ calls.

*Status:* VarLifetime struct and helper functions are implemented in garbager.rs but
DISABLED pending more testing. Complex interactions with:
- Return values: Must NOT delete variables being returned (ownership transfer)
- Control flow: Early exits need deletes inserted before return/throw
- Ptr.offset() creates aliased pointers that can't be individually deleted

The implementation is marked with ~#[allow(dead_code)]~ in Rust. Can be enabled
incrementally once proper testing infrastructure is in place.
...
3. Return handling: Variables used in return expression should NOT be deleted

Handle control flow:
- Early exits (return/throw): Insert deletes for all still-live variables before exit
  - EXCEPT variables used in the return/throw expression (ownership transfers)
...
:STATUS: In Progress (Bug #143 FIXED, may still be blocked by #144)
:BRANCH: wip110final
