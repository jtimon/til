...
:RELATED: Bug #65 (specific shadowing divergence), Bug #97 (fix via name mangling)
:END:
...
*** Not yet working
1. *Macro parameters in struct defaults*: init type-checks the struct literal
   inside the macro body before macro expansion, so macro params are
   "undefined symbol". This blocks the template use case:
   ~make_typed(T: Type) returns struct { ... Vec.new(T) ... }~
2. *Namespace blocks on macro-defined structs*: init processes namespaces
   before macro expansion, so the struct isn't registered yet
3. *Immutable-only fields in compiled mode*: codegen doesn't emit constants
