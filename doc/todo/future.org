...
AST transformation in a new "garbage" phase. Inserts ~var.delete()~ calls directly
into the AST after last use. Both compiler and interpreter benefit automatically.

Pipeline position: parse -> desugar -> ufcs -> *garbage* -> precomp -> typer -> ...

Files: garbage.rs, garbage.til (new)

*** Implementation Steps

**** Step 1: Add is_borrowed to Ptr
...
**** Step 2: Auto-generate delete() for structs
In desugarer phase, auto-generate ~delete()~ method for any struct that lacks one.

Prerequisites:
- Add ~delete()~ to primitive types (I64, U8, Bool) that does nothing
- This ensures ALL types have ~delete()~, simplifying the garbage phase

Algorithm:
1. When processing a struct definition, check if it has a ~delete~ method
2. If not, generate one that calls ~field.delete()~ for every field
   - Fields processed in reverse declaration order (like C++/Rust destructors)
3. Insert the generated method into the struct's members
...
Files: desugarer.rs, desugarer.til, i64.til (new), u8.til, bool.til

**** Step 3: Garbage Phase Infrastructure
New AST transformation pass that inserts delete() calls.
...
Note: No need for ~has_delete~ field - after Step 2, every struct has ~delete()~.
...
Files: garbage.rs, garbage.til

**** Step 4: Handle Early Exits (return/throw)
Before any return/throw statement, insert deletes for all still-live variables.
Files: garbage.rs, garbage.til

**** Step 5: Handle Branches (if/else/switch)
Conservative: Only insert delete if ALL paths agree variable is dead.
Otherwise, insert delete after the branch construct.
Files: garbage.rs, garbage.til
...
**** Step 7: Scope Exit Safety Net
Insert deletes for any remaining live variables at function end.
Files: garbage.rs, garbage.til
