...
- [X] Create src/std/meta.til with Introspection struct
- [X] Implement has_const() as ext_func in Rust (src/rs/ext.rs)
- [X] Implement has_field() as ext_func in Rust (src/rs/ext.rs)
- [ ] Implement has_const() in C for compiled mode (src/ext.c)
- [ ] Implement has_field() in C for compiled mode (src/ext.c)
- [X] Test has_const() with existing types (check for clone, len, size, etc.)
