...
** TODO DRY #11: Error Message Construction                            :HIGH:
:PROPERTIES:
...
** TODO DRY #13: Eval + Throw Propagation Pattern (PARTIAL)            :HIGH:
:PROPERTIES:
:DISCOVERED: 2025-01-10
:IMPACT: 25 locations total (14 in interpreter.rs, 11 in ext.rs)
:EFFORT: 3-4 hours (estimated), 1 hour (partial progress)
:LOCATIONS: interpreter.rs (14 remaining), ext.rs (4 replaced, 7 remaining)
:BENEFIT: Eliminate ~75 lines total, reduce error-prone boilerplate
:PROGRESS: eval_or_throw! macro created, 4/25 locations replaced (16%), 17 net lines saved
:END:
...
*** Estimated Savings
~75 lines of repetitive throw propagation code (revised from initial ~150 estimate)

*** Partial Implementation Notes (2025-01-11)
Created =eval_or_throw!= macro in both interpreter.rs and ext.rs that consolidates the pattern:
#+BEGIN_SRC rust
let result = eval_expr(context, e)?;
if result.is_throw { return Ok(result); }
let value = result.value;
#+END_SRC

Into a single macro call:
#+BEGIN_SRC rust
let value = eval_or_throw!(context, e);
#+END_SRC

Completed 4 replacements in ext.rs (func_malloc, func_free, func_memset).
Remaining: 7 in ext.rs + 14 in interpreter.rs = 21 locations.

**Next Steps:** Continue replacing remaining patterns systematically. Each replacement saves 3 lines and improves code readability by making the throw propagation intent explicit.

** TODO DRY #17: Enum Payload Extraction Pattern                     :MEDIUM:
:PROPERTIES:
:DISCOVERED: 2025-01-10
...
- Careful analysis of existing enum payload extraction patterns across switch statements and catch blocks
- Complex byte array manipulation and type conversion logic
- Integration with the arena memory model
- Handling of nested enum types and custom struct payloads
- Extensive testing due to the critical nature of payload extraction

Given the complexity, this task should be tackled in a dedicated session with sufficient context budget. The refactoring involves restructuring core interpreter functionality that handles runtime enum value extraction, which requires careful attention to edge cases and error handling.

** TODO DRY #19: Boolean Condition Type Checking                        :LOW:
:PROPERTIES:
:DISCOVERED: 2025-01-10
...
** TODO DRY #20: Context Symbol Lookup with Type Validation             :LOW:
:PROPERTIES:
:DISCOVERED: 2025-01-10
...
** FIXED DRY #14: Type Insertion Pattern                                :MEDIUM:
:PROPERTIES:
:DISCOVERED: 2025-01-10
:COMPLETED: 2025-01-11
:IMPACT: 3 blocks in interpreter.rs
:EFFORT: 2 hours (actual)
:LOCATIONS: interpreter.rs lines 877-916, 966-997, 810-820
:BENEFIT: Consolidated 3 blocks of duplicated code, saved 45 lines net
:END:
...
** FIXED DRY #15: Variadic Function Argument Handling                   :MEDIUM:
:PROPERTIES:
:DISCOVERED: 2025-01-10
:COMPLETED: 2025-01-11
:IMPACT: 2 locations in typer.rs and interpreter.rs
:EFFORT: 2 hours (actual)
:LOCATIONS: typer.rs lines 267-279, interpreter.rs lines 1477-1485
:BENEFIT: Consolidated validation logic, eliminated ~17 lines
:END:
...
** FIXED DRY #16: Struct Member Lookup Pattern                          :MEDIUM:
:PROPERTIES:
:DISCOVERED: 2025-01-10
:COMPLETED: 2025-01-11
:IMPACT: 8 locations across init.rs and interpreter.rs
:EFFORT: 1.5 hours (actual)
:LOCATIONS: init.rs lines 161, 321, 336, 389, 476, 495; interpreter.rs lines 1053, 1105
:BENEFIT: Eliminated 14 lines net, improved consistency and safety
:END:
...
|----------+-------+------------------------+-------------------|
| HIGH     |     2 | 7-10 hours             | 250-350 lines     |
| MEDIUM   |     2 | 6.5-9 hours            | 130+ lines        |
| LOW      |     2 | 3 hours                | ~45 lines         |
|----------+-------+------------------------+-------------------|
| TOTAL    |     6 | 16.5-22 hours          | 425-525 lines     |
| FIXED    |     6 | 10.5 hours             | 150 lines         |
