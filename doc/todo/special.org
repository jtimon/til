...
After cleanup (2025-11-12), most removable technical debt has been eliminated.

**What remains is ALSO technical debt**, but it's necessary technical debt that cannot
be removed until self-hosting. These are primitive type operations that should eventually
live in core.til, not Rust.
...
**Is this technical debt?**: Yes - eventually this should be handled generically in TIL, but it requires string literal support in the language that's currently in Rust.
...
**Is this technical debt?**: Yes - Str initialization should eventually be handled by TIL code, not Rust special cases.
...
**ALL special cases are technical debt** - some can be removed now (already done),
others require self-hosting first. The distinction is:

- **Removable now**: Workarounds from when structs-as-fields didn't work (DONE)
- **Requires self-hosting**: Primitive operations that should live in TIL, not Rust

Current state: Most immediately-removable technical debt has been eliminated.
What remains is technical debt that requires self-hosting to properly address.
