...
**Reason**: Added complexity without simplifying code, introduced subtle bugs with arena offset sharing

Goal: Both const (default) and mut parameters use references. Only copy creates actual copies.

*** Attempted Implementation (2025-11-12)

Tried two approaches:
1. Share arena offsets for ALL non-copy parameters (const + mut)
   - Result: Type checker errors in core.til line 400
   - Cause: Context corruption during import processing

2. Share arena offsets ONLY for mut parameters
   - Result: More tests passed, but still fails at args.til line 580
   - Issue: Global arena singleton + context cloning + import interpretation creates subtle corruption

*** Root Cause

The global arena (Arena::g() singleton) combined with context cloning during import processing
creates a window for corruption when arena offsets are shared. The exact mechanism is unclear,
but the pattern is consistent: complex import chains fail, simple tests pass.

*** Why Not Pursued

- Did NOT simplify code as expected (added ~50-100 lines, introduced complexity)
- All tests pass WITHOUT Phase 3
- Phase 2 (copy keyword) already provides explicit control
- Correctness > Performance at this stage
- Can revisit after self-hosting with better debugging tools

*** Documentation

See doc/bot/phase3_findings.org for detailed analysis and alternative approaches.

*** Original Plan (For Future Reference)

**** Change Default Parameter Passing (Const)
File: src/rs/interpreter.rs (lines 1615-1700)
- Replace insert_struct() + copy_fields() with arena offset sharing
- Share arena offset from caller context
- Register field offsets via map_instance_fields()

**** Include Primitives in Reference Passing
File: src/rs/interpreter.rs (lines 1593-1604)
- Replace insert_i64(), insert_bool() allocations with offset sharing
- For literals: Still allocate (caller evaluates these)

**** Keep Copy Parameter Behavior
- Copy parameters continue to use copy_fields() deep copy
- Check is_copy field to determine behavior

** Phase 4: Update Core Library ⏸ BLOCKED

**Status**: BLOCKED by Phase 3 deferral
**Reason**: Cannot remove Bug #24 workarounds without reference semantics

Goal: Remove Bug #24 workarounds once references work correctly.

The copy-modify-reassign patterns in List.push(), List.set(), List.pop() and similar
methods must remain until Phase 3 is completed. These workarounds are necessary because
field access returns copies, not references.

Example workaround (must stay):
#+BEGIN_SRC til
// List.push - lines 1003-1009
mut names := self.type_names
names.push(type_name)
self.type_names = names
#+END_SRC

This will be simplified once Phase 3 implements reference semantics:
#+BEGIN_SRC til
// Future: after Phase 3
self.type_names.push(type_name)  // Will work directly
#+END_SRC
