...
+ ~dont_delete~. User syntax unchanged.

Generated code for ~for item: MyStruct in vec { body }~:
#+BEGIN_SRC til
for _i in 0..vec.len() {
...
Key pieces:
- ~get_by_ref~: like ~get~ but returns borrowed Ptr (~is_borrowed=1~)
- ~Ptr.create_alias~: ext_func, zero-copy alias (U8 ext_func wrapper pattern)
- ~dont_delete~: garbager directive, skips ~Type.delete()~ for aliased local

Dependencies: #117 Step 8 (~dont_delete~), ~get_by_ref~, ~create_alias~ ext_func.
