...
*** Planned Fix
This bug will be fixed by implementing pass-by-reference semantics with three argument modes. See doc/todo/pre.org for the complete implementation plan:

*Phase 1: Type checker immutability enforcement*
- Prevent modification of immutable parameters
- Foundation for reference semantics

*Phase 2: Add 'copy' keyword*
- Explicit copy when needed: proc(copy s: Struct)
- Complements mut and default (immutable)

*Phase 3: Optimize to use references*
- Default parameters: pass by reference (immutable)
- mut parameters: pass by reference (mutable)
- copy parameters: explicit copy
- Primitives stay pass-by-value (optimal)

*Phase 4: Update core library*
- Remove Bug #24 workarounds from List, Vec, Map methods
