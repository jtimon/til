...
*** Description
When calling a throwing function inside struct literal named parameters, the compiler fails to recognize that the function throws and incorrectly reports that the error type is never thrown.

#+BEGIN_EXAMPLE til
Map.clone := func(self: Map) returns Map throws AllocError {
    return Map(
        keys=self.keys.clone(),      // Vec.clone() throws AllocError
        values=self.values.clone()   // Vec.clone() throws AllocError
    )
}

// Compiler error:
// `AllocError` is declared in the throws section, but this function never throws it.
#+END_EXAMPLE

*** Workaround
Assign throwing function results to variables before using them in struct literals:

#+BEGIN_EXAMPLE til
Map.clone := func(self: Map) returns Map throws AllocError {
    cloned_keys := self.keys.clone()
    cloned_values := self.values.clone()
    return Map(
        keys=cloned_keys,
...
*** Location
- Type checker error detection in struct literal parameter evaluation
