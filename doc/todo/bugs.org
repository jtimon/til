...
:STATUS: In Progress - Steps 3-8 done, copy_fields removed, own params simplified
:END:

*** CRITICAL: The Goal
Remove =copy_fields= (shallow copy) from interpreter and codegen. Replace with:
1. Garbager inserts =Type.clone()= calls for struct-typed identifier args
2. Interpreter/ccodegen use offset binding instead of copy_fields (or the equivalent in codegen)

*** CRITICAL: The Rule
- *MUST* remove the corresponding shallow copy for every deep copy added
- *MUST NOT* commit with both shallow and deep copy for the same case

*** MUST: Port to TIL
Any Rust changes *MUST* be ported to equivalent TIL files before testing.
...
*** Dependencies
- Bug #160: Eliminate field paths from arena_index (FIXED)
...
**** Step 2: DONE - Move dynamic type info to Ptr
Added fields to Ptr struct (ptr.til):
- =alloc_size: I64= - total bytes allocated
- =elem_type: I64= - Ptr to null-terminated type name C string
- =elem_size: I64= - bytes per element (0 = single value, not array)

Changes:
- =Ptr.new_by_size=: sets alloc_size
- =Ptr.new=: sets elem_size
- =Ptr.new_array=: new method for array allocation
- =Ptr.clone=: deep copy if alloc_size > 0, else borrowed shallow
- =Ptr.offset=: copies type info, marks as borrowed
- =Vec._alloc_ptr=: helper that sets elem_type and elem_size
- Updated all Ptr compound literals in ccodegen.rs/til and ext.c (5-field format)
- Updated size_of(Str) test: 32 -> 56 bytes (Ptr grew from 16 to 40)

**** Step 3: DONE - mut declarations (commits ba5054fe, 045c0307, 2c2bd35e)

The pattern for replacing copy_fields with clone:

**1. Garbager (AST transformation)**:
- Detect: =mut x := y= where y is struct identifier
- Transform to: =mut x := Type.clone(y)=
...
**2. Interpreter (offset binding only)**:
- No special handling for mut vs non-mut
- Just bind declaration name to result offset
- No copy_fields, no field mapping

Rust (interpreter.rs):
#+BEGIN_SRC rust
let offset = context.scope_stack.lookup_var(&expr_result_str)
    .ok_or_else(|| ...)?;
context.scope_stack.frames.last_mut().unwrap()
    .arena_index.insert(declaration.name.to_string(), offset);
#+END_SRC
...
**3. Ccodegen (removed duplicate logic)**:
- Clone call is already in AST from garbager, no special handling needed
- Just emit the expression normally

Rust (ccodegen.rs) - REMOVED this code:
#+BEGIN_SRC rust
// REMOVED - garbager handles this now
...
TIL (ccodegen.til) - REMOVED this code:
#+BEGIN_SRC til
// REMOVED - garbager handles this now
mut rhs_is_identifier := false
switch rhs_first_param.node_type {
case NodeType.Identifier(_):
    rhs_is_identifier = true
case:
}
mut needs_clone := false
if not(has_rhs_string).and(rhs_is_identifier) {
    switch var_type {
    case ValueType.TCustom(type_name):
        needs_clone = context.scope_stack.has_struct(type_name)
    case:
    }
}
if needs_clone {
    ...
    case ValueType.TCustom(type_name):
        output.push_str(til_name(type_name))
        output.push_str("_clone(&")
        emit_expr(rhs_first_param, output, 0, ctx, context)?
        output.push_str(")")
    case:
    }
}
...
**** Step 4: Remaining copy_fields locations (progress tracking)
| Location              | Purpose                  | Status         |
|-----------------------+--------------------------+----------------|
| mut declarations      | =mut x := y=            | DONE (Step 3)  |
| Function copy params  | =func(copy p: T)=       | DONE (Step 4)  |
| Struct literal fields | =Point(field=y)=        | DONE (Step 5)  |
| Assignment statements | =x = y=, =x.field = y=  | DONE (Step 6)  |
| Nested member assign  | struct member defaults   | DONE (Step 7)  |
| Own params (ident)    | =func(own p: T)= ident  | DONE (Step 7)  |
| Own params (expr)     | =func(own p: T)= expr   | DONE (Step 7)  |

**** Step 5: DONE - Struct literal fields (commit d697125c)

Replace =insert_struct_instance= + =copy_fields= with memcpy for struct literal
field initialization (=Point(field=y)=).

Pattern: get src_offset from =lookup_var=, dest_offset from =get_field_offset=,
type_size from =get_type_size=, then memcpy src to dest.

No garbager change needed - clone is already inserted for identifier args by Step 4's
garbager logic.

**** Step 6: DONE - Assignment statements (commit 16cfd9bb)

Replace =copy_fields= with memcpy for struct assignment (=x = y=, =x.field = y=).

Two sub-cases:
- Field path (=x.field = y=): memcpy into field slot via =get_field_offset=
- Simple var (=x = y=): memcpy into existing slot via =lookup_var=

Must use memcpy (not offset rebinding) for simple vars because the variable may live
in an outer scope frame. Offset rebinding would only update the current frame.

**** Step 7: DONE - Remaining copy_fields (nested member, own params, copy_fields deleted)

Replaced all remaining =copy_fields= calls with memcpy, then deleted =copy_fields=
from both =eval_arena.rs= and =eval_arena.til=. Also removed =temp_src_key= from
=SavedOffsets= (no longer needed without copy_fields' symbol registration dance).

Three locations replaced:
- Nested member assignment: struct member default values during struct definition
- Own params (identifier args): =func(own p: T)= with identifier argument
- Own params (expression args): =func(own p: T)= with expression argument

Also cleaned up unnecessary =.map_err()= wrappers on =get_field_offset= / =get_type_size=
calls (Steps 5-7). These functions already return meaningful error strings; the
=.map_err()= was cosmetic noise, and in TIL the equivalent =catch (err: Str)= is wrong
(you never catch =Str= — it's the final propagation type, not a specific error type).

**** Step 8: DONE - Own params simplified to offset binding

=own= params changed from allocate+memcpy to pure offset binding (same as =default=
and =mut=). Ownership transfer doesn't need copying — just bind the existing offset
and remove the source from the caller's scope.

Removed dead code:
- =SavedOffsets= struct (no longer needed without allocation overwriting caller offsets)
- =insert_struct_instance_into_frame= function (no longer called)
- =EvalArenaMapping= import from interpreter.rs

*** Parameter passing semantics summary
...
Only =copy= params need the garbager to insert =Type.clone()= calls.
=own= params should just rebind the offset and remove the source from
the caller's scope (ownership transferred, no copy needed).
