...
Make ~create_alias~ a compound directive handled by compiler phases:
- **init/typer**: sees ~create_alias(item: MyStruct, addr)~, declares ~item~ in scope
- **garbager**: expands to low-level ops (declaration + rebind), does NOT insert
  auto-delete for the variable (replaces separate ~dont_delete~)
- **interpreter/ccodegen**: see expanded ops, no wasted default construction

Desugared for-in becomes:
#+BEGIN_SRC til
