...
*** Description
When a function takes variadic arguments (~args: ..Str~), there's no way to pass
a Vec or Array of values to it. This is needed when forwarding arguments or
when arguments are collected dynamically.

*** Example
#+BEGIN_SRC til
// We have a Vec of args
...
const args = ["--verbose", "output.txt"];
runCmd(exePath, ...args);  // spreads array into arguments
#+END_SRC

*Python (unpacking operator):*
#+BEGIN_SRC python
args = ["--verbose", "output.txt"]
run_cmd(exe_path, *args)  # unpacks list into arguments
#+END_SRC

*Rust (no direct equivalent, uses slices):*
#+BEGIN_SRC rust
...
#+BEGIN_SRC ruby
args = ["--verbose", "output.txt"]
run_cmd(exe_path, *args)  # splats array into arguments
#+END_SRC

*Go (variadic expansion):*
#+BEGIN_SRC go
args := []string{"--verbose", "output.txt"}
runCmd(exePath, args...)  // expands slice into variadic
#+END_SRC

*** Suggested TIL Syntax
#+BEGIN_SRC til
// Option 1: Prefix operator like Python/Ruby
run_cmd(output, exe_path, ..extra_args)

// Option 2: Suffix like Go
run_cmd(output, exe_path, extra_args..)

// Option 3: Explicit spread function
run_cmd(output, exe_path, spread(extra_args))
#+END_SRC
...
**** 2026-01-10: Current State
Build passes. 134/134 tests pass. However:
- 7 tests in all_common (work with both rstil and til compilers)
- 58 tests in til_interpreted (work with til interpret, not til run)
- These 58 tests fail in til_compiled mode due to C codegen issues

*** Current Error Summary

The 58 tests in til_interpreted have these failure modes when run with ~./bin/til run~:

| Error Type | Count | Example |
|------------|-------|---------|
| too few args to til_Array_get | 38 | Missing arguments in generated function calls |
| segfault | 16 | Crash during C execution |
| malloc failed | 1 | ufcs.til - memory issue during compilation |
| may pass | 2 | exit.til, undefined.til (expected exit codes) |
...
See plan file: ~/.claude/plans/precious-enchanting-dewdrop.md

**** Iterative Fix Workflow
1. User picks a test from til_interpreted that they want working in til_compiled
2. Run til_diff to compare rstil vs til generated C code
3. Analyze the diff - identify what's different
...
8. Run ~make benchmark 2>&1 | tee tmp/benchmark_output.txt~
9. Re-run all failing tests in parallel to update TODO comments:
   - Run ~timeout 10 ./bin/til run src/test/X.til 2>&1~ for each test
   - Update inline TODO comments with current errors
   - If any tests now pass, move them to all_common
   - If tests were moved, run ~make benchmark~ again to verify
10. Commit the fix (tests.til has up-to-date TODOs)
...
- ~invalid initializer~ (e.g., ~til_Str x = til_panic;~) - function treated as value
