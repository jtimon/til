...
:STATUS: Fixed (Option B - per-function counter reset)
...
**** Option A: Reset counter per-file (simpler)
Reset ~LOOP_VAR_COUNTER~ at the start of ~parse_tokens()~. This ensures
deterministic naming within each file, but different files may still get
different values if processed in different orders.

**** Option B: Reset counter per-function (like Issue #127)
Reset the counter for each function definition, similar to how Issue #127
fixed ~precomp_forin_counter~. This provides stronger determinism guarantees.

**** Option C: Fix file processing order (addresses root cause)
Investigate why files are processed in different orders between builds.
Replace HashMap with Vec or use sorted iteration where iteration order matters.
This would provide complete determinism regardless of counter reset strategy.
