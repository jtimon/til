...
| emit_break | Y | Y | MATCH | mut result + while loop = acceptable TIL patterns |
| emit_continue | Y | Y | MATCH | mut result + while loop = acceptable TIL patterns |
| emit_switch | Y | Y | MATCH | Removed SwitchCaseInfo, parser enforces default-last |
| emit_fcall | Y | Y | DIVERGE | Fixed replacen hack, needs full verification |
