...
Then at line 2329, TIL switch uses ~custom_type_name~ ("Dynamic") instead of
~resolved_type_name~ ("I64"), causing the wrong code path.

*** Reproducing
#+BEGIN_SRC til
test := proc() {
    mut v := Vec.new(I64)
    v.push(10)  // ERROR: Source '10' not found in caller context
}
#+END_SRC
...
*** Fix
Both Rust and TIL should use ~resolved_type_name~ (no shadowing).
See Issue #66 for long-term shadowing policy discussion.
