...
*** Reproducing
#+BEGIN_SRC bash
./bin/rstil run src/test/literals.til
# Then inspect gen/c/test/literals.c
#+END_SRC

In src/test/literals.til:
#+BEGIN_SRC til
x: U8 = 34
y: U8 = 35
r: U8 = 69
...
Generated C code (wrong):
#+BEGIN_SRC c
const til_I64 til_x = 34;
const til_I64 til_y = 35;
const til_I64 til_r = 69;
#+END_SRC

Expected:
#+BEGIN_SRC c
const til_U8 til_x = 34;
const til_U8 til_y = 35;
const til_U8 til_r = 69;
#+END_SRC

*** Root Cause
Likely in ccodegen.rs where variable declarations are emitted - the type
annotation is being ignored and defaulting to I64 for integer literals.
...
log_with_level(2, "hello", "world")  // Works: level=2, messages=["hello", "world"]
log_with_level("hello", "world")     // FAILS: "hello" goes to level position (type error)
#+END_SRC

This prevents patterns like `run_cmd(mut output: Str, timeout: I64 = 0, args: ..Str)`.

*** Reproducing
See test_optional_before_variadic in src/test/args.til (currently commented out).

*** Expected Behavior
When the provided arg type doesn't match the expected param type AND the param
has a default value, the typer should:
1. Use the default value for that param
2. Shift the provided arg to the next position (variadic in this case)

*** Workaround
Use separate functions: run_cmd (no timeout) and run_cmd_timeout (with timeout).

*** Fix Plan
Root cause: In check_fcall (typer.rs line ~496), arg matching is purely positional:
#+BEGIN_SRC rust
let arg = match func_def.args.get(std::cmp::min(i, max_arg_def - 1)) { ... }
#+END_SRC

Fix approach - type-aware matching when optional args exist:
1. Before assigning arg to param, check if type matches
2. If param has default AND type doesn't match AND variadic follows:
   - Skip this param (use default)
   - Continue checking next optional param
3. Handle multiple optional args before variadic:
#+BEGIN_SRC til
func(a: I64 = 1, b: Str = "x", c: Bool = false, msgs: ..Str)
func("hello")           // a=1, b="x", c=false, msgs=["hello"]
func(5, "hello")        // a=5, b="x", c=false, msgs=["hello"]
func(5, "y", "hello")   // a=5, b="y", c=false, msgs=["hello"]
#+END_SRC

Files to modify:
- src/rs/typer.rs - check_fcall function (lines 474-627)
- src/self/typer.til - same changes
- src/test/args.til - uncomment test_optional_before_variadic (line 739)

** Bug #60: ccodegen passes const args by value instead of by pointer
:PROPERTIES:
...
*** Description
In TIL, function arguments without `mut`, `copy`, or `own` modifiers should be
passed as const by reference (const pointer in C). Currently, ccodegen.rs passes
them by value, causing stack overflow when functions with large struct arguments
(like Context, Expr) are called recursively.

*** Reproducing
#+BEGIN_SRC bash
./bin/til build src/examples/empty.til  # Segfault (stack overflow)
#+END_SRC

*** Root Cause
In ccodegen.rs:2754-2758, the default case (no mut/copy/own) generates:
#+BEGIN_SRC rust
} else {
...
This produces `const til_Context til_context` (by value) instead of
`const til_Context* til_context` (by pointer).

*** Fix
Requires changes in three places in ccodegen.rs:

1. *emit_func_signature* - Change signature generation:
#+BEGIN_SRC rust
} else {
    // const: pass by const pointer, read-only
    output.push_str("const ");
    output.push_str(&arg_type);
    output.push_str("* ");
}
...
2. *Call sites* - Pass address for const args:
   - Currently: ~func(arg)~
   - Should be: ~func(&arg)~

3. *Inside function bodies* - Dereference const pointer args:
   - Currently: ~til_arg.field~ or ~til_arg~
   - Should be: ~til_arg->field~ or ~(*til_arg)~

This affects emit_arg_with_param_type, emit_expr (for Identifier), and
potentially other places where arguments are emitted or used.
