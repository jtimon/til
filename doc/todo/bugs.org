...
# ERROR: gcc failed: too few arguments to function 'til_Str_clone'
#+END_SRC

*** Root Cause
In ccodegen.rs, when generating C struct literals, function calls inside field
initializers are emitted inline without checking if they use out-parameter
calling convention. Throwing functions need their return value and error
extracted to temp variables before use in the struct literal.

*** Workaround
Extract the throwing function call to a temp variable before the struct literal:
#+BEGIN_SRC til
cloned_name := original.clone()
items.push(Item(name=cloned_name, count=42))
#+END_SRC
