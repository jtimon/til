...
** Bug #27: Cannot inline Vec.new() as function argument
:PROPERTIES:
...
*** Workaround
Create a variable first, then pass the variable:
#+BEGIN_SRC til
empty_params := Vec.new(Expr)
return Expr.new_explicit(..., empty_params, ...)
#+END_SRC

This workaround is used in src/core/self/parser.til:1326-1330.

*** Impact
- Code is more verbose than Rust equivalent
- Cannot directly translate certain Rust patterns
- All inline Vec.new() calls in function arguments must use workaround
