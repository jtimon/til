...
*** Root Cause (Likely)
The bug is in src/rs/interpreter.rs. When Map.get writes back to the =val= parameter, the field access chain (frame.funcs) is not being resolved correctly, causing the out parameter to write to wrong location or be lost.

This is similar to Bug #24 (struct field access returns copy instead of reference), but specifically affects the interpreter's handling of mut parameters in method calls on struct fields.

*** Reproducing Test
src/test/bug43_test.til

*** Files to Investigate
- src/rs/interpreter.rs: Handle mut parameter write-back for struct field method calls
- src/self/interpreter.til: Same (must match after fix)
