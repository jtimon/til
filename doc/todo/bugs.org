...
- Removed now-unnecessary OutOfBounds catch blocks
...
~OutOfBounds~, or ~KeyNotFoundError~ are excluded (these depend on runtime state).
...
- Throw with format: `throw OutOfBounds.new(format(...))` ✅
