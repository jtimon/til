...
| rstil run       | pass                      |
| til interpret   | pass                      |
| til run         | timeout (>150s)           |

The rstil versions now pass. The ~til run~ hangs - either infinite loop in
til's ccodegen or extremely slow compilation.
...
*** TODO
- [ ] Investigate why ~til run bug47.til~ hangs/times out
