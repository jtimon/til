...
:STATUS: In Progress (interpreters fixed, ccodegen pending)
:END:
...
Test case: src/test/bug159.til (passes in rs_interpreted, til_interpreted)
...
Added fields to Ptr to enable deep cloning:
...
**** Step 3: DONE (interpreters only) - Replace copy_fields with clone calls
Changed both Rust and TIL interpreters' =mut x := y= to call =Type.clone(y)=
instead of =copy_fields=. Key implementation details:
- =try_call_clone= helper builds FCall AST for =Type.clone(source)=
- After clone returns, allocate fresh memory with =insert_struct_instance=
- Copy from clone result (handles types like Bool.clone that return self)
- Throw error if no clone method (should never happen, preinit generates them)

**** Step 4: TODO - Fix ccodegen
The C code generator (ccodegen.rs/til) still uses shallow copy for struct
declarations. Need to emit clone calls instead of memcpy for =mut x := y=.
Tests: rs_compiled and til_compiled expect failure until this is done.
