...
:STATUS: Resolved - no longer occurs after Bug #133 fix merged
:REPRODUCER: See tmp/test_145.til
...
After merging the Bug #133 fix to master, this bug no longer manifests.
The til_interpreted mode now passes the test case.
...
til_compiled:    SEGFAULT (needs TIL port of Bug #133)
#+END_SRC

*** Previous workaround (no longer needed for til_interpreted)
...
TIL port NOT complete - requires low-level arena memory access that TIL doesn't
have equivalent functions for. The rs_compiled mode is fixed, til_compiled still
needs the port.

**** Current test results (simple Vec<I64>):
- rs_interpreted: PASS
- rs_compiled: PASS (Bug #133 fix works)
- til_interpreted: PASS (Bug #145 resolved)
- til_compiled: SEGFAULT (needs TIL port)

**** What works now (rs_compiled only):
...
- til_compiled mode (needs TIL port)

**** Future work:
1. Port Bug #133 fix to TIL (ccodegen.til, etc.) - requires adding low-level
   arena byte access functions to TIL's EvalArena
2. Implement recursive serialization for nested Vecs
3. Add List/Map serialization support
