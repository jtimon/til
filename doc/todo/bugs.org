...
:IMPACT: Build performance, eliminates manual dependency lists
:STATUS: Design needed
:END:

*** Description
Have til translate/build/run analyze dependencies from import() calls automatically,
instead of hardcoding them in make.til like a traditional Makefile.

When a file has a shebang with ~til run~, if the binary already exists and neither
the source nor any of its dependencies changed, run the binary directly without
recompiling.

*** Design Ideas
- Parse imports statically to build dependency graph
- For non-TIL dependencies, add ~ext_dep("src/ext.c")~ declaration
- Compare mtimes: if binary newer than all deps, skip recompilation
- Similar to how Go handles imports and incremental builds

*** Open Questions
- Where to cache the dependency graph? Recompute each time vs store in file?
- For ext_dep files: track their includes too, or just the file itself?
- System libraries: probably ignore (assume stable)
...
*** Open Questions
- Warning vs error? (warnings can be ignored, errors force action)
- How to suppress for legitimate cases:
  - core.til is imported everywhere, not all exports used in every file
  - Unused struct methods (might be part of an interface/API)
  - Test helpers only used in some tests
  - Public library exports
- Syntax for suppression - don't want Rust-style #[allow(unused)] annotations
- Compare approaches from other languages (Go, Zig, etc.)
