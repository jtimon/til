...
*** Description
Two related problems in field/variable lookup:

1. Redundant arena_index entries: map_instance_fields() creates separate entries for
   every struct field (myVec, myVec.type_name, myVec._len, etc.) when only the base
   is needed since fields are contiguous and Vec<Declaration> preserves order.

2. Harmful fallback patterns: eval_arena functions try lookup_var() first, then fall
   back to get_field_offset() if that fails. This "try X, fallback Y" pattern hides
   bugs and makes code unpredictable.

*** Solution
Deterministic dispatch based on identifier structure:
- If id contains "." -> ALWAYS use get_field_offset() (it's a field path)
- If id has no "." -> ALWAYS use lookup_var() (it's a variable)

No fallbacks. The code KNOWS what operation to perform.

*** Affected Functions (eval_arena.rs/til)
- get_u8 (lines 113-127 / 128-146)
- get_i64 (lines 129-149 / 148-166)
- insert_i64_core (lines 160-168 / 176-183)
- insert_u8_core (lines 207-215 / 226-233)
- copy_fields (lines 264-276 / 291-302)
- get_enum (lines 815-823 / 976-987)

*** Files to Modify
- ~src/rs/eval_arena.rs~ - replace 6 fallback patterns with deterministic dispatch
- ~src/rs/init.rs~ - remove field insertions from map_instance_fields()
- ~src/self/eval_arena.til~ - port eval_arena.rs changes
- ~src/self/init.til~ - port init.rs changes

*** Implementation Plan
Per CLAUDE.md: run make benchmark, change Rust, port to TIL, run make benchmark again.

**** Step 1: Replace fallback patterns with deterministic dispatch
Change all 6 functions in eval_arena.rs from:
#+BEGIN_SRC rust
...
let offset = if id.contains('.') {
    ...
    ctx.scope_stack.lookup_var(id).ok_or_else(|| ...)?
};
#+END_SRC

Then port to eval_arena.til.

**** Step 2: Remove field insertions from map_instance_fields()
In init.rs, stop inserting individual field entries. Only insert base struct name.
Then port to init.til.

**** Step 3: Remove field mappings from insert_struct_core()
In eval_arena.rs, stop appending field mappings. Only return base offset.
Then port to eval_arena.til.

**** Step 4: Test
Run make benchmark to verify all tests pass.
...
**** Step 3: TODO - Replace copy_fields with clone calls
Change interpreter's =mut x := y= to call =Type.clone(y)= instead of =copy_fields=.
Previous attempt caused stack overflows - should work after Step 2.
...
:PROPERTIES:
:DISCOVERED: 2026-01-31
:IMPACT: Benchmark numbers for compiled modes may be inaccurate
:STATUS: Open
:END:

*** Description
The benchmark shows Rust compiled improving (-7%) while TIL compiled gets slower (+60%)
after Issue #110 switch desugaring. This doesn't make sense since both should generate
similar C code and run similar binaries.

Suspicion: The profiling in test_runner.til may be counting compilation time in some
cases where it should only count runtime after compilation.

*** Investigation needed
- Review test_runner.til timing logic for compiled mode
- Verify timing only captures execution, not compilation
- Compare what rstil vs til_interpreted actually measure
