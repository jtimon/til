...
The ~next_mangled()~ function (ccodegen.rs:74-81) uses ~ctx.current_function_name~:
- When ~Some(func_name)~: generates ~_tmp_funcname_N~
- When ~None~: generates ~_tmp_N~

**** The actual bug location

~emit_struct_func_body~ (ccodegen.rs:2527-2597) does NOT set ~current_function_name~
before emitting the function body. Compare with regular functions at line 2835
which do:

#+BEGIN_SRC rust
// Regular functions (line 2833-2836):
let prev_function_name = ctx.current_function_name.clone();
let prev_mangling_counter = ctx.mangling_counter;
ctx.current_function_name = Some(decl.name.clone());  // <-- THIS LINE
ctx.mangling_counter = 0;

// Struct methods (emit_struct_func_body) - MISSING the above pattern
#+END_SRC

Result: struct methods like ~Array.new~, ~Array.get~, ~Vec.clone~ generate
temp names like ~_tmp_0~, ~_tmp_1~ instead of ~_tmp_Array_new_0~.

*** The Fix

Add to ~emit_struct_func_body~ before calling ~emit_stmts~:

#+BEGIN_SRC rust
// Save and set current function name for deterministic temp naming
let prev_function_name = ctx.current_function_name.clone();
let prev_mangling_counter = ctx.mangling_counter;
...
Tests pass despite the bug because:
- Temp variable names are still unique within each function (counter resets per function)
- The original gcc error may have been from a different issue now fixed

The bug is cosmetic but should be fixed for consistency.
...
*** Files to Modify
- src/rs/ccodegen.rs: Add current_function_name handling to emit_struct_func_body
