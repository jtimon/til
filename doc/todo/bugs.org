...
- Error handling -> Bug #98 (ext_proc error types)
...
*** Reproducer
Run any of:
- ~rstil interpret src/test/bug139.til~
- ~rstil run src/test/bug139.til~
- ~rstil_til interpret src/test/bug139.til~
- ~rstil_til run src/test/bug139.til~

Currently exits 0 (no error). Once fixed, should exit 1 with type error.

*** When Fixed
In ~src/tests.til~, add ~expected_status=1~ to the all_common entry for bug139.til.
...
** Bug #112: Add HashMap type with O(1) lookup
:PROPERTIES:
...
**** Step 1: Create HashMap alongside Map - DONE
- [X] Keep existing ~Map~ unchanged (ordered, O(log n) binary search lookup)
- [X] Created ~HashMap~ type in ~src/std/hashmap.til~
- [X] Uses ~Str.hash()~ (FNV-1a) for string keys, FNV-1a on raw bytes for others
- [X] Added 13 HashMap tests to ~src/test/maps.til~

**** Step 2: Test thoroughly before using elsewhere - DONE
- [X] All 95 tests pass in all 4 modes (rs_interpreted, rs_compiled, til_interpreted, til_compiled)
- [X] HashMap tests cover: insert, set, get, contains_key, clear, growth/resize, multiple overwrites
- [X] Stress test with 50 string keys verifies hash distribution

**** Step 3: Use HashMap where Rust uses it - TODO
- [ ] Replace ~Map~ with ~HashMap~ in TIL self-hosted code where Rust uses HashMap
- [ ] Key locations: ~ScopeFrame~ (symbols, funcs, enums, arena_index lookups)
- [ ] Keep ~Map~ where Rust uses OrderedMap (sorted order matters)
...
a primary reason the TIL self-hosted interpreter is ~2.7x slower than rstil.
...
*** Previous Attempt (branch wip_112)
A previous attempt to replace Map with HashMap was blocked by Bug #113 (self-hosted
interpreter failed with "symbol 'msg' not found"). The new plan avoids this by
keeping both types separate - we can test HashMap independently without affecting
the existing Map implementation.

*** Implementation Notes
- Use existing ~Str.hash()~ function (FNV-1a already implemented)
- Bucket count: power of 2 (allows bitwise AND instead of modulo)
- Collision handling: separate chaining (linked list via index arrays)
- Load factor: resize when > 0.75 full
- Same API as Map (get, set, insert, contains_key, etc.)
