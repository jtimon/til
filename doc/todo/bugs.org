...
3. *Forward declarations position differs*
...
*Analysis*:
The ~next_mangled~ function looks correct:
#+BEGIN_SRC til
...
The Rust version is essentially identical but n.to_str() correctly returns "0", "1", etc.

*Suspected cause*: Either ~n.to_str()~ is returning ~"_tmp_"~ due to some type confusion,
or the counter isn't being properly saved/restored. Key difference: Rust uses
~ctx.mangling_counter = 0~ to reset counter per function, TIL might be missing this reset.

*Fix steps*:
1. Verify ~emit_func_declaration~ resets ~ctx.mangling_counter = 0~ after saving old value
2. Verify ~n.to_str()~ in TIL ccodegen returns correct integer string
3. Add debug println to trace what n.to_str() actually returns
4. Check if there's variable shadowing causing ~n~ to get wrong value
...
***** Issue 9: Pointer dereference - ~til_b.data~ should be ~(*til_b).data~

*Symptom*: Field access on pointer params uses ~.~ instead of ~->~ or ~(*ptr).~

*Location*: ~emit_identifier~ or ~emit_expr~ when handling field access

*Rust reference*: Look for how field access on mutable (pointer) params is emitted

*Analysis*:
When accessing a field on a mutable parameter (which is passed as pointer),
the C code needs explicit dereference: ~(*ptr).field~ or ~ptr->field~.
TIL ccodegen is treating the pointer like a value type.

*Fix steps*:
1. In ~emit_expr~ field access case, check if base variable is in ~current_ref_params~
2. If it's a ref param, emit ~(*var).field~ instead of ~var.field~
3. Rust uses ~ctx.current_ref_params.contains(&name)~ to check

***** Issue 10: Literal hoisting - Type and I64 literals need temp variables

*Symptom*: Can't take address of string/integer literal for pointer params

*Location*: ~hoist_variadic_args~ and ~emit_variadic_call~

*Rust reference*: ~src/rs/ccodegen.rs~ lines 1352-1371 (Bug #60 fix)
...
*Fix steps*:
1. Find ~hoist_variadic_args~ in ccodegen.til
2. Add hoisting for Type literal (elem_type string)
3. Add hoisting for I64 literal (count)
4. Emit temp variable declarations before Array.new call
5. Pass ~&type_temp~ and ~&count_temp~ instead of inline literals

***** Issue 11: Dynamic casting - need ~(til_Dynamic*)&val~

*Symptom*: Array.get/set calls missing Dynamic pointer cast

*Location*: ~emit_variadic_call~ or wherever Array.get/set is emitted

*Rust reference*: Look for ~(til_Dynamic*)~ casts in ccodegen.rs

*Analysis*:
When passing values to Array.set which expects ~til_Dynamic*~, we need:
- ~(til_Dynamic*)&value~ for the value argument
TIL ccodegen just passes ~value~ without cast or address-of.

*Fix steps*:
1. In hoist_variadic_args or emit_variadic_call, find Array.set emission
2. Add ~(til_Dynamic*)&~ prefix when passing value to Dynamic* param
3. Similarly for Array.get output parameter

***** Issue 3: Forward declarations position
...
*Fix steps*:
1. Read ~emit~ function in both ccodegen.rs and ccodegen.til
2. Identify pass order difference
3. Reorder passes in TIL to match Rust
4. Key: Forward decls (emit_func_prototype) must come before ext.c include
...
| Function | Rust | TIL | Notes |
|----------|------|-----|-------|
| next_mangled | Y | Y | |
| til_name | Y | Y | |
| emit_struct_literal_assign | Y | Y | |
| emit_struct_literal_start | Y | Y | |
| til_func_name | Y | Y | |
| get_func_name_string | Y | Y | |
| get_til_func_name_string | Y | Y | |
| get_type_arg_name | Y | Y | |
| get_fcall_func_def | Y | Y | |
| param_needs_by_ref | Y | Y | |
| til_type_to_c | Y | Y | |
| value_type_to_c_name | Y | Y | |
| has_str | - | Y | TIL only |
| get_field_type_dependency | Y | - | Rust only |

***** Type/Struct/Enum Detection - Present
| Function | Rust | TIL |
|----------|------|-----|
| is_struct_declaration | Y | Y |
| is_enum_declaration | Y | Y |
| is_enum_with_payloads | Y | Y |
| is_constant_declaration | Y | Y |
| is_global_declaration | Y | Y |
| is_throwing_fcall | Y | Y |
| is_func_declaration | Y | Y |
| enum_has_payloads | Y | Y |
| variant_has_payload | - | Y |
| get_struct_name | Y | Y |
| get_struct_dependencies | Y | Y |
| get_enum_name | Y | Y |
| get_enum_dependencies | Y | Y |
| get_type_name | Y | Y |
| get_type_dependencies | Y | Y |
| get_fcall_func_name | Y | Y |
| get_struct_construction_type | Y | Y |
| get_enum_construction_type | Y | Y |
| get_case_variant_info | Y | Y |
| parse_pattern_variant_name | Y | Y |

***** Topological Sort - Present
| Function | Rust | TIL |
|----------|------|-----|
| topological_sort_types | Y | Y |
| topological_sort_structs | - | Y |

***** Emit Functions - All Present
| Function | Rust | TIL |
|----------|------|-----|
| emit | Y (pub) | Y |
| emit_struct_declaration | Y | Y |
| emit_struct_constants | Y | Y |
| emit_struct_func_prototypes | Y | Y |
| emit_struct_func_body | Y | - |
| emit_struct_func_bodies | Y | Y |
| emit_enum_declaration | Y | Y |
| emit_enum_with_payloads | Y | Y |
| emit_enum_to_str_function | Y | Y |
| emit_enum_to_str_for_declaration | Y | Y |
| emit_enum_size_of_constant | Y | Y |
| emit_size_of_function | Y | Y |
| emit_func_signature | Y | Y |
| emit_func_prototype | Y | Y |
| emit_func_declaration | Y | Y |
| emit_constant_declaration | Y | Y |
| emit_global_declaration | Y | Y |
| emit_expr | Y | Y |
| emit_body | Y | Y |
| emit_stmts | Y | Y |
| emit_if | Y | Y |
...
| emit_switch | Y | Y |
| emit_fcall | Y | Y |
| emit_declaration | Y | Y |
| emit_funcdef | Y | Y |
| emit_assignment | Y | Y |
| emit_return | Y | Y |
| emit_throw | Y | Y |
| emit_literal | Y | Y |
| emit_str_literal | Y | Y |
| emit_identifier_without_nested | - | Y |
| prescan_func_level_catches | - | Y |

***** Hoisting/Throwing Functions - Present
| Function | Rust | TIL |
|----------|------|-----|
| check_throwing_fcall | Y | Y |
| hoist_throwing_expr | Y | Y |
| hoist_throwing_args | Y | Y |
| hoist_for_ref_params | Y | Y |
| hoist_for_dynamic_params | Y | Y |
| hoist_variadic_args | Y | Y |
| detect_variadic_fcall | Y | Y |
| emit_variadic_call | Y | Y |
| emit_throwing_call | Y | Y |
| emit_throwing_call_propagate | Y | Y |
| emit_throwing_call_with_goto | Y | Y |
| emit_fcall_with_hoisted | Y | Y |
| emit_fcall_name_and_args_for_throwing | Y | Y |
| emit_arg_or_hoisted | Y | Y |
| emit_arg_with_param_type | Y | Y |
| emit_arg_with_mut | - | Y |

***** Collection Functions - Present
| Function | Rust | TIL |
|----------|------|-----|
| collect_func_info | Y | Y |
| collect_nested_func_info | Y | Y |
| collect_declarations_in_body | Y | Y |
| collect_declarations_recursive | Y | Y |
