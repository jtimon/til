...
:IMPACT: Compile failure when using clones inside struct literal initializers
:STATUS: Open
:END:

*** Description
When a function call (like `clone_value_type()` or `Str.clone()`) appears inside
a struct literal initializer, the C codegen generates incorrect code that doesn't
use the proper out-parameter calling convention.

*** Example
#+BEGIN_SRC til
// This fails:
SymbolInfo(value_type=clone_value_type(decl.value_type), is_mut=is_mut)

// Workaround - extract to temp variable first:
vt := clone_value_type(decl.value_type)
SymbolInfo(value_type=vt, is_mut=is_mut)
#+END_SRC

*** Generated C (wrong)
#+BEGIN_SRC c
// Generated (wrong - 1 arg instead of 3):
til_clone_value_type(til_decl.value_type)

// Should be:
til_ValueType _tmp;
til_AllocError _err;
til_clone_value_type(&_tmp, &_err, til_decl.value_type);
#+END_SRC

*** Root Cause
In src/rs/ccodegen.rs, when emitting struct literal field values, function calls
are being emitted as direct expressions instead of being hoisted to temp variables
with proper out-parameter handling.

*** Reproducing
#+BEGIN_SRC bash
./bin/rstil run src/til.til help
# Fails with: error: too few arguments to function 'til_clone_value_type'
#+END_SRC
...
*** Related
- Bug #50: Similar pattern - both require per-call analysis instead of once-at-definition
- #51 is about per-call comptime-ness (precomp phase)
- #50 is about per-call variable capture (codegen phase)
...
*** Related
- Bug #51: Similar pattern - both require per-call analysis instead of once-at-definition
- #50 is about per-call variable capture (codegen phase)
- #51 is about per-call comptime-ness (precomp phase)
...
:STATUS: In Progress - Shallow clone issue, infinite loop in precomp phase
:END:
...
Found a minimal reproducer in =src/test/bug47_test.til=:
...
Reproduce:
#+BEGIN_SRC bash
./bin/rstil run src/test/bug47_test.til  # FAILS with NodeType.?
#+END_SRC
...
*** Root Cause
TIL clone methods were doing shallow copies (copying Vec struct bytes, sharing
the underlying ptr), while Rust's =#[derive(Clone)]= does deep copies. When a
Vec was cloned and then one copy deleted, the other had a dangling pointer.

Specifically in typer.til: =id_params= was cloned, passed to =Expr.new_clone()=,
then =id_params.delete()= was called - freeing memory still referenced by the Expr.

*** The Fix
Updated all clone methods in parser.til to do deep copies matching Rust semantics:
- Added =clone_literal()=, =clone_node_type()=, =clone_value_type()= functions
- Fixed =Expr.clone()= to deep clone params and node_type
- Fixed =SFuncDef.clone()=, =SStructDef.clone()=, =SEnumDef.clone()=,
  =Declaration.clone()= to iterate and clone each element
- Added =PatternInfo.clone()= method
- Removed =id_params.delete()= calls in typer.til that triggered the use-after-free
- Added AllocError catches in precomp.til for new throwing clones

*** Files Modified
- src/self/parser.til: Deep clone implementations for all structs and enums
- src/self/typer.til: Removed id_params.delete() calls
- src/self/precomp.til: Added AllocError catches

*** Update (2025-12-26): NOT FIXED - Infinite Loop in Precomp Phase

The bug47_test.til still times out when run in compiled mode. Debug output showed
the infinite loop occurs in the *precomp phase*, specifically in =precomp_expr= or
=precomp_fcall=. Last output before hang:

#+BEGIN_EXAMPLE
precomp #356 NodeType.Return L59
precomp #357 NodeType.LLiteral L59
precomp #358 NodeType.Declaration L63
precomp #359 NodeType.FCall L63
precomp #360 NodeType.Identifier L63
precomp #361 NodeType.NamedArg L63
precomp #362 NodeType.LLiteral L63
#+END_EXAMPLE

*** Systematic Clone Analysis (2025-12-26)

Root cause: TIL's Vec shallow copy (copies ptr, not data) differs from Rust's
deep clone. Every =.clone()= in Rust must have corresponding =.clone()= in TIL.

Clone counts per file (Rust vs TIL):
| File          | Rust | TIL | Status     |
|---------------+------+-----+------------|
| eval_arena    |   33 |  33 | DONE       |
| lexer         |    4 |   0 |  4 missing |
| parser        |   56 |  25 | 31 missing |
| typer         |   71 |   9 | 62 missing |
| interpreter   |  121 |  69 | 52 missing |
| precomp       |   78 |  30 | 48 missing |
| init          |   51 |  29 | 22 missing |
| scavenger     |   21 |   3 | 18 missing |
| TOTAL         |  435 | 198 | ~237 missing |

eval_arena.til clones have been matched 1:1 with Rust. Remaining files need
the same systematic treatment before bug can be verified as fixed.
...
- src/tests.til: Added bug48_test.til with expected_status=1

*** Regression Test
src/test/bug48_test.til (expects exit code 1 - type error)
