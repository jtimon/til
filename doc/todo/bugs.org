...
test_it := proc() {
    ...
    mut val := 0
    v.get(0, val)
    // This line fails with "symbol 'msg' not found":
...
*** Observations
- Works with rstil (Rust interpreter)
- Fails with til (self-hosted interpreter)
- Issue is NOT specific to Map - happens with Vec.get too
- The error mentions 'msg' which is the parameter name in test() function
- Error thrown from ~generate_struct_mappings~ in ~src/self/eval_arena.til~

*** Investigation Notes
The error occurs when calling ~test(loc(), val.eq(42), "msg")~ after using val
as a mut parameter to Vec.get/Map.get. Somewhere in the interpreter, when
setting up the function call frame for test(), the symbol lookup for the 'msg'
parameter fails. This suggests a divergence between how the Rust and TIL
interpreters handle mut parameters or function call frames.
