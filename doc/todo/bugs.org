...
should emit an error. Currently it silently accepts the unnecessary ~?~.

*** Example
#+BEGIN_SRC til
// Vec.new doesn't throw, but this compiles without error:
ModeDef := struct {
    mut imports: Vec = Vec.new(Str)?  // Should error: Vec.new doesn't throw
}
#+END_SRC

*** Expected Behavior
The typer should emit an error like: "Error: ? operator used on non-throwing call"
...
:STATUS: Partial fix (counter reset done, file order indeterminism remains)
:REPRODUCER: Run `make clean && make benchmark` twice, diff bootstrap/til.c
:END:
...
// Build 1:
-    til_I64 _loop_3 = 0;
-    while (til_I64_lt(&_loop_3, til_size).data) {
+// Build 2:
+    til_I64 _loop_1 = 0;
+    while (til_I64_lt(&_loop_1, til_size).data) {
#+END_SRC

*** Root Cause
Introduced in commit 511ac894 (Dec 14, 2025) - "Add struct/enum constructor type inference".
...
Unlike ~precomp_forin_counter~ (fixed in Issue #127), this counter is NEVER
reset - not per-file, not per-function.

#+BEGIN_SRC rust
// src/rs/parser.rs lines 9-14
...
The counter value when parsing a file depends on which other files have already
been parsed. Only 2 files have ~for _ in~ loops:
- ~src/core/vec.til:220~
- ~src/self/interpreter.til:37~

**** Secondary issue: Non-deterministic file processing order
The file processing order itself appears non-deterministic between builds.
This may be due to HashMap iteration order (Rust HashMaps use randomized
hashing by default). Even if the counter is reset per-file, different file
orders would still produce different counter values.

*** Fix Applied

**** Done: Reset counter per-function (like Issue #127)
Reset ~LOOP_VAR_COUNTER~ at the start of each function definition, similar to
how Issue #127 fixed ~precomp_forin_counter~. This ensures deterministic
~_loop_N~ names within each function.

Files modified:
- ~src/rs/parser.rs~: Added ~save_loop_var_counter~, ~reset_loop_var_counter~,
  ~restore_loop_var_counter~ functions; wrap body parsing in ~parse_func_proc_definition~
- ~src/self/parser.til~: Ported the same fix

*** Remaining Issue

**** TODO: Fix file processing order (addresses root cause)
The file processing order itself is still non-deterministic between builds,
likely due to Rust HashMap iteration order (randomized hashing by default).

Possible solutions:
- Replace HashMap with Vec where iteration order matters
- Use sorted iteration over HashMap keys
- Implement an OrderedMap in Rust similar to TIL's Map (which uses Vec for keys)

This would provide complete determinism and is worth fixing separately.

*** Impact
- Makes git diffs noisy when regenerating bootstrap/til.c
- Harder to verify that code changes are intentional vs artifacts

** Bug #145: til_interpreted diverges from rs_interpreted for func in global init
:PROPERTIES:
:DISCOVERED: 2026-01-27
:IMPACT: til_interpreted crashes when func returns Vec used in global initialization
:STATUS: Open - regression on branch current133, not present on master
:REPRODUCER: See src/test/test_bug133.til (run on branch current133)
...
fails with "Ptr.new_by_size: invalid size" while rs_interpreted works correctly.

NOTE: This bug is a regression introduced by changes on branch ~current133~.
On master, til_interpreted works fine for this case. The bug only manifests
after applying the Bug #133 fix.
...
rs_interpreted:  PASS
rs_compiled:     SEGFAULT (Bug #133)
til_interpreted: PASS
til_compiled:    SEGFAULT (Bug #133)
#+END_SRC

*** Test Results on branch current133
#+BEGIN_SRC
rs_interpreted:  PASS
rs_compiled:     PASS (Bug #133 fix works)
til_interpreted: FAIL ("Ptr.new_by_size: invalid size") <-- Bug #145
til_compiled:    PASS (Bug #133 fix works)
#+END_SRC

*** Root Cause
Unknown. The Bug #133 fix changed something in how precomputation or initialization
works. The TIL interpreter on branch current133 handles func return values differently
during module initialization than the Rust interpreter.

*** Workaround
Use ~proc~ instead of ~func~ for functions that return Vec and are called during
global initialization:

#+BEGIN_SRC til
// FAILS in til_interpreted:
create_data := func() returns Vec { ... }
data := create_data()  // Global init

// WORKS:
create_data := proc() returns Vec { ... }
data := create_data()  // Global init
#+END_SRC

*** Affected Files
- src/examples/music.til (create_chromatic, create_notes_solfege, create_base_notes)
...
:STATUS: Rust FIXED (2026-01-29), TIL port pending
:RELATED: Issue #110 (switch desugaring)
:END:

*** Description
The ccodegen hoisted_exprs map tracks which expressions have been "hoisted" (extracted
to separate statements with temp variables) so that emit_expr can use the temp var
instead of re-emitting the expression.

The identity mechanism differs between Rust and TIL:
- Rust uses memory address: ~expr as *const Expr as usize~
- TIL uses line*1000000+col as workaround: ~expr.line.mul(1000000).add(expr.col)~
...
The TIL workaround exists because TIL copies Vec elements into local variables
(~vec.get(i, expr)?~), while Rust iterates by reference. Same logical expression
accessed in hoisting vs emission has different local variable addresses in TIL.

*** Why hoisted_exprs exists
Hoisting happens in a pre-pass, but the same expression may be encountered later
in emit_expr through a different code path. The lookup asks "is this the exact
same expression I hoisted earlier?" This is needed for:
1. Deeply nested expressions hoisted at outer level
2. Expressions in emit_expr that were hoisted by hoist_throwing_args

*** Solution Roadmap

**** Solution B: Pass hoisted info explicitly (medium-term)
Restructure so hoisted info flows through all code paths explicitly, eliminating
the need to "rediscover" what was hoisted.

Pros:
- No fragile identity mechanism
- Clearer data flow

Cons:
- Larger refactor
- Must update all emit_* functions

**** Solution C: Single-pass hoist+emit (long-term, preferred)
Combine hoisting and emission into a single pass. When we encounter an expression
that needs hoisting, we hoist and emit in one step - no need to look up later.

Pros:
- Eliminates hoisted_exprs entirely
- Simplest conceptual model
- No identity tracking needed

Cons:
- Significant refactor of ccodegen architecture
- Must ensure Rust and TIL stay in sync during refactor

*** Current Symptoms
- 2/94 tests fail in til_compiled mode after switch desugaring (Issue #110)
- Segfaults from dangling pointers when hoisting fails to match

*** Implementation Attempt: Solution C (2026-01-28)
Attempted to implement single-pass hoist+emit via ~emit_arg_string~ function.

**** What was added
- New functions in ~src/rs/ccodegen.rs~ (lines ~1961-2589):
  - ~emit_arg_string~: Process arg for FCall, return string (hoisting if needed)
  - ~emit_throwing_arg_string~: Handle throwing calls
  - ~emit_variadic_arg_string~: Handle variadic calls
  - ~emit_fcall_arg_string~: Recursively process nested FCall args
  - Helper functions for Dynamic/ref params

**** Why it failed to integrate
1. **Dynamic/ref param hoisting requires expression type determination**
   - Must declare temp var with correct C type
   - Type determination logic is complex (~hoist_for_dynamic_params~ lines 1082-1170)
   - New code was incomplete, causing "Cannot determine type" errors

2. **Arg string approach conflicts with emit-as-you-go pattern**
   - Current code: emit function name, then emit args inline
   - New approach: collect arg strings first, then emit call
   - Hoisting must happen BEFORE function name is emitted
   - Required restructuring the entire emit_fcall flow

3. **Struct constructors with NamedArgs need special handling**
   - NamedArgs are only valid in struct constructors
   - ~emit_arg_string~ must detect and delegate to ~emit_expr~ for these

4. **Mixing old and new mechanisms caused incorrect code generation**
   - Attempt to use new mechanism for throwing calls + old mechanism for Dynamic/ref
   - Led to incorrect C code (type mismatches, missing & operators)

**** Rust Fix Completed (2026-01-29)
Solution C (single-pass hoist+emit) was successfully implemented in Rust.
...
1. **Converted all call sites to use ~emit_arg_string~**
   ...
   - ~emit_variadic_call~, ~emit_throwing_call~, ~emit_throwing_call_propagate~, etc.
...
3. **Removed ~hoisted_exprs~ HashMap entirely**
   ...
   - No more fragile memory-address-based identity tracking

**Result:** ~-1400 lines of code removed, simpler architecture, no identity tracking needed.

**** TIL Porting Plan

**Step 1: Generate diff file**
#+BEGIN_SRC bash
git diff b8fab027..HEAD -- src/rs/ccodegen.rs > tmp/bug143_rust_diff.txt
#+END_SRC

**Step 2: Port changes following ~doc/rs2til.org~ strictly**
- Go through ~tmp/bug143_rust_diff.txt~ section by section
- Apply equivalent changes to ~src/self/ccodegen.til~
- Follow rs2til.org translation rules exactly

**Step 3: Sync naming and comments**
- Variable names must match between Rust and TIL
- Comments must be identical (copy verbatim)
- TIL was using ~expr.line.mul(1000000).add(expr.col)~ workaround instead of addresses,
  but after the port both versions should look similar since hoisted_exprs is removed

**Step 4: Handle TIL-specific issues, port back to Rust**
- **Shadowing:** If TIL needs variable renames to avoid shadowing, port those
  renames back to Rust so both stay in sync
- **Tuples:** If Rust uses tuples that TIL can't handle, create a struct in both
  Rust and TIL (TIL doesn't have tuple syntax)
- Goal: After porting, both files should differ only in syntax, not in structure
...
**Step 6: Move bug to fixed.org**
Once TIL port is complete and all tests pass, move Bug #143 to ~doc/todo/fixed.org~
...
:STATUS: Open
:RELATED: Bug #99 Step 5a
:END:

*** Description
The C codegen generates undefined behavior when passing rvalue results by-reference.
GCC's ~-Wdangling-pointer~ warning (971 occurrences) reveals the problem:
#+BEGIN_SRC c
til_foo(({ til_Str _tmp = til_bar(); &_tmp; }))
#+END_SRC

The ~_tmp~ variable is local to the statement expression, so ~&_tmp~ is a dangling
pointer once the expression ends. GCC warns about this with ~-Wdangling-pointer~.

*** Why It Works (By Luck)
The temp is on the stack and the outer function call happens immediately, before the
stack location is overwritten. This is undefined behavior that happens to work because:
- No optimization reorders the operations
- Stack isn't clobbered between statement expression end and function call
- Compiler doesn't exploit UB for optimization

This could break with different optimization levels, compilers, or future gcc versions.

*** Affected Patterns (ccodegen.rs lines)
- Enum constructors needing address-of (line 1812)
- UFCS placeholder "_" chained expressions (line 1835)
- FCall results for struct types (line 1890)
- Custom type constructors (line 1913)

*** Fix Approach
Hoist temp variables to statement level instead of using statement expressions:
#+BEGIN_SRC c
// Before (dangling):
til_foo(({ til_Str _tmp = til_bar(); &_tmp; }))

// After (safe):
til_Str _tmp_42 = til_bar();
til_foo(&_tmp_42);
#+END_SRC

This requires restructuring ~emit_fcall_arg~ to:
1. Pre-scan arguments to identify which need temp variables
2. Emit temp declarations before the call
3. Reference the temps in the call arguments

*** Complexity Notes
- Affects core function call emission logic
- Must be ported to both ccodegen.rs and ccodegen.til
- May interact with existing ~ctx.hoisted_exprs~ mechanism
...
*** Description
The type checker allows assigning an I64 value to a Ptr field without error.
This was discovered when fixing Bug #99 Step 5f (-Wmissing-braces).

*** Example
~NULL := 0~ is defined as I64, but ~Declaration.default_value~ is Ptr:
#+BEGIN_SRC til
// This should be a type error but isn't:
index_decl := Declaration(..., default_value=NULL)

// Correct code:
index_decl := Declaration(..., default_value=Ptr())
#+END_SRC

*** Fix Reference
See commit 5031b866 which fixed the symptom by using Ptr() instead of NULL.
...
:STATUS: Partial - Steps 1-4 done for simple types; Vec<Vec<T>>, List, Map still broken
:REPRODUCER: src/test/test_bug133.til (segfaults in compiled modes on master)
:RELATED: Bug #145 (similar issue in til_interpreted mode)
:BRANCH: current133
:END:
...
#+BEGIN_SRC til
// Works (Vec<Str> - supported):
create_chromatic := func() returns Vec { ... }

// Broken (Vec<Vec<I64>> - not yet supported), use proc:
...
*** Progress (2026-01-27)
Steps 1-4 implemented for simple Vec types on branch ~current133~. Files modified:
...
- Ported to TIL: eval_arena.til, init.til, precomp.til, ccodegen.til
...
**** Still broken:
- ~Vec<Vec<T>>~ - inner Vecs contain invalid pointers (falls back to raw bytes)
- ~List~ - linked node pointers not serialized
- ~Map~ - internal structure not serialized

**** Blockers for merging branch ~current133~:
1. **Bug #145**: Even with the fix, til_interpreted mode crashes when func returns
   Vec in global init. music.til workarounds must stay as proc, defeating the
   benefit of the fix for that file.
2. **Vec<Vec<T>> not supported**: music.til's ~mode_intervals~ (Vec<Vec<I64>>) would
   still crash in compiled mode. Must either:
   - Implement recursive serialization for nested Vecs, OR
   - Keep ~create_mode_intervals~ as proc (current workaround)
3. **Partial benefit**: The fix only helps compiled modes (rs_compiled, til_compiled).
   til_interpreted still needs proc workaround due to Bug #145.
...
- [ ] Step 5a: Remove -Wno-dangling-pointer (971 occurrences) - see Bug #140
...
*** Progress (2026-01-25)
Most warning issues now consolidated into Bug #99 (gcc warnings) and Bug #140 (dangling pointers).
...
- [ ] Fix dangling pointer pattern - see Bug #140
