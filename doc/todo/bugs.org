...
:STATUS: In Progress - Codegen bug, examining generated C patterns
:END:
...
This happens in init.til's =infer_identifier_type= function when iterating over
...
*** Minimal Reproducing Test (2025-12-19)
src/test/bug47_test.til reproduces the corruption:
...
DEBUG C local_count=0          <- CORRUPTED when entering for-in loop
...
*** Generated C (gen/c/test/bug47_test.c)
- Global: =static til_I64 til_DEBUG_CALL_COUNT;= (line 174)
- Loop iterator =til_cursor= declared outside while loop (line 925)
- Vec_get copies via =til_memcpy(dest, src, type_size)=

*** Reproducing (self-hosted til)
#+BEGIN_SRC bash
...
*** Fix Required
Mirror what interpreter already does: remove the symbol from scope when passed to =own= param.

*Implementation plan:*

1. In typer's function call checking (where we iterate over args)
2. If param has =is_own=, and arg is an identifier:
   - Remove the symbol from scope (like interpreter does)
3. Subsequent uses automatically get "undefined symbol" error

No new data structures needed - reuses existing scope/symbol machinery.

*** Call-site own syntax (optional/aesthetic)
Call-site =own= syntax (=func(own x)=) is not implemented in parser.
Currently only declaration syntax (=own n: I64=) is supported.
This would be purely for code clarity - making ownership transfer explicit to readers.
The mechanism works without it.

*** Reproducing Test
src/test/bug49_test.til

#+BEGIN_SRC bash
./bin/rstil interpret src/test/bug49_test.til  # FAIL - eval error (should be typer)
./bin/rstil run src/test/bug49_test.til        # PASS - bug: x still accessible
#+END_SRC
