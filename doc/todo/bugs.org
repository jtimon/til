...
:STATUS: Part 1 done, Part 2a+2b+2c+2d done (Vec+Array fully cleaned), List remaining
...
Affected containers:
- ~Vec~ (vec.til): DONE (both removed, type info via Ptr.elem_type/elem_size)
- ~Array~ (array.til:11-12): ~type_name: Str~, ~type_size: I64~ -- DONE (both removed, ptr migrated I64->Ptr)
...
Array was a special case -- it used ~mut ptr: I64~ (raw integer) instead of
~Ptr~. Migrated to ~Ptr~ in Part 2b.
...
Vec.type_size has now been removed (Part 2a). Map works because Vec constructors
now set ptr.elem_size on a local Ptr before assigning to vec.ptr, so elem_size
is available from the start -- option (c) from the original plan.
...
**** Part 2a: Vec.type_size removal (DONE)
Removed ~type_size: I64~ from Vec struct. All code now uses ~self.ptr.elem_size~.
Vec constructors (~new~, ~new_from_type_info~, ~with_capacity~) set elem_size
on a local Ptr then assign it to ~vec.ptr~ (avoids chained-assignment interpreter
bug where ~vec.ptr.elem_size = X~ writes to a copy).

Updated ~clone~ and ~split_off~ to copy elem_size to the new Vec's Ptr.
Updated codegen (~ccodegen.rs~ + ~ccodegen.til~): precomputed Vec static
initializers set elem_size in Ptr (5th field) instead of emitting ~.type_size~.
Updated ~extract_vec_contents~ (~eval_heap.rs~ + ~eval_heap.til~): ptr offset
shifted from ~str_size + 8~ to ~str_size~, elem_size read from Ptr at offset 32.

Key bug found and fixed: ~Vec.delete()~ had ~self.ptr = Ptr()~ which zeroed
the entire Ptr including elem_size. ~Set.clear()~ calls ~Set.delete()~ which
calls ~Vec.delete()~, so after clearing a Set, elem_size was lost. Fix: removed
~self.ptr = Ptr()~ from ~Vec.delete()~ -- after ~self.ptr.delete()~, data is
already NULL; metadata (elem_size) is preserved naturally.

**** Part 2 Arena: Migrate from Vec to raw Ptr (DONE)
Arena is a simple bump allocator that only stores bytes. Replaced Vec with
Ptr + manual len/cap/growth, eliminating type_size overhead entirely.
Committed at 8d28723c. Future: Arena's type is always U8, so it could
potentially use raw memory operations (malloc/realloc/memcpy) directly
instead of going through Ptr abstractions.

**** Part 2b: Array type_size removal + ptr I64->Ptr migration (DONE)
Removed ~type_size: I64~ from Array struct and migrated ~ptr~ from raw ~I64~
to ~Ptr~ struct. All methods now use ~self.ptr.elem_size~ and Ptr operations
(offset, copy_from_dynamic, delete) instead of raw pointer arithmetic.

~Array.new~ and ~clone~ use ~malloc~ directly (not ~Ptr.new_by_size~) to avoid
circular dependency: ~panic~ uses variadic args -> ~Array.new~ -> ~Ptr.new_by_size~
-> ~panic~ (infinite recursion).

Updated ~insert_array_into_frame~ in eval_heap (Rust + TIL) to write full Ptr
struct (5 fields at known offsets) instead of a single I64.
Updated ~src/ext.c~ (~args->ptr~ -> ~args->ptr.data~).

**** Part 2c: Array type_name removal (DONE)
Removed ~type_name: Str~ from Array struct. Field was write-only -- set in
~new()~ and copied in ~clone()~, but never read. For-in loops get element type
from compile-time annotations (desugarer), not runtime type_name. Unlike Vec,
whose type_name is read by Set for Str-vs-memcmp dispatch, Array has no
runtime consumers. Removed type_name writing from ~insert_array_into_frame~
in eval_heap (Rust + TIL). Array is now just ~{ptr: Ptr, _len: I64}~ (48 bytes,
down from 80 bytes originally).

**** Part 2d: Vec type_name removal (DONE)
Removed ~type_name: Str~ from Vec struct (saves 56 bytes, Vec now just 56 bytes:
~{ptr: Ptr, _len: I64, cap: I64}~). Unlike Array (where type_name was write-only),
Vec's type_name was read at runtime by ~Vec.contains~, ~Vec.to_str~, ~Set~, ~Map~,
and ~HashMap~ for Str-specific dispatch (~Str.eq(self.type_name, "Str")~).

Replacement: added ~Vec._is_str_type()~ helper that compares ~Ptr.elem_type~
(C string pointer) against "Str" using ~memcmp~ (4 bytes including null terminator).
Constructors now set ~p.elem_type = type_str.c_string.data~ on a local Ptr (needed
intermediate ~type_c := type_str.c_string~ to avoid chained field access on function
return, which the interpreter doesn't support).

Updated 6 call sites across set.til (3), map.til (1), hashmap.til (2) to use
~_is_str_type()~ instead of reading ~type_name~.

Updated codegen (ccodegen.rs + ccodegen.til): precomputed Vec emission no longer
emits ~.type_name~ Str initializer. Sets elem_type as 4th field of Ptr in the
initializer. Same for nested Vec emission (~emit_precomputed_nested_vec_static~).

Updated eval_heap (eval_heap.rs + eval_heap.til): ~extract_vec_contents~ reads
elem_type from Ptr offset 24 (C string pointer) via new ~read_c_string()~ helper
(reads bytes until null terminator). Removed ~extract_str_from_bytes~ (no longer used).
Nested Vec parsing in codegen also reads elem_type from Ptr offset 24 instead of
extracting a Str struct.

Updated introspections.til: removed ~VEC_HAS_TYPE_NAME~ test assertion.
...
5. *Collection .get()*: copy_to_dynamic memcpy creates aliases. Partially mitigated
   ...
   remain -- should use get_by_ref + cast (zero-copy pointer into collection storage).

*** Remediation plan

**** Phase 1: Eliminate .get() from all collections (no dependencies) -- DONE
All copying .get() methods removed from Vec, Map, Array, Set, List.
All callers converted to get_by_ref + cast (zero-copy pointer into storage).
...
(is_borrowed=false), caller uses cast. Internally, pop would get_by_ref the last
