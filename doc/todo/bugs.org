...
| CatchLabelInfoEntry | DIVERGE | TIL has extra ~catch_block: Expr~ field |
| ParamTypeInfo | DIVERGE | Rust ~Option<ValueType>~ -> TIL uses ~has_type: Bool~ flag instead of Ptr |
| SwitchCaseInfo | DIVERGE | Rust ~Option<&Expr>~ -> TIL uses ~has_case_body: Bool~ flag instead of Ptr |
| ThrowingFCallInfo | DIVERGE | TIL has extra ~is_throwing: Bool~, uses ~has_return_type: Bool~ instead of Ptr |
| VariadicFCallInfo | DIVERGE | TIL has extra ~is_variadic: Bool~ field |
| CodegenContext | DIVERGE | Rust uses ~Option<String>~, TIL uses ~Str = ""~. FIX: Change Rust to ~String~ per rs2til.org |

****** TIL-only Structs (UNNECESSARY per rs2til.org)
| Struct | Fields | Issue |
|--------|--------|-------|
| FuncNameResult | ~found: Bool~, ~name: Str~ | ~Option<T>~ returns should ~throw KeyNotFoundError~ |
| FuncLookupResult | ~found: Bool~, ~func_def: SFuncDef~ | ~Option<T>~ returns should ~throw KeyNotFoundError~ |

NOTE: Attempted to remove FuncLookupResult (2026-01-02) by changing get_fcall_func_def
to throw KeyNotFoundError instead of returning result struct. All 18 call sites were
updated mechanically. Result: til_compiled tests segfaulted. Reverted. Same issue
previously occurred with FuncNameResult. These may require deeper investigation.

***** Helper Functions - Present
| Function | Rust | TIL | Status | Notes |
|----------|------|-----|--------|-------|
| next_mangled | Y | Y | DIVERGE | TIL uses len()>0 check instead of Option match, no func_name variable |
| til_name | Y | Y | DIVERGE | TIL uses separate if statements instead of match pattern |
...
| get_func_name_string | Y | Y | DIVERGE | TIL uses FuncNameResult struct instead of Option, parts is Str not Vec |
| get_til_func_name_string | Y | Y | DIVERGE | TIL uses FuncNameResult struct instead of Option |
| get_type_arg_name | Y | Y | MATCH | Option->throws is correct rs2til.org translation |
| get_fcall_func_def | Y | Y | DIVERGE | TIL uses FuncLookupResult struct instead of Option (should use throws) |
...
| has_str | - | Y | DELETE | Not in Rust - remove from TIL |
| get_field_type_dependency | Y | - | PORT | Missing from TIL - needs porting |

***** Type/Struct/Enum Detection - Present
| Function | Rust | TIL | Status | Notes |
|----------|------|-----|--------|-------|
| is_struct_declaration | Y | Y | DIVERGE | Extra unused vars (decl, struct_def), uses get() vs direct indexing |
| is_enum_declaration | Y | Y | DIVERGE | Same pattern: extra vars, get() vs direct indexing |
| is_enum_with_payloads | Y | Y | DIVERGE | Same pattern: extra vars, get() vs direct indexing |
| is_constant_declaration | Y | Y | DIVERGE | Same pattern: extra vars, get() vs direct indexing |
| is_global_declaration | Y | Y | DIVERGE | Same pattern: extra vars, get() vs direct indexing |
| is_throwing_fcall | Y | Y | DIVERGE | Uses FuncLookupResult struct instead of Option |
| is_func_declaration | Y | Y | DIVERGE | Same pattern: extra vars, get() vs direct indexing |
| enum_has_payloads | Y | Y | DIVERGE | Uses loop index + get() vs iterator |
| variant_has_payload | - | Y | DELETE | Not in Rust - remove from TIL |
| get_struct_name | Y | Y | DIVERGE | Rust returns Option, TIL returns "" sentinel |
| get_struct_dependencies | Y | Y | DIVERGE | Rust uses get_field_type_dependency helper, TIL inlines logic |
| get_enum_name | Y | Y | DIVERGE | Rust returns Option, TIL returns "" sentinel |
| get_enum_dependencies | Y | Y | DIVERGE | Loop style differs, extra variables |
| get_type_name | Y | Y | DIVERGE | Rust returns Option, TIL returns "" sentinel |
| get_type_dependencies | Y | Y | DIVERGE | Similar structure but empty return logic differs |
| get_fcall_func_name | Y | Y | DIVERGE | Rust returns Option, TIL uses FuncNameResult struct |
| get_struct_construction_type | Y | Y | DIVERGE | Rust returns Option, TIL returns "" sentinel, loop style differs |
| get_enum_construction_type | Y | Y | DIVERGE | Rust returns Option, TIL returns "" sentinel |
| get_case_variant_info | Y | Y | DIVERGE | Rust uses match, TIL uses switch + mut result variable |
| parse_pattern_variant_name | Y | Y | DIVERGE | TIL has extra catch blocks for error handling |

***** Topological Sort - Present
| Function | Rust | TIL | Status | Notes |
|----------|------|-----|--------|-------|
| topological_sort_types | Y | Y | DIVERGE | Rust uses Vec<Vec>, TIL uses Map<I64, Vec>, different loop styles |
| topological_sort_structs | - | Y | DELETE | Old function, Rust uses topological_sort_types |
...
| Function | Rust | TIL | Status | Notes |
|----------|------|-----|--------|-------|
| emit | Y (pub) | Y | DIVERGE | Switch vs if-let, collect_nested_func_info uses "" vs None |
| emit_struct_declaration | Y | Y | DIVERGE | Uses continue vs if block, different error handling |
| emit_struct_constants | Y | Y | DIVERGE | Common patterns |
| emit_struct_func_prototypes | Y | Y | DIVERGE | Common patterns |
| emit_struct_func_body | Y | - | PORT | Missing from TIL - needs porting |
| emit_struct_func_bodies | Y | Y | DIVERGE | Common patterns |
| emit_enum_declaration | Y | Y | DIVERGE | Common patterns |
| emit_enum_with_payloads | Y | Y | DIVERGE | Common patterns |
| emit_enum_to_str_function | Y | Y | DIVERGE | Common patterns |
| emit_enum_to_str_for_declaration | Y | Y | DIVERGE | Common patterns |
| emit_enum_size_of_constant | Y | Y | DIVERGE | Common patterns |
| emit_size_of_function | Y | Y | DIVERGE | Common patterns (FIXED Issue 6 only) |
| emit_func_signature | Y | Y | DIVERGE | Common patterns |
| emit_func_prototype | Y | Y | DIVERGE | Common patterns |
| emit_func_declaration | Y | Y | DIVERGE | Common patterns |
| emit_constant_declaration | Y | Y | DIVERGE | Uses has_str() helper (DELETE), common patterns |
| emit_global_declaration | Y | Y | DIVERGE | Common patterns |
| emit_expr | Y | Y | DIVERGE | Common patterns |
| emit_body | Y | Y | DIVERGE | Common patterns |
| emit_stmts | Y | Y | DIVERGE | Common patterns |
| emit_if | Y | Y | DIVERGE | Common patterns |
| emit_while | Y | Y | DIVERGE | Common patterns |
| emit_break | Y | Y | DIVERGE | Common patterns |
| emit_continue | Y | Y | DIVERGE | Common patterns |
| emit_switch | Y | Y | DIVERGE | Common patterns |
| emit_fcall | Y | Y | DIVERGE | get_til_func_name_string replaced with replacen() hack |
| emit_declaration | Y | Y | DIVERGE | Common patterns |
| emit_funcdef | Y | Y | DIVERGE | Common patterns |
| emit_assignment | Y | Y | DIVERGE | Common patterns |
| emit_return | Y | Y | DIVERGE | Common patterns |
| emit_throw | Y | Y | DIVERGE | Common patterns |
| emit_literal | Y | Y | DIVERGE | Common patterns |
| emit_str_literal | Y | Y | DIVERGE | Common patterns |
| emit_identifier_without_nested | - | Y | DELETE | Not in Rust - remove from TIL |
| prescan_func_level_catches | Y | Y | DIVERGE | Common patterns |
...
| Function | Rust | TIL | Status | Notes |
|----------|------|-----|--------|-------|
| check_throwing_fcall | Y | Y | DIVERGE | Rust returns Option, TIL throws; TIL inlines func lookup logic |
| hoist_throwing_expr | Y | Y | DIVERGE | Common patterns |
| hoist_throwing_args | Y | Y | DIVERGE | Common patterns |
| hoist_for_ref_params | Y | Y | DIVERGE | Common patterns |
| hoist_for_dynamic_params | Y | Y | DIVERGE | Common patterns |
| hoist_variadic_args | Y | Y | DIVERGE | Common patterns |
| detect_variadic_fcall | Y | Y | DIVERGE | Common patterns |
| emit_variadic_call | Y | Y | DIVERGE | Common patterns |
| emit_throwing_call | Y | Y | DIVERGE | Common patterns |
| emit_throwing_call_propagate | Y | Y | DIVERGE | Common patterns |
| emit_throwing_call_with_goto | Y | Y | DIVERGE | Common patterns |
| emit_fcall_with_hoisted | Y | Y | DIVERGE | Common patterns |
| emit_fcall_name_and_args_for_throwing | Y | Y | DIVERGE | Common patterns |
| emit_arg_or_hoisted | Y | Y | DIVERGE | Common patterns |
| emit_arg_with_param_type | Y | Y | DIVERGE | Common patterns |
| emit_arg_with_mut | - | Y | DELETE | Not in Rust - Rust uses emit_arg_with_param_type |

***** Collection Functions - Present
NOTE: Same common divergence patterns.

| Function | Rust | TIL | Status | Notes |
|----------|------|-----|--------|-------|
| collect_func_info | Y | Y | DIVERGE | Common patterns |
| collect_nested_func_info | Y | Y | DIVERGE | Common patterns |
| collect_declarations_in_body | Y | Y | DIVERGE | Common patterns |
| collect_declarations_recursive | Y | Y | DIVERGE | Common patterns |
