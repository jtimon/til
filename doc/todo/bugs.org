...
Added resolve_inferred_types() function in typer.rs that walks AST after
check_types and replaces INFER_TYPE with resolved types from scope_stack.
Called from interpreter.rs and builder.rs after check_types.

**** Current approach issues
- resolve_inferred_types is called from multiple places (interpreter.rs, builder.rs)
- Scattered calls instead of integrated into typer phase
- TIL port not yet done (typer.til, interpreter.til, builder.til)

**** Better approach (TODO)
Integrate into check_types itself:
1. Change check_types signature to return (Expr, Vec<String>) or Result<Expr, String>
2. Make check_types take mut Expr and modify in place, OR return modified AST
3. Single entry point - callers just call check_types and get resolved AST back
4. This aligns with Issue #127 pipeline refactoring
