...
:STATUS: Partial
:RELATED: Issue #127 (pipeline refactoring)
:END:

*** Description
After typer completes, no INFER_TYPE references should remain in the AST.
Currently, Declaration nodes may still have value_type=INFER_TYPE after typer,
requiring later phases (precomp, sugar, ccodegen) to re-infer types.

*** Problem
This causes architectural issues:
1. Precomp calls get_value_type() to look up types that should already be resolved
2. Sugar phase (Issue #127) needs type info but can't rely on AST having it
3. Each phase duplicates type inference logic instead of reading resolved types

*** Expected Behavior
Typer should mutate the AST to replace all INFER_TYPE with the resolved concrete type.
Post-typer phases should be able to assert/panic if they encounter INFER_TYPE.

*** Current state
- resolve_inferred_types() exists but has unnecessary scope_stack lookup
- builder.til was never updated with TIL port
- Post-typer phases still have INFER_TYPE handling code that can be removed

*** Workflow for each step
After completing each step:
1. Run `make benchmark` to verify all tests pass
2. Update Bug #128 with progress notes
3. Commit changes

*** Step 2: Port missing TIL changes to builder.til

File: src/self/builder.til

Add missing calls to match src/rs/builder.rs:
- Use type_check() where Rust uses it (returns TypeCheckResult with resolved AST + errors)
- Call resolve_inferred_types() in collect_imports and for core.til/mode imports

Then: make benchmark, update progress, commit.

*** Step 3: Simplify resolve_inferred_types

Files: src/rs/typer.rs, src/self/typer.til

Remove scope_stack lookup, just use get_value_type() directly.

Then: make benchmark, update progress, commit.

*** Step 4: Fix desugarer to not introduce INFER_TYPE

Files: src/rs/desugarer.rs, src/self/desugarer.til

The desugarer runs after typer and creates for-in index variables with INFER_TYPE.
Since desugaring happens post-typer, these won't be resolved.

Fix:
- Use I64 directly for the ~_for_i~ index variable (always initialized to 0,
  compared to len(), used with get() - all I64 operations)
- Use the explicitly provided type for the element variable (~for elem: MyType in collection~
  uses MyType directly instead of INFER_TYPE)

Note: ~for elem: auto in collection~ won't work until Bug #129 is fixed.

Then: make benchmark, update progress, commit.

*** Step 4b: Fix resolve_inferred_types to recurse into StructDef

Files: src/rs/typer.rs, src/self/typer.til

resolve_inferred_types() had a default case that recursed into e.params, but
StructDef stores function bodies in default_values (a HashMap<String, Expr>),
not in e.params. This meant type inference inside struct method bodies was
never resolved.

Fix: Add explicit StructDef case that iterates over default_values and
recursively calls resolve_inferred_types on each function body expression.

Then: make benchmark, update progress, commit.

*** Step 5: Remove INFER_TYPE handling from precomp

Files: src/rs/precomp.rs, src/self/precomp.til

In `precomp_declaration`:
- `if decl.value_type != INFER_TYPE` guard that skips type checking: REMOVE
- TIL: same guard in `precomp_declaration`

Port to TIL. Then: make benchmark, update progress, commit.

*** Step 6: Remove INFER_TYPE handling from interpreter

Files: src/rs/interpreter.rs, src/self/interpreter.til

In `eval_declaration`:
- `if declaration.value_type != INFER_TYPE` guard that skips type checking: REMOVE
- Match `INFER_TYPE => return error`: KEEP (already correct)
- Struct member type inference: two places that call `get_value_type` for
  INFER_TYPE members, then check again for INFER_TYPE: REPLACE with lang_error
- TIL: same 4 locations in `eval_declaration`

In `eval_assignment`:
- Match `INFER_TYPE => return error`: KEEP (already correct)
- TIL: same in `eval_assignment`

In `eval_identifier_expr`:
- Match `INFER_TYPE => return error`: KEEP (already correct)
- TIL: same in `eval_identifier_expr`

Port to TIL. Then: make benchmark, update progress, commit.

*** Step 7: Remove INFER_TYPE handling from ccodegen

Files: src/rs/ccodegen.rs, src/self/ccodegen.til

In `get_field_type_dependency`:
- `s if s == INFER_TYPE => None`: REPLACE with lang_error
- TIL: same in `get_field_type_dependency`

In `emit_constant_declaration`:
- `if type_name != INFER_TYPE` with else that falls back to I64: REPLACE with lang_error
- TIL: same in `emit_constant_declaration`

In `til_type_to_c`:
- `if name == INFER_TYPE` returns error: KEEP (already correct)
- TIL: same in `til_type_to_c`

In `emit_declaration`:
- Two places: `is_infer` check that infers from RHS: REPLACE with lang_error
- TIL: same in `emit_declaration`

In `collect_declarations_recursive`:
- `if name == INFER_TYPE` that infers from RHS: REPLACE with lang_error
- TIL: same in `collect_declarations_recursive`

Port to TIL. Note: TIL has many INFER_TYPE as Vec.get() placeholder values - those stay.

Then: make benchmark, update bugs.org and mark Bug #128 as DONE (move to
Fixed Bugs section), final commit.

*** Verification
Each step runs make benchmark. All tests must pass before committing. If any
code path still produces INFER_TYPE after typer, the new assertions will catch it.

*** Progress
- Step 2 DONE: Added resolve_inferred_types() calls to builder.til in
  collect_imports, core.til handling, mode imports, and main file handling
- Step 3 DONE: Simplified resolve_inferred_types() to use get_value_type()
  directly instead of scope_stack lookup. Kept scope management (FuncDef,
  Body, If/While, ForIn) intact - only Declaration case changed.
- type_check() fix: Modified to skip resolve_inferred_types when check_types
  has errors. This avoids duplicate errors and short-circuits correctly.
- Catch handler fix: Added NodeType::Catch handler to resolve_inferred_types
  to declare catch variable in scope before processing body. This mirrors
  check_catch_statement behavior and fixes "Undefined symbol" errors for
  catch variables like err.msg.
- Step 5 DONE: Remove INFER_TYPE handling from interpreter. Added struct
  member type resolution to typer's StructDef handler (was missing). Removed
  INFER_TYPE guard in eval_declaration - type check always runs now. Replaced
  struct member INFER_TYPE inference with lang_error.
- Step 6 DONE: Remove INFER_TYPE handling from ccodegen. Replaced inference
  logic with panic/lang_error in: get_field_type_dependency, emit_constant_declaration,
  emit_declaration (2 places), collect_declarations_recursive.

*** Step 7: Unify type resolution with type checking
No need for two passes (check_types + resolve_inferred_types). Merge them into
a single pass that both checks types and resolves INFER_TYPE in one traversal.
...
** Bug #125: Non-deterministic order of "Unused variable" errors
:PROPERTIES:
:DISCOVERED: 2026-01-19
:IMPACT: Test output comparison fails intermittently
:STATUS: Open
:END:

*** Description
When multiple unused variables are declared in the same function, the order of
"Unused variable" errors is non-deterministic between runs. This causes test
failures when comparing expected vs actual output.

*** Root Cause
In src/rs/init.rs:79, `function_locals` is a `HashSet<(String, usize, usize)>`.
The `get_unused_symbols()` function at line 140 iterates over this HashSet,
and HashSet iteration order is non-deterministic in Rust.

*** Fix
Sort the unused symbols before returning them from `get_unused_symbols()`,
e.g. by name, or by (line, col, name) for consistent ordering.
