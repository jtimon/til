...
bbb := Color.Green(true)  // Global with Bool payload

test := proc() {
    local_bbb := bbb  // SEGFAULT here
    println(enum_to_str(local_bbb))
}
test()
#+END_SRC

*** Notes
- Works correctly with ~rstil interpret~
- Works correctly with local variables (only globals trigger the bug)
- Pattern binding fix (Bug #71) works correctly for local enum variables
