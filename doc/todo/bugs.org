...
*** Notes
Bug #52 was discovered while investigating this issue (static buffer in ext.c
causing string corruption in debug output), but fixing it did not resolve the
NodeType memory corruption.
