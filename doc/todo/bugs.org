...
#+BEGIN_EXAMPLE
// lexer.til (mode liba)
print_lex_error := func(...) {
    print(...)  // Should be allowed in mode liba
}

// til.til (mode cli)
import("self.lexer")
// Error: funcs cannot call procs (print_lex_error's print calls)
#+END_EXAMPLE

*** Expected Behavior
The `mode liba` file should retain its relaxed purity rules when imported. A func in a `mode liba` file should be able to call print/println regardless of the importing file's mode.

*** Current Behavior
The importing file's mode (`cli`) takes precedence, causing "funcs cannot call procs" errors for print calls in the imported `mode liba` file.

*** Workaround
Change funcs that use print to procs, or avoid print in funcs entirely.

*** Files to Investigate
- src/rs/typer.rs: Mode handling during import
- src/rs/init.rs: Import processing
