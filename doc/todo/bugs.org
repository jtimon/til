...
*** Description
The =run_cmd= ext_proc doesn't capture command output correctly in compiled mode.
The mutable output string parameter remains empty (or gets only a newline).

*** Reproduction
#+BEGIN_SRC til
mut output := ""
exit_code := output.run_cmd("bash", "-c", "echo hello")
// Interpreted: output = "hello\n", len = 6
// Compiled: output = "\n", len = 1
#+END_SRC

*** Test
src/test/syscalls.til has =test_run_cmd_output()= (currently commented out).

*** Blocking
This blocks using compiled =bin/test= for faster test execution.
