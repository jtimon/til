...
** Bug #70: mut_test fails with garbage memory on macOS (til_interpreted)
:PROPERTIES:
:DISCOVERED: 2025-12-31
:IMPACT: test failure on macOS CI
:STATUS: Active
:RELATED: Bug #65 (likely caused by this fix)
:END:

*** Description
~src/test/mut_test.til~ fails on macOS CI with ~til interpret~:
#+BEGIN_EXAMPLE
src/test/mut_test.til:65:19:assert_eq failed: expected '0', found '3328'
#+END_EXAMPLE

The failure is in ~test_const_to_const~ which reads ~v.x~ from a default-initialized Vec2.
The value should be 0, but garbage memory is returned.

*** Cause Analysis
The failure started between commits 590410c (passed) and 4a6bedd (failed).
The only til.c change in that range was commit ecf0567 (Bug #65 fix), which
changed ~custom_type_name~ to ~resolved_type_name~ in ~eval_user_func_proc_call~.

This affects how struct parameters are passed to functions. The Bug #65 fix may have
introduced platform-specific undefined behavior that manifests on macOS but not Linux.

*** Workaround
Commented out ~mut_test.til~ from ~til_interpreted~ test category until fixed.
