...
Build passes. 109/109 tests pass. Test breakdown:
- 33 tests in all_common (pass all 4 modes)
- 32 tests in til_interpreted (pass til interpret, fail til compile)
- 37 tests in rs_common (pass rstil, fail til compile)
- 1 test in til_common (panic.til - different error message format)

*** Current Error Summary

The 32 tests in til_interpreted have these failure modes when run with ~./bin/til run~:

| Error Type | Count | Example |
|------------|-------|---------|
| segfault | 28 | Crash during C execution |
| gcc shadow warning | 1 | bug52.til - variable shadowing in generated C |
| gcc error undeclared | 1 | c_mem.til - variable not declared |
| gcc type mismatch | 2 | arenas.til, arrays.til - incompatible pointer types |
...
Current test: ~src/test/arrays.til~
