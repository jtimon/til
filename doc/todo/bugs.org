...
:IMPACT: 4 til_interpreted tests fail (bug56, editor_mode_test, enums, forward_declarations)
:STATUS: Active
:END:
...
*** Errors Seen
#+BEGIN_SRC
src/test/bug56.til:5:11: type ERROR: Enum variant 'Variant1' uses undefined type ''.
src/test/bug56.til:17:12: til init ERROR: Enum variant payload type 'unknown' not yet supported
#+END_SRC

*** Investigation Notes
- ~str_to_value_type~ in parser.til looks identical to parser.rs (lines 523-541 vs 284-292)
- Both have catch-all: ~return ValueType.TCustom(arg_type)~
- The type name is being lost somewhere in the enum parsing chain before it reaches the typer
- Need to trace where enum variant types are parsed and stored

*** Reproducing
#+BEGIN_SRC til
MyEnum := enum {
    Variant1: Str,  // This fails with til, works with rstil
}
#+END_SRC

Test files: src/test/bug56.til, src/test/enums.til

*** Plan to Fix
1. Add debug prints in parser.til where enum variants are parsed
2. Compare with parser.rs to find the divergence
3. Fix the parsing/storage of enum variant payload types
4. Test all 4 affected tests
...
*** Investigation Notes
- Tested that ~mut~ struct parameters persist modifications across function calls
- Tested that nested struct field modifications work
- The ~push~ and ~pop~ calls on scope_stack work correctly
- ~remove_symbol~ in init.til uses get/modify/set pattern (lines 273-290)
- Rust version uses ~last_mut()~ to directly modify

*** Hypothesis
The get/modify/set pattern in ~remove_symbol~ might not be propagating changes correctly
when the ScopeStack is accessed through a chain of struct fields (~context.scope_stack~).

*** Plan to Fix
1. Add debug prints to verify remove_symbol is called and modifies the frame
2. Check if the issue is in how Vec.set works for ScopeFrame
3. Compare the scope state before/after remove_symbol call
4. May need to trace through with gdb on generated C code
...
*** Fix
In typer.rs/typer.til, add a check when processing FCall args: if the parameter
is ~own~ and the argument is a const variable, emit a type error.
