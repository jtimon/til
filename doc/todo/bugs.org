...
**** Why It Failed
The I64Range.get() method throws OutOfBounds. The for-in desugaring
...
while (til_lt(&_for_i_func_0, &(til_I64){til_I64Range_len(...)}).data) {
    til_OutOfBounds _thrown_...;
    til_I64Range_get(...);  // throws
    if (0) { _catch_...:
        til_OutOfBounds til_err = ...;  // PROBLEM: hoisted
        til_panic(...);
    }
    // body
}
...
I64Range := struct {
    ...
    get := proc(self: I64Range, index: I64, mut out: Dynamic) throws OutOfBounds {
        if index.gteq(self.len()) {
            throw OutOfBounds(msg="I64Range.get: index out of bounds")
        }
        if self.end.gteq(self.start) {
...
1. Catch block placement: Catch blocks were placed AFTER emit_arg_with_param_type(),
   so when param_by_ref.get() or param_types.get() threw OutOfBounds,
...
til inlines:
#+BEGIN_SRC c
*_err1 = (til_OutOfBounds){.msg = til_format(...)};
#+END_SRC
