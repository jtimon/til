...
** Bug #35: Special `_` variable does not allow repeated declarations
:PROPERTIES:
...
*** Expected Behavior
The `_` variable should be special-cased to allow multiple declarations in the same scope, since it's used to explicitly discard return values (per Bug #8 fix that requires explicit `_ :=` for unused return values).

*** Current Workaround
Use indexed variable names:
#+BEGIN_EXAMPLE til
_1 := context.imports_init_done.insert(path)
_2 := context.imports_typer_done.insert(path)
#+END_EXAMPLE

Or use dummy names:
#+BEGIN_EXAMPLE til
_init := context.imports_init_done.insert(path)
_typer := context.imports_typer_done.insert(path)
#+END_EXAMPLE

*** Affected Files
- src/rs/init.rs - init_context declaration handling
- src/self/init.til - same

*** Potential Fix
In init_context when processing declarations:
1. Check if declaration name is "_"
2. If so, skip the "already declared" check
3. Optionally: don't store "_" in symbols at all (just evaluate the RHS)

** Bug #34: Code between throwing call and catch runs unconditionally
:PROPERTIES:
:DISCOVERED: 2025-12-14
...
*** Description
When there is code between a throwing function call and its catch block, that code runs unconditionally (on both success and error paths) instead of only on success.

#+BEGIN_EXAMPLE til
// Expected behavior:
result = throwing_call()   // throws on error
found = true               // Should only run on SUCCESS
catch (err: E) { }         // Catches the error

// Actual behavior:
// found = true runs ALWAYS, regardless of success/failure
#+END_EXAMPLE

*** Root Cause
The C code generator emits the if/else chain for error handling, then emits subsequent statements AFTER the chain:

#+BEGIN_SRC c
int _status = throwing_call(...);
if (_status == 0) {
    // success case - maybe assign return value
} else if (_status == 1) {
    // catch body
}
found = true;  // BUG: runs always, not inside success block!
#+END_SRC

The code generator only handles two patterns correctly:
1. Immediate catch after throwing call (no intervening statements)
2. Declaration/assignment with throwing call: ~result = throwing_call()~ followed by catch

*** Workarounds
Two patterns work correctly:

*Pattern 1: Use contains_key before get (for Map lookups)*
#+BEGIN_EXAMPLE til
// Instead of:
map.get(key, value)
found = true
...
*Pattern 2: Set flag INSIDE catch block*
#+BEGIN_EXAMPLE til
// Instead of:
found = false
lookup_call()
found = true              // BUG: runs always
catch (err: E) { }
...
lookup_call()
    ...
    found = false         // Set to false only on error
}
#+END_EXAMPLE

*** Proper Fix
The code generator should detect statements between a throwing call and its catch block, and emit those statements inside the success branch of the if/else chain.

*** Affected Files
- src/rs/ccodegen.rs: emit_stmts, emit_throwing_call_with_goto
- src/self/ccodegen.til: emit_stmts, emit_throwing_call_with_goto

*** Workaround Locations
These files contain workarounds for this bug (marked with TODO #34):
- src/self/init.til: lookup_symbol, lookup_func, lookup_enum, lookup_struct, init_context Declaration check
...
*** Workaround
Use while loops with explicit initialization using a valid enum variant:

#+BEGIN_EXAMPLE til
mut i := 0
while i.lt(vec_of_valuetypes.len()) {
    mut val := ValueType.TCustom("")  // Use any valid variant as placeholder
    vec_of_valuetypes.get(i, val)
    // body
    i.inc()
}
#+END_EXAMPLE
...
*** Potential Fixes
1. Add a default/placeholder variant to enums that need for-in iteration
2. Change for-in desugaring to use a different mechanism that doesn't require default initialization
3. Allow enums to specify a default variant for this purpose
