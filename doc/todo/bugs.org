...
:STATUS: In Progress - Steps 3-7 done, copy_fields fully removed from interpreter
:END:
...
**TODO**: =own= params should be by-reference (just bind offset + remove from caller),
like =default= and =mut=. The memcpy added here is correct but redundant — ownership
transfer doesn't need copying. Only =copy= params need clone from garbager. This is
a future optimization to simplify the own param path to pure offset rebinding.
