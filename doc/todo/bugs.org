...
*** Description
The ~continue~ statement causes infinite loops when used in ~for~ loops.
This happens because ~for~ desugars to ~while~ with the loop variable increment
...
for i in 0..10 {
    if i.eq(5) {
        continue  // skips implicit i.inc(), loops forever at i=5
    }
    println(i)
}

// WORKAROUND: use while with increment BEFORE continue
mut i := 0
while lt(i, 10) {
    i.inc()  // increment first
    if i.eq(5) {
        continue  // now safe
    }
    println(i)
...
*** Root Cause
The precompiler desugars ~for i in 0..N { body }~ to:
#+BEGIN_SRC til
mut i := 0
while lt(i, N) {
    body
...
*** Reproducing
#+BEGIN_SRC bash
timeout 2 ./bin/rstil interpret src/test/bug57_continue_for.til
# Times out (infinite loop) - when fixed, should print 0-9 skipping 5
#+END_SRC

*** Fix Options
1. Move increment before body in desugaring (changes for-loop semantics)
2. Transform continue to jump past body but before increment
3. Document as limitation (current approach)
...
the interpreter rejects it with "Struct payload must be a variable identifier
or literal, got FCall". This only happens inside func/proc bodies - top level
declarations work fine. Compiled mode also works fine.
...
// FAILS: inside func/proc
test_func := func() returns MyEnum throws AllocError {
    x := "hello"
    return MyEnum.Variant1(x.clone())  // ERROR!
}

// WORKAROUND: extract to variable first
...
*** Reproducing
#+BEGIN_SRC bash
./bin/rstil interpret src/test/bug_enum_fcall.til
# Output:
# top level declaration: OK
# workaround in func: OK
# ERROR: ... Struct payload must be a variable identifier or literal, got FCall
#+END_SRC

*** Root Cause
Likely in interpreter.rs where enum construction is evaluated. The top-level
declaration code path handles FCall payloads, but the func/proc body code path
does not.
