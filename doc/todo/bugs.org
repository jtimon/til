...
:STATUS: Open
:REPRODUCER: See tmp/test_global_init.til
...
rs_interpreted:  PASS
rs_compiled:     PASS
til_interpreted: FAIL ("Ptr.new_by_size: invalid size")
til_compiled:    PASS
#+END_SRC

*** Root Cause
Unknown. The TIL interpreter handles func return values differently during module
initialization than the Rust interpreter. May be related to precomputation attempts
in the TIL interpreter that don't exist in the Rust version.
...
:REPRODUCER: rstil run src/examples/music.til jonio do (mode_intervals is Vec<Vec<I64>>)
