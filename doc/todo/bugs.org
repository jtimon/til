...
**** Remaining steps
- Step 3: HashMap (same pattern as Map)
- Step 4: Remove Vec.new_from_type_info
- Step 5: Vec -- remove type_size, use ptr.elem_size everywhere
- Step 6: Array -- remove type_name/type_size, migrate ptr from I64 to Ptr
- Step 7: List (needs investigation, heterogeneous)
...
:RELATED: Bug #159 (shallow copies), Bug #114 (ScopeFrame copy leak), Issue #117 (ASAP destruction)
:END:
...
:STATUS: Open -- partially fixed, shallow copies remain throughout codebase
:RELATED: Bug #165 (memory leak blocked by this), Bug #114 (ScopeFrame leak)
:END:
