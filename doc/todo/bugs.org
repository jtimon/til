...
:STATUS: Open -- interpreter copy_fields removed; ccodegen shallow copies remain; deep Vec.clone/delete blocked on Issue #91 (function pointers); .get() elimination in progress
...
- =get=, =push=, =set=, =clone=, =extend=, =extend_with=, =contains=, =remove=,
...
- =insert=, =remove=, =contains=, =get= -- copy_to_dynamic for comparison/retrieval

**** 5. src/core/array.til -- Direct memcpy, same dont_delete protection
- =get=, =set=, =clone= -- copy_to/from_dynamic

**** 6. ccodegen -- enum_get_payload
- Shallow memcpy of enum payload data

**** 7. ccodegen -- create_alias directive
- Emits =memcpy(&var, (void*)addr, sizeof(type))= -- shallow copy
- Interpreter handles this as =insert_var(name, addr)= -- zero copy, pure offset binding
- The C equivalent would be a pointer cast: =Type* var = (Type*)addr;=
- Tracked under Bug #168 as next actionable item
...
5. *Collection .get()*: copy_to_dynamic memcpy creates aliases. Partially mitigated
   by clone-after-get (Step 9), but the .get() method itself is the wrong API --
   should use get_by_ref + create_alias (zero-copy pointer into collection storage).

*** Remediation plan

**** Phase 1: Eliminate .get() from all collections (no dependencies)
Replace all .get() callers with get_by_ref + create_alias + dont_delete.
Then delete the .get() method from Vec, Map, Array, Set.
Add get_by_ref to List (currently missing).
One commit per collection type. Independent of all other fixes.
...
**** Phase 4: Ccodegen return move semantics
After =*_ret = var;=, emit =memset(&var, 0, sizeof(Type));= to zero the source.
Turns shallow copy into move. Only for simple local variables (not ref params,
not ret_var_alias, not field access).
...
:STATUS: Open - params.get/e.get done (zero-copy Expr.get), ScopeStack frame copies remain
:END:

*** Progress: Zero-copy Expr.get() (2026-02-14)

Categories A+B from copies.org (490 calls copying Expr) are now zero-copy:
- Replaced ~.params.get(i, var)~ with ~.get(i)~ + ~dont_delete(var)~ across
  all 11 TIL self-hosted files. Uses get_by_ref + create_alias (no copy).
- Garbager updated to collect dont_delete vars recursively from nested
  Body/If/While/Catch nodes, and strip dont_delete calls from Body handlers.
- Fixed double-free in ccodegen save/restore pattern (~saved_ref_params~,
  ~saved_catch_labels~): after restoring to ctx fields, the local aliases
  the field's heap data. Added dont_delete for these locals.

Remaining: ScopeStack frame iteration (Category C, 35 calls copying ScopeFrame),
Map.get (Category D), Vec.get on non-params Vecs (Category E).

*** Description
TIL's ScopeStack lookup functions (lookup_var, lookup_symbol, lookup_func, lookup_enum,
lookup_struct, has_var, has_symbol, has_func, has_enum, has_struct, etc.) copy entire
ScopeFrame objects when iterating through frames:

#+BEGIN_SRC til
// Current pattern (leaks memory) - ALL lookup/has functions use this
...
*** Why Bug #144 (for-in by-ref) doesn't fix this
These functions use ~for i in range~ + ~self.frames.get(i, frame)~ (manual
index loops), NOT ~for frame: ScopeFrame in self.frames~ (for-in loops).
They must iterate in reverse (innermost scope first), and for-in has no
reverse iteration support. The manual ~.get()~ call always copies.

Fix requires either:
- Manual ~get_by_ref~ usage: ~frame_ptr := self.frames.get_by_ref(i)~ then
  pointer cast (like create_alias does), but awkward without language support
- Reverse for-in: ~for frame: ScopeFrame in self.frames.reverse()~
...
Note: The has_* methods still copy ScopeFrame via manual ~.get()~ calls.
Bug #144 (now fixed) only helps ~for item: Type in collection~ loops, not
manual index loops. Option C avoids copying the *definition* objects
(VarDef, FuncDef, etc.) but not the ScopeFrame itself.
