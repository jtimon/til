...
| get_func_name_string | Y | Y | DIVERGE | Throws KeyNotFoundError (correct), but Rust uses vec+join, TIL uses string concat |
| get_til_func_name_string | Y | Y | DIVERGE | Throws KeyNotFoundError (correct), but Rust uses vec+join, TIL uses string concat |
