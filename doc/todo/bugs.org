...
** Bug #34: Code after catch blocks doesn't run in error path :FIXED:
:PROPERTIES:
:DISCOVERED: 2025-12-14
:FIXED: 2025-12-15
...
*** Description
Code placed AFTER catch blocks (not between throw and catch) doesn't run when an error is caught.

#+BEGIN_EXAMPLE til
bug34_map_get("missing", value)
found = true  // Between throw and catch - correctly skipped on error
catch (err: Bug34Error) {
    // error caught
}
// Code AFTER catch - BUG: was not running when error was caught
test(loc(), found.not(), "found should be false")
#+END_EXAMPLE
...
*** Fix
- Track ~last_catch_index~ to know where catch blocks end
- Break out of main loop when past last catch
- Emit after-catch statements at the ~_end_catches~ label
- Add ~emit_stmts_no_catches~ helper for proper throwing call handling
