...
*** Implementation Attempt (2026-01-25) - Deferred
...
- Double declaration errors occurred when both type_check_and_resolve and helpers
  tried to register the same symbols

Root cause: The helper functions are tightly coupled with scope management.
Simply removing recursion breaks the symbol registration needed for type checking.

*** Proper Fix (future work)
To properly fix this, need to:
1. Separate validation logic from scope registration in helper functions
2. Have type_check_and_resolve do all scope management and registration
3. Helpers become pure validation functions that read from scope but don't modify it
4. Carefully handle order of operations (register before validate body)
