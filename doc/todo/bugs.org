...
2. Interpreter uses offset binding instead of copy_fields
...
*** CRITICAL: User Decides Order
- *MUST* wait for user to specify which copy_fields case to tackle next
- *MUST NOT* jump ahead or work on cases user hasn't assigned

*** MUST: Port to TIL
Any Rust changes *MUST* be ported to equivalent TIL files before testing.

*** Known Blocker: Function Return Scope Problem
When function returns struct literal with rebound fields:
1. Field rebindings exist in function scope
2. Function returns, scope pops
3. Rebindings lost before caller can use them

This may block struct literal fields. User aware. Wait for user direction.
...
**** Step 3: DONE - Move clone logic from interpreter to garbager (mut declarations)
...
- Removed duplicate clone logic from ccodegen (both Rust and TIL)
...
**** Remaining copy_fields locations (progress tracking)
...
**** Step 4: TODO - Handle assignment statements
Transform =x = y= (where y is a struct) to =x = Type.clone(y)= in garbager.

**** Step 5: TODO - Handle copy function params
Transform copy params at call sites to use clone in garbager.
