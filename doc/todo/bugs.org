...
Build passes. 81/81 tests pass (0 failures). Test breakdown:
- 64 tests in all_common (pass all 4 modes)
- 1 test in til_common (panic.til)
- 4 tests in rs_interpreted
- 3 tests in rs_compiled
- 8 tests in rs_common
- 1 test in til_interpreted (maps.til)
...
*** Remaining Work
- maps.til still in til_interpreted (fails in til_compiled mode)
- 8 tests in rs_common (pass with rstil but not with til)
