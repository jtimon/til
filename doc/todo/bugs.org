...
| Function | Rust | TIL | Status | Notes |
|----------|------|-----|--------|-------|
| emit | Y (pub) | Y | DIVERGE | Large function ~200 lines, uses eq() instead of method chaining in places |
...
| emit_func_declaration | Y | Y | DIVERGE | ~100 lines, uses gteq() not .gteq(), loop var names differ (e.g. ~i~ vs ~ai~) |
| emit_constant_declaration | Y | Y | DIVERGE | Structural: TIL inlines literal handling, Rust uses c_type + emit_literal() |
| emit_global_declaration | Y | Y | DIVERGE | Similar to emit_constant_declaration - inlined literal handling |
| emit_expr | Y | Y | DIVERGE | ~100 lines, switch/case structure OK but many minor style diffs |
...
| hoist_for_ref_params | Y | Y | DIVERGE | ~120 lines, Option params→has_*:Bool pattern |
| hoist_for_dynamic_params | Y | Y | DIVERGE | ~120 lines, Option params→has_*:Bool pattern |
...
| Function | Rust | TIL | Status | Notes |
|----------|------|-----|--------|-------|
| collect_func_info | Y | Y | MATCH | Fixed loop vars to idx, comments match, HashMap iter via keys acceptable |
| collect_nested_func_info | Y | Y | DIVERGE | ~100 lines, Option parent_func_name→has_*:Bool pattern |
| collect_declarations_in_body | Y | Y | MATCH | Same comments, same vars (decls, collected, stmt), switch/case OK |
| collect_declarations_recursive | Y | Y | DIVERGE | ~80 lines, loop vars differ, switch/case OK |
