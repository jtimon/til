...
*** Possible Solutions
1. Make struct field access return mutable references when accessed on mut variables
2. Add explicit reference/pointer syntax for struct fields
3. Implement copy-on-write semantics with reference counting
4. Add compiler warning when calling mutating methods on field copies
