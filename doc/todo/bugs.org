...
*** Description
~src/test/bug47.til~ fails with exit code 141 in rs_compiled mode. GCC fails
due to ~-Werror~ treating warnings as errors. The warnings are about
~til_Vec_push~ expecting ~const til_Dynamic*~ but receiving typed pointers.

*** Root Cause
Generated C code passes typed pointers (e.g., ~til_Str*~, ~til_ValueType*~)
directly to ~til_Vec_push~ which expects ~const til_Dynamic*~.

*** Example Warning
#+BEGIN_SRC
gen/c/test/bug47.c:3630:117: warning: passing argument 3 of 'til_Vec_push' from incompatible pointer type
  til_Vec_push(&_err, &til_allowed_procs_in_funcs, &_tmp_mode_from_name_2);
                                                   ^~~~~~~~~~~~~~~~~~~~~~
                                                   til_Str *
expected 'void * const*' but argument is of type 'til_Str *'
#+END_SRC

Also has shadowing warnings for variables like ~til_s_cloned~ in generated clone functions.

*** Potential Fix
The C codegen should cast typed pointers to ~const til_Dynamic*~ when calling
~til_Vec_push~, or Vec push should be specialized per type.
...
*** Description
~make benchmark~ returns exit code 0 even when tests fail. Example output shows
"Failed: 2/107" but the process exits successfully. This breaks CI/CD pipelines
that rely on exit codes to detect failures.
...
...
Failed:                  2/107
src/tests.til:739:15:2// tests failed.
$ echo $?
0
#+END_SRC

*** Expected Behavior
Exit code should be non-zero when any tests fail.

** Bug #92: Unify range and collection iteration
:PROPERTIES:
:DISCOVERED: 2026-01-08
:IMPACT: DRY, type safety, code clarity
:STATUS: In Progress - Step 1 (I64Range) working in Rust, TIL port pending
:END:

*** Description
Range loops (~for i in 0..100~) and collection loops (~for x: T in vec~) used
separate desugaring paths. This violates DRY and prevents type annotations
on range loops.

*** Solution: Unified Iteration via I64Range Struct

**** Key Insight: NodeType::Range vs I64Range Struct
- ~NodeType::Range~ - AST node for ~0..10~ syntax (needed for switch/case patterns too)
- ~I64Range~ - TIL struct with len()/get() methods for iteration

**** Implementation
1. Parser: Remove ~130 lines of range-to-while desugaring, create ForIn("I64") node
2. Precomp: When ForIn collection is a Range node, wrap in I64Range struct
3. Precomp: Hoist I64Range to variable (like Vec), use same len()/get() desugaring

**** Generated Code (after fix)
#+BEGIN_SRC c
til_I64Range _range_func_0 = {.start = 0, .end = 5};
til_I64 _for_i_func_0 = 0;
while (til_lt(&_for_i_func_0, &til_I64Range_len(&_range_func_0)).data) {
    til_I64Range_get(&_range_func_0, &_for_i_func_0, &til_i);
    // body
    _for_i_func_0 = til_add(&_for_i_func_0, &(til_I64){1});
}
#+END_SRC

*** Incremental Plan

**** Step 1: I64Range [CURRENT]
- ~for i in 0..100~ works with I64Range
- Simple, hardcoded for I64
- No type annotation needed (defaults to I64)

Files modified:
- ~src/core/range.til~ - I64Range struct with len()/get()
- ~src/core/core.til~ - Added import
- ~src/rs/parser.rs~ - Removed range desugaring, creates ForIn node
- ~src/rs/precomp.rs~ - Wraps Range in I64Range, hoists to variable
- ~src/self/parser.til~ - Port pending
- ~src/self/precomp.til~ - Port pending

**** Step 2: Generic Range with Dynamic [FUTURE]
Like Vec, use Dynamic for type flexibility:
#+BEGIN_SRC til
Range := struct {
    mut type_name: Str = ""
    mut type_size: I64 = 0
    ...
    new := proc(T: Type, start_val: Dynamic, end_val: Dynamic) returns Range { ... }
    len := func(self: Range) returns I64 { ... }
    get := proc(self: Range, index: I64, mut out: Dynamic) throws IndexOutOfBoundsError { ... }
}
#+END_SRC

Then parser accepts type annotation:
#+BEGIN_SRC til
for i: U8 in 0..100 {}    // Range.new(U8, 0, 100)
for c: Str in "a".."z" {} // Range.new(Str, "a", "z")
for i in 0..10 {}         // Range.new(I64, 0, 10) - default
#+END_SRC
