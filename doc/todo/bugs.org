...
1. *MATCH* - Only when:
   - Actual code fix was made and tested (method chaining, for-in loops, unused vars, etc.)
   - Line-by-line comparison confirms semantic equivalence
   - rs2til.org explicitly documents the pattern as acceptable
2. *DIVERGE* - When in doubt:
   - Assumed patterns are "acceptable" without verification
   - Similar but not verified identical
...
| Function | Rust | TIL | Status | Notes |
|----------|------|-----|--------|-------|
| emit | Y (pub) | Y | DIVERGE | collect_nested_func_info uses "" vs None - needs review |
| emit_struct_declaration | Y | Y | DIVERGE | Uses continue vs if block - needs review |
