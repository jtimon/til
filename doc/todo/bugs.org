...
*** Known gcc warnings
- Bug #94 documents some warnings in bug47.til compilation
- Need to audit all generated C code for warnings
...
- ~gdb~ with breakpoints to inspect variable values
- Look at ~gen/c/test/*.c~ for generated code

*** Current Fix Plan

One issue per commit. Don't try to fix everything at once.
...
Current test: ~src/examples/hello_script.til~
...
**** Issues Found in hello_script.til Diff
1. ~til_Array_get~ missing arguments - til generates ~til_Array_get(&_err0)~ but rstil
   generates ~til_Array_get(&_err0, array, &index, (til_Dynamic*)&out)~
2. String literals encoded as numbers - til generates numeric encoding like
   ~"1151149947..."~ instead of readable ~"src/core/exit.til:13:5:"~
3. Function names emitted as variables - ~til_Str x = til_panic;~ treats function as value
4. Extra Array operations - til generates extra ~Array_new~ and ~Array_set~ calls
5. Compound literals vs intermediate variables - til uses ~&(til_I64){expr}~ inline,
   rstil uses ~til_I64 _tmp = expr; &_tmp~
