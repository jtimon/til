...
** Bug #176: UFCS .clone() doesn't resolve on create_alias variables
:PROPERTIES:
:DISCOVERED: 2026-02-19
:IMPACT: Type error at compile time -- workaround is to use Type.clone(v) function call style
:END:

When a variable is created via ~create_alias(v, Type, ptr)~, calling ~v.clone()~ fails with:
~type ERROR: Could not find function definition for 'v.clone'~

The type checker doesn't resolve UFCS method calls on create_alias variables. The same
issue likely affects other UFCS method calls beyond .clone().

Workaround: Use explicit function call style ~Type.clone(v)~ instead of ~v.clone()~.
...
:STATUS: Mostly fixed - frame iteration zero-copy, Map.get within lookups still copies
:END:

*** What was done

The original bug was that all ScopeStack lookup/has functions copied entire
ScopeFrame objects (5 Maps each) on every iteration. Three rounds of fixes:

**** Option C: has_* methods (2026-01-29)
Added has_var, has_symbol, has_func, has_enum, has_struct methods to ScopeStack.
Updated call sites using lookup_*.is_some() pattern to use has_* instead.
Avoided copying definition objects but still copied ScopeFrame itself.

**** Categories A+B: Zero-copy Expr.get() (2026-02-14)
...
490 Expr copy calls eliminated.

**** Category C: Zero-copy ScopeFrame iteration (2026-02-14)
All 23 ScopeStack lookup/has functions converted from:
#+BEGIN_SRC til
// OLD - copies entire ScopeFrame (5 Maps) per iteration
mut frame := ScopeFrame()
self.frames.get(i, frame)
#+END_SRC
to:
#+BEGIN_SRC til
// NEW - zero-copy pointer alias into Vec storage
frame_ref := self.frames.get_by_ref(i)?
create_alias(frame, ScopeFrame, frame_ref.data)
#+END_SRC

Only 2 non-iteration ~frames.get()~ calls remain (top frame, global frame).

*** Remaining (not part of this bug)
Map.get (Category D) and Vec.get on non-params Vecs (Category E) still copy.
These are general collection semantics issues, not ScopeStack-specific.
