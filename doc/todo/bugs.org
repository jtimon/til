...
*** Step 4: Remove INFER_TYPE handling from interpreter

Files: src/rs/interpreter.rs, src/self/interpreter.til

Per Bug #128 notes:
- Line 1187: `if decl.value_type != INFER_TYPE` check
- Lines 1200, 1231, 1243, 1416, 1749: Pattern matches on INFER_TYPE
- TIL: Lines 1236, 1271, 1433, 1449, 1548, 1902

Replace INFER_TYPE checks with assertions/panics.

Then: make benchmark, update progress, commit.

*** Step 5: Review/fix INFER_TYPE in precomp
...
Note: precomp creates NEW declarations (for-in desugaring). These should use
resolved types from the iterator, not INFER_TYPE.
