...
*** Example
#+BEGIN_SRC til
// Current syntax - type is inferred
for i in 0..100 {
    // i is inferred as I64
}

// Desired syntax - explicit type annotation (NOT currently supported)
for i: U8 in 0..100 {
    // i is explicitly U8
}
#+END_SRC
...
For range loops, adding optional type annotation should be straightforward:
- Parser already handles ~for VAR: TYPE in COLLECTION~ for collection iteration
- Range loops could accept the same syntax
- Implementation: just add a type check when explicit type is provided

True for-in loops (collection iteration) require explicit types because
type inference is harder. Range loops already infer I64, so adding optional
explicit types is just an additional type check.
