...
** Bug #73: TTypeDef value_type stored as TStructDef instead of TEnumDef in til interpreter
:PROPERTIES:
:DISCOVERED: 2026-01-01
:IMPACT: Cannot import self.init with til interpret (blocks bug47 test)
:STATUS: Active
:END:

*** Description
When running ~./bin/til interpret~ on files that import ~self.init~, the type checker
fails with "Struct 'TTypeDef' not found" at init.til:826. TTypeDef is an enum defined
in parser.til but when looked up, its value_type is ~TType(TStructDef)~ instead of
~TType(TEnumDef)~, causing lookup_struct to be called instead of lookup_enum.
...
Running: ~./bin/til interpret test.til~
Error: ~src/self/init.til:826:36: init ERROR: Struct 'TTypeDef' not found~

Note: ~./bin/rstil interpret test.til~ works correctly.

*** Analysis
1. TTypeDef is defined in parser.til as ~enum { TEnumDef, TStructDef }~
2. When parser.til is imported, init_context should store TTypeDef with value_type=TType(TEnumDef)
3. Later, when typer processes init.til code like ~ValueType.TType(TTypeDef.TEnumDef)~,
   it looks up TTypeDef and finds value_type=TType(TStructDef) instead
4. The switch case for TStructDef is matched, calling lookup_struct("TTypeDef")
5. That fails because TTypeDef is an enum, not a struct

*** Investigated Areas
- C code for get_value_type EnumDef case (line 14330-14332): Returns correct TType(TEnumDef)
- C code for init_context TEnumDef case (line 16998-17050): Pattern match and declare_symbol look correct
- value_type_clone: Correctly copies TType variants
- declare_symbol/Map_set/Vec_push: All appear to use memcpy correctly
- lookup_symbol/Map_get: Retrieval looks correct

*** Suspected Cause
The value_type is being corrupted somewhere between declaration and lookup. Possible causes:
- Memory corruption in Map storage/retrieval
- Compound literal lifetime issues in C codegen
- Type size mismatch causing partial copies

*** Workaround
None - any file importing self.init fails with til interpret.
