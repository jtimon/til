...
TestStruct := struct {
    mut ptr_field: Ptr = Ptr()
}

_s := TestStruct(ptr_field=0)  // Only usage is in global
#+END_SRC

Generated C code has:
#+BEGIN_SRC c
static til_TestStruct _s;  // ERROR: til_TestStruct not defined
#+END_SRC

But the struct definition is never emitted.

*** Affected Modes
- rs_compiled: FAIL (gcc error)
- til_compiled: FAIL (gcc error)
- rs_interpreted: PASS
- til_interpreted: PASS

*** Likely Cause
The struct scavenger is too aggressive - it removes struct definitions that appear
unused, not realizing they're needed for global variable declarations. If not the
scavenger, then ccodegen itself is likely to blame for not emitting the definition.

*** Reproducer
Run: ~rstil run src/test/bug139.til~

Currently fails with gcc error "unknown type name 'til_TestStruct'".

*** When Fixed
In ~src/tests.til~, move bug139.til entries from rs_interpreted/til_interpreted
to all_common (after Bug #139 is also fixed with expected_status=1).
...
*** Reproducer
Run: ~rstil interpret src/test/bug139.til~

Currently exits 0 (no error). Once fixed, should exit 1 with type error.

*** When Fixed
In ~src/tests.til~, update the two entries for bug139.til:
- Add ~expected_status=1~ to both rs_interpreted and til_interpreted entries
- Move to all_common once Bug #150 is also fixed (compiled modes currently fail)
