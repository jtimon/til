...
3. *Forward declarations missing*
   - rstil emits forward declarations before ~#include <ext.c>~
   - til doesn't emit them at all

4. *~#include <ext.c>~ vs inline helpers*
   - rstil: ~#include <ext.c>~
   - til: emits inline helpers (u8_to_i64, single_print, etc.) instead
   - Should use #include <ext.c> like rstil

5. *Constants missing til_ prefix*
...
7. *Temp variable naming*
   - rstil: ~_tmp_til_U8_from_i64_19~ (incrementing counter)
   - til: ~_tmp_U8_from_i64__tmp_~ (reuses same name, causes conflicts)
...
8. *Function call argument passing*
   ...
   - Follows from #6 - if param is pointer, must use &
