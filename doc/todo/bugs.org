...
NOTE: FuncNameResult removed (2026-01-05) - get_func_name_string and get_til_func_name_string
now throw KeyNotFoundError instead of returning result struct. Callers use try/catch pattern.

Attempted to remove FuncLookupResult (2026-01-02) by changing get_fcall_func_def
to throw KeyNotFoundError instead of returning result struct. All 18 call sites were
updated mechanically. Result: til_compiled tests segfaulted. Reverted. Needs deeper investigation.
...
| get_func_name_string | Y | Y | MATCH | Rust Option -> TIL throws KeyNotFoundError per rs2til.org |
| get_til_func_name_string | Y | Y | MATCH | Rust Option -> TIL throws KeyNotFoundError per rs2til.org |
