...
** 2. Don't use unwrap()
~unwrap()~ is hidden ~panic!()~. Use ~?~ or ~match~ instead.
- ~?~ propagates errors (translates to TIL's implicit throw propagation)
- ~match~ handles cases explicitly (translates to TIL's ~switch~)
