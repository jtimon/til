...
| Preinit   | AST            | AST            | Auto-generate delete/clone methods         |
...
| Garbager  | AST, Context   | AST            | Insert clone/delete calls for memory mgmt  |
...
*** Preinit (src/rs/preinit.rs, src/self/preinit.til)
- Auto-generates delete() and clone() methods for structs and enums
- Issue #108: Methods are generated into namespace blocks, not struct definitions
- Runs before Init so that auto-generated methods are registered in scope
- Also runs during Init for imported files (init_import_declarations)
- Entry: preinit_expr()
...
- Desugars syntactic sugar constructs:
  ...
  - switch: switch/case -> nested if/else (if a {} else { if b {} else { ... } })
- Runs after typer because AST is annotated with resolved types
- Entry: desugar_expr()
- Performance note: switch desugars into nested if/else, NOT a flat if/else-if
  chain. The nested structure may enable better gcc optimization. When the TIL
  interpreter's ext func dispatch was converted from if/else-if to switch, the
  til_interpreted benchmark improved by ~2.8% (621s -> 604s). Prefer switch over
  long if/else-if chains for string dispatch in hot paths.
...
*** Garbager (src/rs/garbager.rs, src/self/garbager.til)
- Inserts clone() calls for copy params and struct literal fields (Issue #159)
- Strips dont_delete() annotations and collects protected var names
- Runs after UFCS so that all method calls are resolved
- Entry: garbager_expr()
...
Build pipeline:
  Lexer -> Parser -> Mode -> Preinit -> Init -> Typer -> Desugarer -> UFCS -> Garbager -> Precomp -> Scavenger -> Build -> GCC

Interpret pipeline:
  Lexer -> Parser -> Mode -> Preinit -> Init -> Typer -> Desugarer -> UFCS -> Garbager -> Precomp -> Interpret
