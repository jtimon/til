...
| Garbager  | AST, Context   | AST            | Auto-generate delete methods, ASAP destroy |
...
- Auto-generates delete() methods for structs that lack them
- Inserts automatic delete() calls for ASAP destruction
- Runs after desugarer, before UFCS:
  - After desugarer: desugared constructs may need delete calls
  - Before UFCS: generated x.delete() calls get transformed by UFCS
- Entry: garbager_expr()

*** UFCS (src/rs/ufcs.rs, src/self/ufcs.til)
- Desugars uniform function call syntax:
  - x.method(y) -> Type.method(x, y)
- Runs after garbager because garbager generates UFCS calls
- Entry: ufcs_expr()
...
Build pipeline:
  Lexer -> Parser -> Mode -> Init -> Typer -> Desugarer -> Garbager -> UFCS -> Precomp -> Scavenger -> Build -> GCC

Interpret pipeline:
  Lexer -> Parser -> Mode -> Init -> Typer -> Desugarer -> Garbager -> UFCS -> Precomp -> Interpret
...
- context.imports_garbager_done - Files that completed garbager phase
