...
| Desugarer | AST, Context   | AST            | Desugar syntactic sugar (switch, for-in)   |
| Garbager  | AST, Context   | AST            | Auto-generate delete() methods for structs |
...
- Desugars syntactic sugar constructs:
  ...
  - switch: switch/case -> if/else chains (Issue #110)
- Runs after typer because AST is annotated with resolved types
- Entry: desugar_expr()

*** Garbager (src/rs/garbager.rs, src/self/garbager.til)
- Auto-generates delete() methods for structs that don't have one
- Calls field.delete() for each mutable field in reverse declaration order
