...
- ✅ Interpreter: Invalidates caller's variable after passing to own param
- ✅ All own tests in src/test/args.til passing (lines 494-621)

Still missing:
- ❌ Use-after-move at typer phase: Interpreter catches at eval, compiler doesn't catch at all
  - Fix: Mirror interpreter - remove symbol from scope when passed to own param (Bug #49)
- ⚪ Call-site `own` syntax: `func(own x)` not parsed (optional - for code clarity only)
...
** 🔮 Future: Owned Struct Fields (Issue #115)
