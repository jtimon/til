...
#+DATE: 2026-02-19
#+RELATED: Issue #91, #105, #106, #107, #108, #161, #181
...
** Some types have literal syntax

Some types in TIL have special literal syntax:

| Type        | Literal syntax                              | Value type   |
|-------------+---------------------------------------------+--------------|
| ~I64~       | ~69~                                        | I64          |
| ~U8~        | ~5~ (with type annotation)                  | U8           |
| ~Str~       | ~"hello"~                                   | Str          |
| ~StructDef~ | ~struct { mut x: I64 = 0 }~                 | StructDef    |
| ~EnumDef~   | ~enum { Red, Green, Blue }~                 | EnumDef      |
| ~FuncSig~   | ~func(I64, I64) returns I64 {}~              | FuncSig      |
| ~FuncDef~   | ~func(a: I64, b: I64) returns I64 { ... }~  | FuncDef      |

Many types do NOT have literal syntax: ~Bool~ uses constants (~true~, ~false~),
user-defined structs use struct literals (~Point { x = 1, y = 2 }~), and types
like ~Vec~, ~Map~, ~Arena~ are instantiated via ~.new()~.

The key insight for first-class types is that ~struct { ... }~ is a literal
that produces a StructDef value, just as ~69~ is a literal that produces an I64
value. This means type definitions can appear anywhere a value can: as function
arguments, in collections, as return values.

#+BEGIN_SRC til
// These are all the same pattern: name := literal
x := 69
name := "hello"
...
// And since they're values, they can go anywhere values go:
mut defs := Vec.new(StructDef)
defs.push(own struct { mut x: I64 = 0, mut y: I64 = 0 })
#+END_SRC

For the compiler, this means the codegen emits type definition literals as
compound literals of their runtime struct type (e.g. ~til_StructDef~ for a
struct literal, with its ~members~ Vec, ~default_values~ Map, and ~ns~
NamespaceDef populated).
...
// Function definitions: FuncDef literal (type inferred as FuncDef)
add := func(a: I64, b: I64) returns I64 { return a.add(b) }

// Or with explicit FuncSig as type (FuncSig form)
...
The distinction between FuncSig and FuncDef mirrors struct/enum:
...
A bodyless func/proc is a FuncSig (type definition). A bodied func/proc is a
FuncDef (value). The presence of a body (and parameter names) distinguishes them.
...
** Definition: FuncSig form vs FuncDef form

There are two ways to define a function, reflecting the two metatypes:

*** FuncSig form

The type between ~:~ and ~=~ is a FuncSig (pure types, no names). The value
after ~=~ provides argument names and body:

#+BEGIN_SRC til
// FuncSig form: type is the signature, value is names + body
...
This is the explicit form -- the FuncSig is the type, the rest is the value.
Internally, FuncSig always stores just types -- parameter names live in the
body binding.

*** FuncDef form

The whole ~func(names: Types) returns Type { body }~ is a FuncDef literal,
just as ~struct { ... }~ is a StructDef literal:

#+BEGIN_SRC til
// FuncDef form: the whole thing is a FuncDef literal, type inferred
add := func(a: I64, b: I64) returns I64 { return a.add(b) }

// Same with explicit metatype:
add : FuncDef = func(a: I64, b: I64) returns I64 { return a.add(b) }
#+END_SRC

This is the natural way to define a function. The literal includes names
and body -- it is the same syntax used for lambdas (anonymous FuncDef
literals). ~:=~ infers the type as FuncDef, just as ~Point := struct { ... }~
infers StructDef.
...
FuncDef literal form -- the parser extracts argument names and body to
produce the same internal representation.

** More examples
#+BEGIN_SRC til
// With throws -- FuncSig form:
...
#+BEGIN_SRC til
// Anonymous FuncDef literal (lambda):
apply(func(a: I64, b: I64) returns I64 { return a.add(b) }, 3, 5)

// Same syntax works for named definitions (FuncDef form):
double := func(a: I64) returns I64 { return a.mul(2) }
#+END_SRC

** Stored in collections

Collections of functions come in two forms:

#+BEGIN_SRC til
// Vec of function pointers with the SAME signature:
// Each element is a different function definition implementing BinaryOp
ops := Vec.new(BinaryOp)
ops.push(add)
ops.push(mul)
ops.get(0)(3, 5)  // 8

// Vec of function SIGNATURES (each can be different):
sigs := Vec.new(FuncSig)
// Each element is a different signature type

// Vec of function DEFINITIONS (each can be different):
defs := Vec.new(FuncDef)
// Each element is a different function definition (any signature)
#+END_SRC

This parallels how structs and enums work:
- ~Vec.new(Point)~ stores Point instances (same type, different values)
- ~Vec.new(StructDef)~ stores struct definitions (different types)
- ~Vec.new(Direction)~ stores Direction instances (same type, different values)
- ~Vec.new(EnumDef)~ stores enum definitions (different types)
...
Lambda functions are anonymous FuncDef literals passed directly as arguments.
They use the same syntax as named FuncDef definitions, just without a name:
...
// Same in FuncSig form (type separated from param name)
...
// Inline enum as argument to macro (Issue #106)
count_variants : macro(T: Type) returns I64 = {
    def := enum_def_of(type_as_str(T))
    return def.variants.len()
}
assert_eq(loc(), 3, count_variants(enum { Red, Green, Blue }))
assert_eq(loc(), 2, count_variants(enum { None, Some: I64 }))
#+END_SRC

Anonymous enums passed as inline arguments are expanded by
=expand_anon_enum_fcalls_recursive= in precomp, which registers them as
=AnonEnumN= with auto-generated delete/clone methods and replaces the
EnumDef node with an Identifier.
...
| TIL        | ~enum { Circle(radius: I64) }~ | Yes (inline args)        |

* Bodies (Issue #181)

A Body is a sequence of statements -- what other languages call a "block".
They already exist everywhere in TIL: function definitions have one, ~if~
has one or two (with else), ~switch~ can have many (one per case), ~while~
and ~for~ have one each. But today a Body is always attached to a control
structure. What if it were a first-class value?

** What a Body is

A Body is zero or more statements executed in sequence. It is the simplest
unit of executable code -- simpler than a function (no signature, no
parameters, no return type). A Body is essentially a zero-argument, void
closure with no captures.

#+BEGIN_SRC til
// A Body in its natural habitat (inside an if):
if condition {
    x = x.add(1)
    println(x)
}

// The stuff between { } is the Body: two statements.
#+END_SRC

** First-class bodies: store, pass, return

Making Body a first-class value means treating it like any other value:
...
make_greeting : func(name: Str) returns Body = {
    return { println("hello", name) }  // captures name -- needs #182
}
...
steps := Vec.new(Body)
steps.push({ println("step 1") })
steps.push({ println("step 2") })
for step in steps {
    step()
}
#+END_SRC
...
A Body is simpler than a function:

| Construct         | Has params | Has return type | Has captures | Has body |
|-------------------+------------+-----------------+--------------+----------|
| Body              | No         | No              | No*          | Yes      |
| func (no closure) | Yes        | Yes             | No           | Yes      |
| func (closure)    | Yes        | Yes             | Yes (#182)   | Yes      |

*Without captures (#182), a Body can only access globals. With captures,
...
// Body with capture list (depends on #182)
summer : Body = [mut total, data] {
    for item in data {
        total = total.add(item)
    }
}
...
anonymous, inline version. In fact, ~proc()~ with no arguments and no
return value is almost the same thing as a Body. The distinction is that
a Body has no function overhead (no frame push/pop in the interpreter, no
C function call in codegen) -- it executes in the caller's scope.

** Use cases

*** Deferred execution (cleanup, finally)
#+BEGIN_SRC til
// Like Go's defer, but explicit
cleanup := { file.close() }
// ... do work ...
cleanup()
#+END_SRC
...
*** Test setup/teardown
#+BEGIN_SRC til
run_test : proc(name: Str, setup: Body, test: Body, teardown: Body) = {
...
*** Option A: Inline (shared scope)
The Body executes as if its statements were pasted at the call site.
It can see and modify all local variables. This is how bare ~{ }~ blocks
work today.

Pro: simple, powerful, zero overhead.
Con: passing a Body as argument is weird -- which scope does it execute in?

*** Option B: Isolated (own scope, captures only)
The Body gets its own scope. It can only access what it captures (requires
#182). This is safer and more predictable for stored/passed bodies.

Pro: predictable, composable, no spooky action at a distance.
Con: needs capture lists (#182) to be useful.

*** Option C: Both
Bare ~{ }~ blocks are inline (current behavior). Body values (stored,
passed, returned) are isolated and require captures. The distinction is
structural: a bare block in a control flow position is inline; a Body
assigned to a variable or passed as an argument is isolated.

This is probably the right answer. It matches how most languages work:
inline blocks share scope, callbacks/closures are isolated.

** Comparison with other languages

| Language   | Block-as-value             | Notes                             |
|------------+----------------------------+-----------------------------------|
| Ruby       | ~{ \vert\vert ... }~ / ~do ... end~ | Blocks and Procs are first-class |
| Smalltalk  | ~[ ... ]~                  | Blocks are objects                |
| Swift      | ~{ () -> Void in ... }~    | Closures (always have captures)   |
| Kotlin     | ~{ ... }~ (lambda)         | Trailing lambda syntax            |
| Zig        | No first-class blocks      | Comptime blocks are special       |
| Rust       | ~\vert\vert { ... }~ (closure) | No bare blocks as values      |
| Go         | ~func() { ... }()~        | Must wrap in function             |
| TIL        | ~Body = { ... }~ (proposed)| Simpler than closure              |
...
block into a first-class value. TIL's Body would be similar to Ruby's
Proc but without implicit passing.

** Implementation considerations

- Parser: Body literals ~{ ... }~ are already parsed. The new part is
  allowing them in expression position (RHS of ~:=~, function arguments,
  return values).
- Typer: new ~Body~ type (or reuse ~proc()~ signature internally).
- Interpreter: a Body value is just a reference to an AST node list.
  Executing it means evaluating those nodes in the appropriate scope.
- Ccodegen: a Body could compile to a C function with no arguments and
  void return, or to an inlined block depending on scope semantics.
- Scavenger: must track what a Body references to keep those alive.
