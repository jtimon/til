...
BinaryOp : FuncSig = func(a: I64, b: I64) returns I64
...
BinaryOp := func(a: I64, b: I64) returns I64           // infers FuncSig
...
#+BEGIN_SRC til
Type := enum { StructDef, EnumDef, FuncSig }
#+END_SRC
...
| ~FuncSig~   | Calling convention      | ~add := func(...){...}~ is a BinaryOp |

A function definition is an instance of its signature, just like a struct
value is an instance of its struct definition. This is consistent across
all three.
...
Vec.new(BinaryOp)   // Vec of function definitions with the same signature (aka for the same function pointer)
Vec.new(FuncSig)    // Vec of function signatures (different signatures)
Vec.new(StructDef)  // Vec of struct definitions
Vec.new(Type)       // Vec of any type definition (structs, enums, funcsigs)
#+END_SRC
...
| ~BinaryOp := func(a: I64) ret I64~  | ~FuncSig~ (or ~Type~) (no body)               |
| ~add := func(a: I64) ret I64 {...}~ | ~func(a: I64) returns I64~ (has body = value) |
...
// Or assigned to a variable with type inference:
...
| TIL        | ~func(a: I64, b: I64) returns I64~ | Param names required (for now) |

TIL keeps param names in the type. This is verbose but readable. Could make
names optional in type position later: ~func(I64, I64) returns I64~.
...
consume := func() [own data] { data.clear() }
#+END_SRC
