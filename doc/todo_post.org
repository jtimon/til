...
** TODO Reconsider Bug #8 Fix Approach After Self-Hosting              :HIGH:
:PROPERTIES:
...
*** Current Approach
Bug #8 (uncaptured return values propagate up call stack) is currently being "fixed" by requiring all return values to be explicitly captured with `_ := func()` pattern. This makes the propagation behavior less of a footgun but doesn't change the underlying unintuitive behavior.

*** Decision Deferred
We need to decide whether to:

1. **Keep current approach**: Require return values to be assigned (enforced by type checker)
   - Pros: Explicit, catches errors, works with current propagation behavior
   - Cons: Verbose, unintuitive for side-effect calls

2. **Fix propagation behavior**: Change rstil to discard uncaptured return values (like most languages)
   - Pros: Intuitive, matches expectations from other languages
   - Cons: Requires rstil changes, may break existing code assumptions

*** Why Deferred
After self-hosting, we'll have more TIL code to evaluate which approach feels better in practice. For now, the workaround (`_ :=`) unblocks parser.til development.

*** Action Required
After self-hosting is achieved:
- Review how often `_ :=` is used in practice
- Gather feedback on which approach feels more natural
- Make final decision and implement chosen approach
- Update documentation and language spec
...
*** Current Behavior (Tested 2025-01-10)
...
- [ ] Add test cases for namespace imports
