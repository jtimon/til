...
- ~has_field(T: Type, field: Str) -> Bool~: Checks if type has a field.
- ~field_value(T: Type, field: Str) -> Dynamic~: Gets comptime value (for consts).
- ~is_struct(T: Type) -> Bool~: Type predicates (e.g., ~is_primitive~, ~is_generic~).
...
Til implements generics via macros, desugaring definitions to macro calls (C++-like syntax as baseline, with potential Jai-ish sugar like ~for T in Type~). Usage instantiates by calling the macro with concrete types.

*** Explicit Templates (~<T>~)
Generics desugar to macro invocations for monomorphization.

#+BEGIN_SRC til
Vec<T> := generic_vec_macro(T)  // Desugars to macro; returns StructDef
...
-- Macro guards: if is_concrete(T) { mono } else { DynamicVec }
#+END_SRC
...
This yields zero-cost statics by default, dynamic fallbacks for size/perf tradeoffs (via ~--prefer-dynamic~ flag). Post-expansion type check validates.
