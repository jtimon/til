...
All types in TIL must implement certain fields and methods to ensure the language runtime can work with them correctly. This document describes these mandatory requirements.

* Mandatory Constant Field: type_size
...
I64 := struct {
    type_size: I64 = 8
    // I64 occupies 8 bytes
}

Bool := struct {
    mut data: U8 = 0
    type_size: I64 = 1
    // Bool is stored as U8, so 1 byte
}

Str := struct {
    mut c_string: I64 = 0  // pointer (8 bytes)
    mut cap: I64 = 0       // capacity (8 bytes)
    type_size: I64 = 16    // total: 16 bytes
}
#+END_SRC
...
- =doc/human/ufcs.org= - Explains how methods like =.size()= can be called with UFCS
