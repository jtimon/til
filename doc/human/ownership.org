...
TIL's default (no keyword) ≈ Mojo's `ref`
...
| Mutable param           | mut (ref)      | mut (ref)           | ✅ Same                             |
| Copy param              | var (copy)     | copy                | ✅ Same concept                     |
| Ownership transfer      | var with ^     | own (at call site)  | TIL: opt-in at call, not signature  |
| Mut local               | var x = y      | mut x := y          | ✅ Equivalent (both copy)           |
...
|--------------------+---------------------+------------------------------------|
| x := y             | ref x = y           | **TIL's default ≡ Mojo's ref**     |
...
** ✅ Phase 1: Immutability Enforcement (COMPLETE)
...
** ✅ Bug #25 Fix: Declaration Semantics (COMPLETE)
...
** ⚠️ In Progress: Ownership Transfer (Partial Implementation)
Status: Basic implementation done, but has bugs

Completed (2025-11-12):
- ✅ Lexer: Added `own` token
- ✅ Parser: Accepts `own` in parameter declarations (mutually exclusive with mut/copy)
- ✅ Type checker: Allows modification of `own` parameters
- ✅ Interpreter: Basic ownership transfer logic for identifiers

Bugs found:
- ❌ Infinite loop when running own parameter tests
- ❌ Field access on own parameters doesn't work (v.x fails to resolve)
- ❌ Use-after-move checking not implemented

Current behavior:
```til
consume_i64 := proc(own n: I64) { ... }  // Compiles
x := 42
consume_i64(x)  // Transfers ownership (when bugs are fixed)
```

Tests added but disabled in src/test/args.til (lines 500-636)

** 🔮 Future: Move-Only Types (Maybe)
