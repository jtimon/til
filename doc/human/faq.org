...
Yes, to both. Til eschews traditional classes and inheritance in favor of lightweight structs with associated functions, promoting data-oriented design (à la Rust or Odin) for cache-friendly, low-level control. Universal Function Call Syntax (UFCS) allows method-like chaining (e.g., `vec.push(42)`), providing object-oriented ergonomics as a stylistic choice without the overhead of vtables or polymorphism pitfalls.
