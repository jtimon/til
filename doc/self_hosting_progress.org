...
|-------------+-----------+------------+------------+---------------------|
| *TOTAL*     |     4,175 |      7,437 | *56%*      | ~40% by function    |
...
*Key Differences:*
- *TIL is MORE VERBOSE*: Uses explicit if/switch where Rust uses pattern matching
- *TIL advantage*: Better error messages with suggestions (e.g., "use 'func' instead of 'fn'")
- *Rust advantage*: More concise with Rust's match expressions

*TODOs in Rust:*
- Implement =throws= parsing properly
- Implement =for=/=in= parsing
- Implement =match= statement
- Better error messages for forbidden words

*Recommendation:*
- Direction: TIL → Rust (backport better error messages)
- This component is READY for homologization
- Focus: Remove TIL verbosity by improving pattern matching support

*** Homologization Tasks
- [ ] Backport error messages from TIL to Rust (lexer.til lines 605-670)
- [ ] Reduce TIL verbosity using switch pattern matching
- [ ] Validate token output equivalence with test suite
...
- [ ] Backport error messages TIL → Rust
- [ ] Reduce TIL verbosity
