...
*** Status: ⚠️ DIVERGED - NEEDS HOMOGENIZATION (Next Priority)

*IMPORTANT:* parser.rs is the **source of truth**. parser.til must be updated to match it.

*Current State (2025-01-09):*
The parsers have **unintentionally diverged** during recent StrMap→Map refactoring work.
...
*Updated Analysis (2025-01-09 post Bug #6 fix):*
Comprehensive comparison reveals **critical missing infrastructure** not in original plan:
- ❌ Mode system completely missing (ModeDef, parse_mode, mode_from_name, can_be_imported)
- ❌ parse_tokens() entry point missing (no public API to invoke parser)
- ❌ func_proc_returns() and func_proc_throws() not extracted as separate functions
- ⚠️ 13 out of 18 tests in test_parser.til now passing (after Bug #6 fix)
- ⚠️ 3 tests skip due to stack overflow in parser.til itself (not rstil bugs)

*Current TIL Implementation (1,471 lines) - NEEDS FIXES:*
- ❌ Uses generic =Map= (string→string) for type storage (WRONG)
- ❌ Uses =Array= for collections with string-based types (WRONG)
- ❌ Simplified data structures don't match Rust (WRONG)
- ⚠️ Recent change broke homogenization: Replaced StrMap with Map
...
1. **PHASE 0 (NEW)**: Add missing critical infrastructure (~6 hours)
   - Add Mode system (ModeDef, parse_mode, mode_from_name, can_be_imported)
   - Add parse_tokens() public entry point
   - Extract func_proc_returns() and func_proc_throws() as separate functions
2. **PHASE 1**: Fix data structure definitions (~5 hours)
   - ValueType enum payloads, SEnumDef, SStructDef, SFuncDef
3. **PHASE 2**: Update helper functions (~4 hours)
   - str_to_value_type, value_type_to_str, get_full_identifier_name
4. **PHASE 3**: Update parse functions (~10 hours)
   - parse_func_proc_args, parse_func_proc_definition, parse_struct_definition, enum_definition
5. **PHASE 4**: Cleanup & documentation (~4 hours)
   - Remove old bug workarounds, align comments
6. **PHASE 5**: Validation & testing (~8 hours)
   - Structural comparison, edge cases, full test suite

***Total Estimated Time: 37 hours (5 days)*** - up from original 22 hours
...
- [ ] NEXT: Execute Phase 0 - Add missing infrastructure
- [ ] Execute Phases 1-3 - Data structures and parse functions
- [ ] Execute Phases 4-5 - Cleanup and validation
