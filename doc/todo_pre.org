...
*** Problem
Test catch blocks should print "ERROR: " prefix for unexpected errors to make test failures more visible, while still using `return` to continue to the next test instead of aborting the entire test suite.
...
- Using `return` instead of `exit(1)` allows remaining tests to run
- Helps distinguish expected errors from unexpected ones in test logs

*** Scope
Review and update catch blocks throughout test files:
- src/test/arrays.til
- src/test/dynamic_arrays.til
