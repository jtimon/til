...
|---------------------+---------------------------------+-------------------------------------------------|
| Known Bugs          | ✅ ALL FIXED                    | Bug #8 fixed with compile-time enforcement      |
...
| Parser Homogenization | ✅ UNBLOCKED                    | Ready to proceed with parser.til work           |
...
No active bugs at this time. All bugs (#1-#8) have been fixed. See [[file:bugs.org][bugs.org]] for details.

** DONE Fix Bug #8: Uncaptured Return Values Propagate Up Call Stack        :CRITICAL:
:PROPERTIES:
:DISCOVERED: 2025-11-10
:FIXED: 2025-11-10
:FILE: src/rs/typer.rs
:IMPACT: Unblocks parser.til homogenization
:RELATED: Parser homogenization
:END:

*** Solution Implemented
Fixed with **compile-time enforcement** via type checker. All function calls that return values must now be explicitly used or discarded.
...
- Provides helpful error messages when return values are not captured
...
*** Why This Approach
Rather than fixing runtime propagation behavior, compile-time enforcement:
- Catches errors earlier (at compile time)
- Makes intent explicit (use vs discard)
- Aligns with modern language best practices
- Prevents subtle bugs from uncaptured return values

*** Files Modified
- =src/rs/typer.rs=: Added context-aware type checking
- =src/test/errors.til=: Fixed one uncaptured return value
- =src/test/return_value_usage.til=: Created comprehensive test suite

*** Impact
- ✅ Parser.til homogenization UNBLOCKED
- ✅ All tests passing
- ✅ Explicit return value handling improves code safety
