...
|---------------------+---------------------------------+-------------------------------------------------|
| Known Bugs          | ✅ BUG #7 FIXED                 | Enum return values now handled correctly        |
| Error Handling      | ✅ WORKING                      | throw/throws/catch fully functional             |
| Import System       | ✅ WORKING                      | Basic imports work in all modes                 |
| Test Suite          | ✅ ALL TESTS PASSING            | All tests pass including enum payloads          |
| Type System         | ✅ ENUM RETURNS WORK            | Functions can return enums with payloads        |
| Parser Homogenization | 🔄 IN PROGRESS                 | Translating from parser.rs line by line         |
...
** BUG Method return causes early return from caller                      :CRITICAL:
:PROPERTIES:
:DISCOVERED: 2025-01-10
:IMPACT: Cannot use methods that return values
:RELATED: Methods, Return statements
:STATUS: DOCUMENTED
:END:

*** Problem
When a method returns a value, it causes the CALLING function to return instead of just the method.
...
parse_func := func(mut lexer: Lexer) {
    println("Before expect")
    lexer.expect(TokenType.LeftParen)  // This line causes parse_func to return!
    println("After expect")  // This line never executes
}
#+END_SRC
...
#+BEGIN_SRC til
lexer_expect := func(mut lexer: Lexer, expected: TokenType) returns Token {
    // Same implementation
}

// Call as: lexer_expect(lexer, TokenType.LeftParen)
// Instead of: lexer.expect(TokenType.LeftParen)
#+END_SRC

*** Impact
- Cannot use Lexer.expect() method in parser.til
- Must use standalone lexer_expect() function
- Affects any method that needs to return a value
