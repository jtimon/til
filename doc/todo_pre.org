...
|---------------------+---------------------------------+-------------------------------------------------|
| Known Bugs          | ❌ BUG #8 ACTIVE                | Uncaptured returns propagate up call stack      |
| Error Handling      | ✅ WORKING                      | throw/throws/catch fully functional             |
| Import System       | ✅ WORKING                      | Basic imports work in all modes                 |
| Test Suite          | ⚠️ PARTIAL                     | Core tests pass, parser blocked by Bug #8       |
| Type System         | ✅ ENUM RETURNS WORK            | Functions can return enums with payloads        |
| Parser Homogenization | ❌ BLOCKED                     | Blocked by Bug #8 (return propagation)          |
...
** TODO Fix Bug #8: Uncaptured Return Values Propagate Up Call Stack        :CRITICAL:
:PROPERTIES:
:DISCOVERED: 2025-01-10
:FILE: src/rs/interpreter.rs
:IMPACT: Blocks parser.til and requires workarounds everywhere
:RELATED: Parser homogenization
:END:

*** Problem
When calling a function/method that returns a value without capturing that value in an assignment, the return value propagates up the call stack, causing the CALLER to also return with that value.

Example that fails:
#+BEGIN_SRC til
parse_func_proc_args := func(mut lexer: Lexer) returns Array {
    lexer_expect(lexer, TokenType.LeftParen)  // Returns Token, not captured
    // Function returns HERE with the Token value!
...
*** Desired Behavior
Uncaptured return values should be discarded:
#+BEGIN_SRC til
parse_func_proc_args := func(mut lexer: Lexer) returns Array {
    lexer_expect(lexer, TokenType.LeftParen)  // Should discard Token return value
    println("This should execute")            // Should continue normally
    ...
}
#+END_SRC

*** Current Workaround
Must capture ALL return values, even when not needed:
#+BEGIN_SRC til
    _ := lexer_expect(lexer, TokenType.LeftParen)  // Works
#+END_SRC

*** What Needs to be Fixed
- Modify rstil's interpreter.rs to check if a function/method call is part of an assignment
- If NOT part of an assignment, discard the return value instead of propagating it
- This is the desired behavior and was simply not a priority to implement yet

*** Impact
- Blocks parser.til homogenization completely
- Requires verbose `_ := func()` workarounds throughout codebase
- Makes TIL code unintuitive and hard to debug
...
** DONE Fix Str handling in catch blocks                                  :HIGH:
CLOSED: [2025-01-10]
:PROPERTIES:
:DISCOVERED: 2025-01-10
:FIXED: 2025-01-10
...
Fixed in interpreter.rs lines 1359-1363. When catching Str with no arena_index, now calls insert_string() to create proper Str struct.

*** Old Bug Report (FALSE POSITIVE)
...
*** Current State
- =src/test/enums.til= - ✅ ALL TESTS PASSING including enum return tests
