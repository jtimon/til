            ...
            ValueType::TType(TTypeDef::TStructDef) => {
                ...
                match &after_dot.node_type {
                    NodeType::Identifier(after_dot_name) => {
                        ...
                        let af_combined_name = format!("{}.{}", f_name, after_dot_name);
                        if let Some(func_def) = context.scope_stack.lookup_func(&af_combined_name) {
                            return value_type_func_proc(&context.path, &e, &af_combined_name, func_def);
                        }
                        ...
                        let member_default_value = match struct_def.default_values.get(after_dot_name) {
                            ...
                            None => {
                                return Err(e.error(&context.path, "init", &format!("struct '{}' has no member '{}'", f_name, after_dot_name)));
                            },
                        };
            ...
            ValueType::TType(TTypeDef::TEnumDef) => {
                ...
                match &enum_after_dot_expr.node_type {
                    NodeType::Identifier(variant_name) => {
                        // First check for associated function (namespace-defined)
                        let af_combined_name = format!("{}.{}", f_name, variant_name);
                        if let Some(func_def) = context.scope_stack.lookup_func(&af_combined_name) {
                            return value_type_func_proc(&context.path, &e, &af_combined_name, func_def);
                        }
                        ...
                        let variant_type = match enum_def.get(variant_name) {
                            ...
                            None => {
                                return Err(e.error(&context.path, "init", &format!("enum '{}' has no variant '{}' and no associated function '{}'", f_name, variant_name, af_combined_name)));
                            },
                        };
    ...
    match &e.node_type {
        ...
        NodeType::NamespaceDef(ns_def) => {
            ...
            for member_decl in &ns_def.members {
                let ns_full_name = format!("{}.{}", ns_def.type_name, member_decl.name);

                // Check for name collisions
                if context.scope_stack.lookup_func(&ns_full_name).is_some() || context.scope_stack.lookup_symbol(&ns_full_name).is_some() {
                    errors.push(e.error(&context.path, "init", &format!(
                ...
                if let Some(ns_member_expr) = ns_def.default_values.get(&member_decl.name) {
                    let ns_member_value_type = match get_value_type(&context, ns_member_expr) {
                        Ok(vt) => vt,
                        Err(err) => {
                            errors.push(err);
                            continue;
                    ...
                    // Register in symbols
                    context.scope_stack.declare_symbol(ns_full_name.clone(), SymbolInfo {
                        value_type: ns_member_value_type.clone(),
                    ...
                    // If it's a function, also register in funcs
                    if let NodeType::FuncDef(func_def) = &ns_member_expr.node_type {
