...
fn compute_reachable(
    ...
    for type_name in &used_types_snapshot {
        // Skip primitive types - they don't have user-defined eq methods
    ...
    // Bug #159: Mark struct/enum clone() and delete() methods as reachable when the type is used
    // These are auto-generated by preinit and called internally by ccodegen for assignments
    for type_name in &used_types_snapshot {
        // Check if this is a struct or enum with clone/delete methods
        if context.scope_stack.has_struct(type_name) || context.scope_stack.has_enum(type_name) {
            ...
            if context.scope_stack.has_func(&clone_method_name) {
                mark_reachable(clone_method_name, &mut reachable, &mut worklist);
            }
            let delete_method_name = format!("{}.delete", type_name);
            if context.scope_stack.has_func(&delete_method_name) {
                mark_reachable(delete_method_name, &mut reachable, &mut worklist);
            }
        }
    }

    // Process any newly added eq/clone/delete methods
    while let Some(func_name) = worklist.pop() {
        if let Some(func_def) = context.scope_stack.lookup_func(&func_name) {
            collect_used_types_from_func(context, &func_def, &mut used_types);

            // Bug #159: Check for variadic params in newly added methods too
            for arg in &func_def.args {
                if let crate::rs::parser::ValueType::TMulti(_) = &arg.value_type {
                    needs_variadic_support = true;
                    break;
