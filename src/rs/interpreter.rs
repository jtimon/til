...
/// Try to call Type.clone(source) and return the result.
/// Returns Ok(Some(result)) if clone was called successfully, Ok(None) if no clone method exists.
/// This is used to implement deep copy semantics for `mut x := y` where y is a struct.
fn try_call_clone(context: &mut Context, type_name: &str, source_name: &str, e: &Expr) -> Result<Option<EvalResult>, String> {
    let clone_method_name = format!("{}.clone", type_name);

    // Check if clone method exists
    if context.scope_stack.lookup_func(&clone_method_name).is_none() {
        return Ok(None);
    }

    // Build FCall expression: Type.clone(source)
    // The AST structure is: FCall[ Identifier("Type")[Identifier("clone")], Identifier(source) ]
    let clone_id = Expr::new_explicit(
    ...
    let type_id = Expr::new_explicit(
        ...
        vec![clone_id],  // "clone" is a sub-identifier of "Type"
        e.line,
        e.col,
    );

    // Build source identifier - handle dotted names like "a.field1"
    let source_id = if source_name.contains('.') {
        // Build nested identifiers for dotted path: "a.b.c" -> Identifier("a")[Identifier("b")[Identifier("c")]]
        let parts: Vec<&str> = source_name.split('.').collect();
        let mut current = Expr::new_explicit(
            NodeType::Identifier(parts.last().unwrap().to_string()),
            vec![],
            e.line,
            e.col,
        );
        for part in parts.iter().rev().skip(1) {
            current = Expr::new_explicit(
                NodeType::Identifier(part.to_string()),
                vec![current],
                e.line,
                e.col,
            );
        ...
        Expr::new_explicit(
            NodeType::Identifier(source_name.to_string()),
            vec![],
            e.line,
            e.col,
        )
    ...
    let fcall = Expr::new_explicit(
        NodeType::FCall(false),
        vec![type_id, source_id],
        e.line,
        e.col,
    );

    // Evaluate the clone call
    let result = eval_expr(context, &fcall)?;
    Ok(Some(result))
}
                ...
                _ => {
                    ...
                    if custom_symbol.value_type == ValueType::TType(TTypeDef::TEnumDef) {
                        ...
                        if declaration.is_mut && !is_temp_return_val {
                            ...
                            // Call Type.clone(source) - this properly clones Ptr/Vec fields
                            if let Some(clone_result) = try_call_clone(context, custom_type_name, &expr_result_str, e)? {
                                if clone_result.is_throw {
                                    return Ok(clone_result); // Propagate throw
                                }
                                // Clone returns a temp with properly cloned nested fields (Ptr, Vec, etc.)
                                // We still need to allocate our own memory and copy from the clone result
                                // because some clone() implementations just return self (like Bool.clone)
                                insert_struct_instance(context, &declaration.name, custom_type_name, e)?;
                                EvalArena::copy_fields(context, custom_type_name, &clone_result.value, &declaration.name, e)?;
                            } else {
