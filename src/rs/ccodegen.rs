...
/// Issue #175: Check if a type name refers to a struct or enum with data fields.
/// Returns true for user-defined structs with mutable data fields (Point, Expr, etc.)
/// and enum definitions. These are the types that generate C struct typedefs
/// and benefit from destination-passing style (_ret out-parameter).
/// Returns false for primitives (I64, Bool, Str -- they are StructDefs with
/// namespace methods but no mutable data fields), FuncSig types, and any
/// type not found in scope.
fn is_struct_or_enum_type(type_name: &str, context: &Context) -> bool {
    if let Some(sym) = context.scope_stack.lookup_symbol(type_name) {
        match &sym.value_type {
            ValueType::TType(TTypeDef::TStructDef) => {
                // Only structs with mutable data fields generate C struct typedefs.
                // Primitives like I64, Bool, Str are StructDefs (they have namespace
                // methods merged into members) but have no mutable data fields --
                // they map to C primitives.
                if let Some(struct_def) = context.scope_stack.lookup_struct(type_name) {
            ...
            ValueType::TType(TTypeDef::TEnumDef) => true,
            _ => false,
        }
    } else {
        false
...
/// Issue #175: Check if a non-throwing function call should use _ret out-parameter.
/// Returns true when the called function:
/// - Has return types (struct or enum type, NOT primitives or FuncSig)
/// - Is NOT external (ext functions keep their original C signatures)
/// - Is NOT throwing (throwing functions already use _ret)
fn fcall_uses_out_ret(func_def: &FuncDef, context: &Context) -> bool {
    if func_def.sig.return_types.is_empty() || func_def.is_ext() || !func_def.sig.throw_types.is_empty() {
        return false;
    }
    match func_def.sig.return_types.first() {
        Some(ValueType::TCustom(name)) => is_struct_or_enum_type(name, context),
        _ => false,
    }
}
...
fn get_struct_dependencies(expr: &Expr) -> Result<Vec<String>, String> {
    ...
    if let NodeType::Declaration(_) = &expr.node_type {
        if !expr.params.is_empty() {
            if let NodeType::StructDef(struct_def) = &expr.params.get(0).unwrap().node_type {
...
fn get_struct_name(expr: &Expr) -> Result<String, String> {
    if let NodeType::Declaration(decl) = &expr.node_type {
        if !expr.params.is_empty() {
            if let NodeType::StructDef(_) = &expr.params.get(0).unwrap().node_type {
...
fn get_enum_dependencies(expr: &Expr) -> Result<Vec<String>, String> {
    ...
    if let NodeType::Declaration(_) = &expr.node_type {
        if !expr.params.is_empty() {
            if let NodeType::EnumDef(enum_def) = &expr.params.get(0).unwrap().node_type {
...
fn get_enum_name(expr: &Expr) -> Result<String, String> {
    if let NodeType::Declaration(decl) = &expr.node_type {
        if !expr.params.is_empty() {
            if let NodeType::EnumDef(_) = &expr.params.get(0).unwrap().node_type {
                return Ok(decl.name.clone());
            }
        }
    }
...
fn hoist_variadic_args(
    ...
    // Issue #175: til_Array_new(&arr, type_temp, &count_temp);
    output.push_str(&indent_str);
    output.push_str(TIL_PREFIX);
...
fn detect_variadic_fcall(
    ...
    let orig_func_name = match get_til_func_name_string(expr.params.get(0).unwrap()) {
        Some(name) => name,
        None => return Ok(None),
    };
    ...
    if param_by_ref {
        ...
        let expr_str = if let NodeType::FCall(_) = &arg.node_type {
            ...
            if name == "_" && !arg.params.is_empty() {
                if let NodeType::FCall(_) = &arg.params.get(0).unwrap().node_type {
                    let base_str = emit_arg_string(arg.params.get(0).unwrap(), None, false, hoist_output, indent, ctx, context)?;
...
fn emit_throwing_arg_string(
    ...
    // Get function name
    let func_name = get_func_name_string(arg.params.get(0).unwrap())
        .ok_or_else(|| arg.lang_error(&context.path, "ccodegen", "Cannot determine function name"))?;
    let orig_func_name = get_til_func_name_string(arg.params.get(0).unwrap()).unwrap_or_else(|| func_name.clone());
...
fn emit_variadic_arg_string(
    ...
    // Issue #175: Use _ret out-parameter for non-ext functions
    let uses_out_ret = fcall_uses_out_ret(&fd, context);
    if uses_out_ret {
        ...
        // Ext function or no return - old style
        hoist_output.push_str(&temp_var);
        hoist_output.push_str(" = ");
        hoist_output.push_str(&til_func_name(&mangled_name));
        hoist_output.push('(');
...
fn emit_fcall_arg_string(
    ...
    match orig_func_name.as_str() {
        ...
        "to_ptr" => {
            ...
            let inner_arg = arg.params.get(1).unwrap();
        ...
        "size_of" => {
            ...
            let type_arg = arg.params.get(1).unwrap();
        ...
        "enum_get_payload_type" => {
            ...
            let ept_enum_arg = arg.params.get(1).unwrap();
            let variant_arg = arg.params.get(2).unwrap();
            let ept_payload_type_arg = arg.params.get(3).unwrap();
    ...
    // Issue #175: Check if this function uses _ret out-parameter
    // (non-throwing, has return type, not ext)
    let has_ret = fd_opt.as_ref()
        .map(|fd| fcall_uses_out_ret(fd, context))
        .unwrap_or(false);
    ...
    // Issue #175: Non-throwing functions with return values use _ret out-parameter
    ...
    if has_ret && !is_func_ptr_call {
        ...
        // Declare temp
        hoist_output.push_str(&indent_str);
        hoist_output.push_str(&ret_type);
        hoist_output.push_str(" ");
        hoist_output.push_str(&temp_var);
        hoist_output.push_str(";\n");

        // Emit call: func(&temp, args...);
        hoist_output.push_str(&indent_str);
        call_str.push_str("&");
        call_str.push_str(&temp_var);
        for arg_str in nested_arg_strings.iter() {
            call_str.push_str(", ");
            call_str.push_str(arg_str);
        }
        call_str.push_str(");\n");
        hoist_output.push_str(&call_str);
...
pub fn emit(ast: &Expr, context: &mut Context) -> Result<String, String> {
    ...
    output.push_str(&format!("        if ({p}raw_strcmp(argv[_i], \"--mem-report\") == 0) {{\n", p=TIL_PREFIX));
    ...
    if context.mode_def.needs_main_proc {
        if main_has_variadic {
            ...
            // Issue #175: Array_new uses _ret out-parameter
...
fn emit_constant_declaration(expr: &Expr, output: &mut String, ctx: &mut CodegenContext, context: &Context) -> Result<(), String> {
    if let NodeType::Declaration(decl) = &expr.node_type {
        if !expr.params.is_empty() {
            ...
            if let NodeType::LLiteral(lit) = &expr.params.get(0).unwrap().node_type {
            ...
            else if let NodeType::Identifier(name) = &expr.params.get(0).unwrap().node_type {
...
fn emit_global_declaration(expr: &Expr, output: &mut String, ctx: &mut CodegenContext, context: &Context) -> Result<(), String> {
    if let NodeType::Declaration(decl) = &expr.node_type {
        if !expr.params.is_empty() {
            ...
            let c_type = match get_value_type(context, expr.params.get(0).unwrap()) {
...
fn collect_nested_func_info(expr: &Expr, ctx: &mut CodegenContext, context: &Context, parent_func_name: Option<&str>) -> Result<(), String> {
    match &expr.node_type {
        NodeType::Declaration(decl) => {
            if !expr.params.is_empty() {
                if let NodeType::FuncDef(func_def) = &expr.params.get(0).unwrap().node_type {
        ...
        NodeType::If => {
            ...
            if expr.params.len() > 1 {
                collect_nested_func_info(expr.params.get(1).unwrap(), ctx, context, parent_func_name)?;
            }
            if expr.params.len() > 2 {
                collect_nested_func_info(expr.params.get(2).unwrap(), ctx, context, parent_func_name)?;
            }
        },
        NodeType::While => {
...
fn emit_struct_func_body(struct_name: &str, member: &crate::rs::parser::Declaration, func_def: &FuncDef, output: &mut String, ctx: &mut CodegenContext, context: &mut Context) -> Result<(), String> {
    ...
    // Issue #175: Suppress -Wunused-parameter for _ret (some functions don't use all code paths)
    // Only emit when _ret parameter exists (throwing, or non-throwing with _ret)
    if !func_def.sig.return_types.is_empty()
        && (!func_def.sig.throw_types.is_empty() || fcall_uses_out_ret(func_def, context)) {
        output.push_str("    (void)_ret;\n");
    }
    ...
    if !func_def.sig.throw_types.is_empty() {
        ...
        output.push_str("    return 0;\n");
    } else if !func_def.sig.return_types.is_empty() && !fcall_uses_out_ret(func_def, context) {
        // Non-struct/enum-returning functions need fallback return (primitives, FuncSig, etc.)
        let ret_type = til_type_to_c(&func_def.sig.return_types[0])?;
        output.push_str(&format!("    return ({}){{}};\n", ret_type));
    }
    // Issue #175: Non-throwing functions with _ret are void, no fallback return needed
...
// For non-throwing with return type (Issue #175):
//   void func_name(RetType* _ret, args...)
// For non-throwing without return type:
//   void func_name(args...)
...
fn emit_func_signature(func_name: &str, func_def: &FuncDef, context: &Context, output: &mut String) -> Result<(), String> {
    ...
    // Issue #175: Determine if this non-throwing function should use _ret out-parameter
    let non_throwing_uses_ret = !is_throwing && fcall_uses_out_ret(func_def, context);

    if is_throwing {
        // Throwing function returns int status code
        output.push_str("int ");
    } else if non_throwing_uses_ret {
        // Issue #175: Non-throwing functions with custom return types use _ret out-parameter
        output.push_str("void ");
    } else if func_def.sig.return_types.is_empty() {
        output.push_str("void ");
    } else {
        // Non-throwing function with primitive/FuncSig return - keep original signature
        let ret_type = til_type_to_c(&func_def.sig.return_types[0])?;
    ...
    // _ret pointer for throwing functions (always) and non-throwing with custom return types
    if is_throwing && !func_def.sig.return_types.is_empty() {
        let ret_type = til_type_to_c(&func_def.sig.return_types[0])?;
        output.push_str(&ret_type);
...
fn is_func_declaration(expr: &Expr) -> Result<bool, String> {
    if let NodeType::Declaration(_) = &expr.node_type {
        if !expr.params.is_empty() {
            if let NodeType::FuncDef(_) = &expr.params.get(0).unwrap().node_type {
                return Ok(true);
            }
        }
    }
    ...
    match &expr.node_type {
        ...
        NodeType::Identifier(name) => {
            ...
            if name == "_" && !expr.params.is_empty() {
                let base_expr = expr.params.get(0).unwrap();
                // Issue #175: If base is an FCall that uses _ret, hoist to temp for field access
                if let NodeType::FCall(_) = &base_expr.node_type {
                    if let Some(fd) = get_fcall_func_def(context, base_expr) {
                        if fcall_uses_out_ret(&fd, context) {
                            // Use emit_arg_string which handles hoisting for _ret functions
                            let base_str = emit_arg_string(base_expr, None, false, output, indent, ctx, context)?;
                            output.push_str(&base_str);
                            for i in 1..expr.params.len() {
                                if let NodeType::Identifier(field) = &expr.params.get(i).unwrap().node_type {
                                    output.push_str(".");
                ...
                emit_expr(base_expr, output, indent, ctx, context)?;
            ...
            if !expr.params.is_empty() {
                if let NodeType::Identifier(field) = &expr.params.get(0).unwrap().node_type {
    ...
    for catch_block in &func_level_catches {
        if catch_block.params.len() >= 3 {
            ...
            if let NodeType::Identifier(err_var_name) = &catch_block.params.get(0).unwrap().node_type {
                if let NodeType::Identifier(err_type_name) = &catch_block.params.get(1).unwrap().node_type {
            ...
            // Bug #97: Use type-mangled names for shadowing support
            let catch_body = catch_block.params.get(2).unwrap();
    ...
    for (scan_idx, scan_stmt) in stmts.iter().enumerate() {
        if let NodeType::Catch = scan_stmt.node_type {
            if scan_stmt.params.len() >= 3 {
                if let NodeType::Identifier(err_type_name) = &scan_stmt.params.get(1).unwrap().node_type {
    ...
    while i < stmts.len() {
        ...
        if let NodeType::Catch = stmt.node_type {
            ...
            if stmt.params.len() >= 3 {
                if let NodeType::Identifier(err_type_name) = &stmt.params.get(1).unwrap().node_type {
        ...
        let (maybe_fcall, maybe_decl_name, maybe_assign_name) = match &stmt.node_type {
            ...
            NodeType::Declaration(decl) => {
                ...
                if !stmt.params.is_empty() {
                    if let NodeType::FCall(_) = stmt.params.get(0).unwrap().node_type {
                        (Some(stmt.params.get(0).unwrap()), Some(decl.name.clone()), None)
                    } else {
                        (None, None, None)
                    }
                } else {
            ...
            NodeType::Assignment(name) => {
                ...
                if !stmt.params.is_empty() {
                    if let NodeType::FCall(_) = stmt.params.get(0).unwrap().node_type {
                        (Some(stmt.params.get(0).unwrap()), None, Some(name.clone()))
                    } else {
                        (None, None, None)
                    }
                } else {
        ...
        if let Some(fcall) = maybe_fcall {
            ...
            if let Some(func_name) = get_til_func_name_string(fcall.params.get(0).unwrap()) {
...
fn emit_variadic_call(
    ...
    let func_name = get_fcall_func_name(fcall)
        .ok_or_else(|| fcall.error(&context.path, "ccodegen", "Cannot determine function name (emit_variadic_call)"))?;
    ...
    // Issue #175: Check if function uses _ret out-parameter (non-ext, non-throwing, has return)
    let func_has_return = func_def_opt.as_ref()
        .map(|fd| fcall_uses_out_ret(fd, context))
        .unwrap_or(false);

    // Helper to emit the function call with pre-computed arg_strings
    // Issue #175: dest_ptr is the _ret destination (e.g. "&var" or "&(Type){}"); empty = no _ret
    let emit_call = |out: &mut String, dest_ptr: &str| {
        out.push_str(TIL_PREFIX);
        out.push_str(&func_name.replace('.', "_"));
        out.push_str("(");
        let mut first_param = true;
        // Issue #175: _ret parameter first
        if !dest_ptr.is_empty() {
            out.push_str(dest_ptr);
            first_param = false;
        }
    ...
    if let Some(var_name) = decl_name {
        if var_name == "_" {
            ...
            output.push_str(&indent_str);
            if func_has_return {
                emit_call(output, &format!("&(({}){{}})", ret_type));
            } else {
                emit_call(output, "");
            }
            output.push_str(";\n");
            ...
            // Issue #175: Emit call with &var as _ret destination
            output.push_str(&indent_str);
            if func_has_return {
                emit_call(output, &format!("&{}", c_var_name));
            } else {
                // Ext functions return by value (not _ret), so use assignment
                let has_return = func_def_opt.as_ref()
        ...
        // Assignment: Issue #175: use build_dest_ptr_expr style
        output.push_str(&indent_str);
        if func_has_return {
            let dest = build_dest_ptr_expr(None, Some(var_name), ctx, context)
                .unwrap_or_else(|| {
                    // Fallback: resolve var name
                    format!("&{}", resolve_var_name(var_name, ctx, context))
                });
            emit_call(output, &dest);
        } else {
            // Ext functions return by value (not _ret), so use assignment
            let has_return = func_def_opt.as_ref()
                .map(|fd| !fd.sig.return_types.is_empty())
                .unwrap_or(false);
            if has_return {
                let resolved = resolve_var_name(var_name, ctx, context);
...
fn emit_throwing_call(
    ...
    // Get function name
    let mut func_name = get_fcall_func_name(fcall)
        .ok_or_else(|| fcall.error(&context.path, "ccodegen", "Cannot determine function name (emit_throwing_call)"))?;
...
fn emit_throwing_call_propagate(
    ...
    // Get function name
    let mut func_name = get_fcall_func_name(fcall)
        .ok_or_else(|| fcall.error(&context.path, "ccodegen", "Cannot determine function name"))?;
...
fn emit_declaration(decl: &crate::rs::parser::Declaration, expr: &Expr, output: &mut String, indent: usize, ctx: &mut CodegenContext, context: &mut Context) -> Result<(), String> {
    ...
    if !expr.params.is_empty() {
        if let NodeType::FuncDef(func_def) = &expr.params.get(0).unwrap().node_type {
            ...
            if !ctx.current_function_name.is_empty() {
                ...
                if !func_def.sig.return_types.is_empty()
                    && (!func_def.sig.throw_types.is_empty() || fcall_uses_out_ret(func_def, context)) {
                    func_output.push_str("    (void)_ret;\n");
                }
    ...
    if !expr.params.is_empty() {
        let rhs = expr.params.get(0).unwrap();
        if let NodeType::FCall(_) = &rhs.node_type {
            let cast_f_name = get_func_name_string(rhs.params.get(0).unwrap());
            if cast_f_name.as_deref() == Some("cast") && rhs.params.len() >= 3 {
                let cast_type_name = if let NodeType::Identifier(name) = &rhs.params.get(1).unwrap().node_type {
                    name.clone()
                } else {
                    return Err("ccodegen: cast: first argument must be a type name".to_string());
                };
                let cast_ptr_expr = rhs.params.get(2).unwrap();
    ...
    let rhs_string: Option<String> = if !expr.params.is_empty() {
        ...
        if let NodeType::FCall(_) = &rhs.node_type {
            ...
            // Bug #157: Check for UFCS field access on FCall
            // Pattern: Identifier("_") with params[0] = FCall, params[1..] = field chain
            // Issue #175: Handle all UFCS FCalls (not just throwing) to capture return values
            if name == "_" && !rhs.params.is_empty() {
                if let NodeType::FCall(_) = &rhs.params.get(0).unwrap().node_type {
                    if let Some(fd) = get_fcall_func_def(context, rhs.params.get(0).unwrap()) {
                        if !fd.sig.return_types.is_empty() {
                            // Hoist the call to capture return value
                            let base_temp = emit_arg_string(rhs.params.get(0).unwrap(), None, false, output, indent, ctx, context)?;
    ...
    if decl.name == "_" {
        if !expr.params.is_empty() {
            ...
            let is_fcall = matches!(&expr.params.get(0).unwrap().node_type, NodeType::FCall(_));
            if is_fcall {
                emit_expr(expr.params.get(0).unwrap(), output, indent, ctx, context)?;
            } else {
                emit_expr(expr.params.get(0).unwrap(), output, 0, ctx, context)?;
                output.push_str(";\n");
            }
        }
        return Ok(());
    ...
    let struct_type = if !expr.params.is_empty() {
        let s = get_struct_construction_type(expr.params.get(0).unwrap(), context)?;
    ...
    let enum_type = if !expr.params.is_empty() {
        let s = get_enum_construction_type(expr.params.get(0).unwrap(), context)?;
        if s.is_empty() { None } else { Some(s) }
    } else {
        None
    };
    ...
    let var_type = if let Some(ref type_name) = struct_type {
        ...
        if !expr.params.is_empty() {
            get_value_type(context, expr.params.get(0).unwrap()).ok()
    ...
    if let Some(type_name) = struct_type {
        ...
        if !expr.params.is_empty() {
            for arg in expr.params.get(0).unwrap().params.iter().skip(1) {
...
fn emit_assignment(name: &str, expr: &Expr, output: &mut String, indent: usize, ctx: &mut CodegenContext, context: &mut Context) -> Result<(), String> {
    ...
    let rhs_str: Option<String> = if !expr.params.is_empty() {
        let rhs_expr = expr.params.get(0).unwrap();
...
fn emit_return(expr: &Expr, output: &mut String, indent: usize, ctx: &mut CodegenContext, context: &mut Context) -> Result<(), String> {
    ...
    if is_throwing {
        ...
        if !expr.params.is_empty() {
            let return_expr = expr.params.get(0).unwrap();
        ...
        // Non-throwing function return
        // Issue #175: Non-throwing functions with struct/enum return types use *_ret = value; return;
        // Only struct/enum types get _ret; primitives and FuncSig keep normal return
        let has_ret = match ctx.current_return_types.first() {
            Some(ValueType::TCustom(name)) => is_struct_or_enum_type(name, context),
            _ => false,
        };

        if has_ret && !expr.params.is_empty() {
            let return_expr = expr.params.get(0).unwrap();

            // Check for throwing call in return expression
            if let NodeType::FCall(_) = return_expr.node_type {
                if let Some(throw_types) = get_fcall_func_def(context, return_expr).map(|fd| fd.sig.throw_types).filter(|t| !t.is_empty()) {
                    // Return expression is a throwing function call - emit with error propagation
                    // The result will be stored via the assign_name "*_ret"
                ...
                // Check for variadic call in return
                if let Some(variadic_fcall_info) = detect_variadic_fcall(return_expr, ctx)? {
                    ...
                    // Issue #175: Emit variadic call writing to _ret
                    output.push_str(&indent_str);
                    if let NodeType::Identifier(func_name) = &return_expr.params.get(0).unwrap().node_type {
                        output.push_str(&til_func_name(func_name));
                    }
                    output.push_str("(_ret");
            ...
            // Return with no expression in a function with return type - bare return
            output.push_str(&indent_str);
            output.push_str("return;\n");
        } else if !expr.params.is_empty() {
            // Return with expression but no _ret (non-struct/enum return types like I64, Bool, Str)
            // Emit normal C return statement
            let return_str = emit_arg_string(expr.params.get(0).unwrap(), None, false, output, indent, ctx, context)?;
            output.push_str(&indent_str);
            output.push_str("return ");
            output.push_str(&return_str);
            output.push_str(";\n");
...
fn emit_throw(expr: &Expr, output: &mut String, indent: usize, ctx: &mut CodegenContext, context: &mut Context) -> Result<(), String> {
    ...
    let nh_thrown_type_name = match &thrown_expr.node_type {
        NodeType::FCall(_) => {
            if !thrown_expr.params.is_empty() {
                if let NodeType::Identifier(name) = &thrown_expr.params.get(0).unwrap().node_type {
    ...
    match nh_error_index {
        ...
        None => Err(format!(
            "ccodegen: thrown type '{}' not found in function's throw types: {:?} (in function '{}')",
            nh_thrown_type_name, ctx.current_throw_types, ctx.current_function_name
        )),
    }
}
...
fn emit_if(expr: &Expr, output: &mut String, indent: usize, ctx: &mut CodegenContext, context: &mut Context) -> Result<(), String> {
    ...
    // (TIL has function-level scoping, not block-level scoping)
    // Bug #97: Use type-mangled names for shadowing support
    let then_decls = collect_declarations_in_body(expr.params.get(1).unwrap(), context)?;
    ...
    if expr.params.len() > 2 {
        let hoist_else_decls = collect_declarations_in_body(expr.params.get(2).unwrap(), context)?;
    ...
    let cond_str = emit_arg_string(expr.params.get(0).unwrap(), None, false, output, indent, ctx, context)?;
    output.push_str(&indent_str);
    output.push_str("if (");
    output.push_str(&cond_str);
    // Bool is a struct with .data field - extract for C truthiness
    if let Ok(crate::rs::parser::ValueType::TCustom(type_name)) = crate::rs::init::get_value_type(context, expr.params.get(0).unwrap()) {
        if type_name == "Bool" {
            output.push_str(".data");
        }
    }
    ...
    let saved_ref_params = ctx.current_ref_params.clone();
    emit_body(expr.params.get(1).unwrap(), output, indent + 1, ctx, context)?;
    ctx.current_ref_params = saved_ref_params;
    ...
    if expr.params.len() > 2 {
        ...
        if let NodeType::If = &expr.params.get(2).unwrap().node_type {
            // Always wrap else-if in braces to ensure hoisted temp vars
            // from nested if conditions have proper scope
            output.push_str(" else {\n");
            let saved_ref_params = ctx.current_ref_params.clone();
            emit_if(expr.params.get(2).unwrap(), output, indent + 1, ctx, context)?;
            ctx.current_ref_params = saved_ref_params;
            output.push_str(&indent_str);
            output.push_str("}\n");
        } else {
            output.push_str(" else {\n");
            let saved_ref_params = ctx.current_ref_params.clone();
            emit_body(expr.params.get(2).unwrap(), output, indent + 1, ctx, context)?;
            ctx.current_ref_params = saved_ref_params;
            output.push_str(&indent_str);
            output.push_str("}\n");
        }
...
fn emit_while(expr: &Expr, output: &mut String, indent: usize, ctx: &mut CodegenContext, context: &mut Context) -> Result<(), String> {
    ...
    // (TIL has function-level scoping, not block-level scoping)
    // Bug #97: Use type-mangled names for shadowing support
    let body_decls = collect_declarations_in_body(expr.params.get(1).unwrap(), context)?;
    ...
    let cond_str = emit_arg_string(expr.params.get(0).unwrap(), None, false, &mut hoist_buffer, indent + 1, ctx, context)?;
...
fn collect_declarations_recursive(expr: &Expr, decls: &mut Vec<CollectedDeclaration>, collected: &mut std::collections::HashMap<String, ValueType>, context: &mut Context) -> Result<(), String> {
    match &expr.node_type {
        ...
        NodeType::If => {
            ...
            if expr.params.len() >= 2 {
                collect_declarations_recursive(expr.params.get(1).unwrap(), decls, collected, context)?;
            }
            if expr.params.len() >= 3 {
                collect_declarations_recursive(expr.params.get(2).unwrap(), decls, collected, context)?;
            }
        }
        NodeType::While => {
...
fn emit_fcall(expr: &Expr, output: &mut String, indent: usize, ctx: &mut CodegenContext, context: &mut Context) -> Result<(), String> {
    ...
    if expr.params.is_empty() {
        return Err(expr.error(&context.path, "ccodegen", "FCall with no params"));
    }

    // Get function name (handles both nested identifiers and precomp'd "Type.method" strings)
    let mut func_name = match get_func_name_string(expr.params.get(0).unwrap()) {
        Some(name) => name,
        None => return Err(expr.error(&context.path, "ccodegen", "FCall first param not Identifier")),
    };

    // For builtins, we need the original name with dots
    let orig_func_name = get_til_func_name_string(expr.params.get(0).unwrap())
    ...
    match orig_func_name.as_str() {
        ...
        "type_as_str" => {
            ...
            if let NodeType::Identifier(type_name) = &expr.params.get(1).unwrap().node_type {
                // Check if this is a Type variable or a literal type name
        ...
        "enum_to_str" => {
            ...
            let arg = expr.params.get(1).unwrap();
        ...
        _ => {
            ...
            if expr.params.get(0).unwrap().params.is_empty() && expr.params.len() >= 2 {
                if let NodeType::FCall(_) = &expr.params.get(1).unwrap().node_type {
                    // The second param is an FCall result - use get_value_type to get its return type
                    if let Ok(fcall_ret_type) = get_value_type(context, expr.params.get(1).unwrap()) {
            ...
            // Issue #175: For non-ext, non-throwing functions with return values,
            // add _ret out-parameter (result is discarded at statement level)
            let fcall_fd = get_fcall_func_def(context, expr);
            let fcall_has_ret = fcall_fd.as_ref()
                .map(|fd| fcall_uses_out_ret(fd, context))
                .unwrap_or(false);
            let mut fcall_has_emitted_ret = false;
            if fcall_has_ret && !is_func_ptr_call {
                // Use compound literal as discard destination for return value
                let ret_type = fcall_fd.as_ref()
                    .and_then(|fd| fd.sig.return_types.first())
                    .map(|t| til_type_to_c(t).unwrap_or_else(|_| "int".to_string()))
                    .unwrap_or_else(|| "int".to_string());
                output.push_str(&format!("&(({}){{}})", ret_type));
                fcall_has_emitted_ret = true;
            }
            ...
            if let Some(variadic_info) = ctx.func_variadic_args.get(&orig_func_name) {
                ...
                if is_stmt_level {
                    ...
                    for (vstmt_arg_i, vstmt_arg_str) in arg_strings.iter().take(regular_count).enumerate() {
                        if fcall_has_emitted_ret || vstmt_arg_i > 0 {
                            output.push_str(", ");
                        }
                    ...
                    for (vexpr_arg_i, vexpr_arg) in expr.params.iter().skip(1).take(regular_count).enumerate() {
                        if fcall_has_emitted_ret || vexpr_arg_i > 0 {
                            output.push_str(", ");
                        }
                ...
                // Emit variadic array pointer
                if let Some(ref arr_var) = variadic_arr_var {
                    if fcall_has_emitted_ret || regular_count > 0 {
                        output.push_str(", ");
                    }
                    output.push_str("&");
                    output.push_str(arr_var);
                ...
                if is_stmt_level {
                    for (nstmt_arg_i, nstmt_arg_str) in arg_strings.iter().enumerate() {
                        if fcall_has_emitted_ret || nstmt_arg_i > 0 {
                            output.push_str(", ");
                        }
