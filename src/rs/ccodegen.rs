...
fn emit_expr(expr: &Expr, output: &mut String, indent: usize, ctx: &mut CodegenContext, context: &mut Context) -> Result<(), String> {
    ...
    match &expr.node_type {
        ...
        NodeType::Switch => Err(expr.lang_error(&context.path, "ccodegen", "Switch should have been desugared before ccodegen")),
        NodeType::DefaultCase => Err(expr.lang_error(&context.path, "ccodegen", "DefaultCase should have been desugared before ccodegen")),
        NodeType::Range => Err(expr.lang_error(&context.path, "ccodegen", "Range should have been desugared before ccodegen")),
        NodeType::Pattern(_) => Err(expr.lang_error(&context.path, "ccodegen", "Pattern should have been desugared before ccodegen")),
