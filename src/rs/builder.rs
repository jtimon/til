...
use crate::rs::lexer::{lexer_from_source, LANG_NAME_141};
...
pub fn build(path: &str, target: &Target, lang: &Lang, translate_only: bool) -> Result<String, String> {
    ...
    // Bug #141: Separate gen dirs for rstil (rs) vs rstil_til (til)
    let c_filename = path.replace(".til", ".c");
    let source_output_path = if c_filename.starts_with("src/") {
        c_filename.replacen("src/", &format!("gen/{}/c/", LANG_NAME_141), 1)
    } else {
        format!("gen/{}/c/{}", LANG_NAME_141, c_filename)
    };
    ...
    // Bug #141: Separate bin dirs for rstil (rs) vs rstil_til (til)
    ...
    let exe_path_str = if bin_filename.starts_with("src/") {
        bin_filename.replacen("src/", &format!("bin/{}/", LANG_NAME_141), 1)
    } else {
        format!("bin/{}/{}", LANG_NAME_141, bin_filename)
    };
