...
- `make` — build bin/til_boot (also regenerates boot/)
- `make test` — build + run all tests
- `bin/til_boot run file.til` — compile and run via builder
- `bin/til_boot interpret file.til` — run via interpreter
...
xvfb-run bin/til_boot run src/examples/gui_hello.til
```
...
For build-only testing (no window needed), use `bin/til_boot build` which just compiles
without running. The test suite already handles this — GUI tests are build-only.

## Generated Files (boot/) — DO NOT EDIT

NEVER edit boot/ files. Not to fix errors, not to "help" the
first pass compile, not for any reason. If the build fails with
errors pointing at boot/, the fix is ALWAYS in the .til source
files (src/self/, src/til.til, src/core/) or in src/c/ (the C modules
that the generated code links against). boot/ is regenerated by
`make` via `til_core` — manual edits get overwritten and mask the
real problem.

- `boot/modes.c` + `boot/modes_*.c/h` generated from `src/self/modes.til`
  (imports the full chain: modes → initer → parser → lexer, plus core/ast)
- `boot/til.c` + `boot/til_*.c/h` generated from `src/til.til`
...
- When you break something during development (bad bootstrap, etc.), restore from HEAD:
  `git checkout HEAD -- boot/` — not git stash, not git reset.
