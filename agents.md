...
- `src/bootstrap/core.c` and `src/bootstrap/core.h` are generated by `make lib`
  (or `make`) from `src/core/core.til`. NEVER edit them manually.
...
- `make` and `make test` are expensive (full recompile + test suite).
  ...
  Example: `make 2>&1 | tee tmp/build.log` then use Read/Grep on `tmp/build.log`.
